...
// Mini router mirroring banking.ts's "banking" feature wiring: a view
// (matrixProcedure) and a create (writeProcedure + matrixMiddleware) endpoint.
const testRouter = createTRPCRouter({
  bankingView: matrixProcedure("banking", "view").query(() => "ok" as const),
  bankingCreate: writeProcedure.use(matrixMiddleware("banking", "create")).mutation(() => "created" as const),
});
...
describe("banking router — matrix migration", () => {
  it("allows a role WITH the matrix grant for the requested feature+action", async () => {
    ...
    await expect(caller.bankingView()).resolves.toBe("ok");
  });
  ...
  it("denies a role WITHOUT the matrix grant (deny-by-default)", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.bankingView()).rejects.toThrow(TRPCError);
    await expect(caller.bankingView()).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("mutation endpoint composed via writeProcedure.use(matrixMiddleware) allows a granted role", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.bankingCreate()).resolves.toBe("created");
  });
