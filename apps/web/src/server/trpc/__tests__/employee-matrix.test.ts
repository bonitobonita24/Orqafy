...
// Mini router mirroring employee.ts's "employees" feature wiring: a view
// (matrixProcedure) and a create (writeProcedure + matrixMiddleware) endpoint.
const testRouter = createTRPCRouter({
  employeesView: matrixProcedure("employees", "view").query(() => "ok" as const),
  employeesCreate: writeProcedure.use(matrixMiddleware("employees", "create")).mutation(() => "created" as const),
});
...
describe("employee router — matrix migration", () => {
  it("allows a role WITH the matrix grant for the requested feature+action", async () => {
    ...
    await expect(caller.employeesView()).resolves.toBe("ok");
  });
  ...
  it("denies a role WITHOUT the matrix grant (deny-by-default)", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.employeesView()).rejects.toThrow(TRPCError);
    await expect(caller.employeesView()).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("mutation endpoint composed via writeProcedure.use(matrixMiddleware) allows a granted role", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.employeesCreate()).resolves.toBe("created");
  });
