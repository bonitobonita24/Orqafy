...
// Mini router mirroring inventory.ts's "inventory" feature wiring: a view
// (matrixProcedure) and a create (writeProcedure + matrixMiddleware) endpoint.
const testRouter = createTRPCRouter({
  inventoryView: matrixProcedure("inventory", "view").query(() => "ok" as const),
  inventoryCreate: writeProcedure.use(matrixMiddleware("inventory", "create")).mutation(() => "created" as const),
});
...
describe("inventory router — matrix migration", () => {
  it("allows a role WITH the matrix grant for the requested feature+action", async () => {
    ...
    await expect(caller.inventoryView()).resolves.toBe("ok");
  });
  ...
  it("denies a role WITHOUT the matrix grant (deny-by-default)", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.inventoryView()).rejects.toThrow(TRPCError);
    await expect(caller.inventoryView()).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("mutation endpoint composed via writeProcedure.use(matrixMiddleware) allows a granted role", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.inventoryCreate()).resolves.toBe("created");
  });
