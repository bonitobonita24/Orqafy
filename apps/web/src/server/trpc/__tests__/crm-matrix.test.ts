...
// Mini router mirroring crm.ts's "crm" feature wiring: a view (matrixProcedure)
// and a create (writeProcedure + matrixMiddleware) endpoint.
const testRouter = createTRPCRouter({
  crmView: matrixProcedure("crm", "view").query(() => "ok" as const),
  crmCreate: writeProcedure.use(matrixMiddleware("crm", "create")).mutation(() => "created" as const),
});
...
describe("crm router — matrix migration", () => {
  it("allows a role WITH the matrix grant for the requested feature+action", async () => {
    ...
    await expect(caller.crmView()).resolves.toBe("ok");
  });
  ...
  it("denies a role WITHOUT the matrix grant (deny-by-default)", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.crmView()).rejects.toThrow(TRPCError);
    await expect(caller.crmView()).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("mutation endpoint composed via writeProcedure.use(matrixMiddleware) allows a granted role", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.crmCreate()).resolves.toBe("created");
  });
