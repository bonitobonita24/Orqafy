...
// Mini router mirroring support.ts's "support" feature wiring: a view
// (matrixProcedure) and a create (writeProcedure + matrixMiddleware) endpoint.
const testRouter = createTRPCRouter({
  supportView: matrixProcedure("support", "view").query(() => "ok" as const),
  supportCreate: writeProcedure.use(matrixMiddleware("support", "create")).mutation(() => "created" as const),
});
...
describe("support router — matrix migration", () => {
  it("allows a role WITH the matrix grant for the requested feature+action", async () => {
    ...
    await expect(caller.supportView()).resolves.toBe("ok");
  });
  ...
  it("denies a role WITHOUT the matrix grant (deny-by-default)", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.supportView()).rejects.toThrow(TRPCError);
    await expect(caller.supportView()).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("mutation endpoint composed via writeProcedure.use(matrixMiddleware) allows a granted role", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.supportCreate()).resolves.toBe("created");
  });
