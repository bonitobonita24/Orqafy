...
// Mini router mirroring invoice.ts's "invoices" feature wiring: a view
...
const testRouter = createTRPCRouter({
  ...
  invoicesCreate: writeProcedure.use(matrixMiddleware("invoices", "create")).mutation(() => "created" as const),
});
...
describe("invoice router — matrix migration", () => {
  it("allows a role WITH the matrix grant for the requested feature+action", async () => {
    ...
    await expect(caller.invoicesView()).resolves.toBe("ok");
  });
  ...
  it("denies a role WITHOUT the matrix grant (deny-by-default)", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.invoicesView()).rejects.toThrow(TRPCError);
    await expect(caller.invoicesView()).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("mutation endpoint composed via writeProcedure.use(matrixMiddleware) allows a granted role", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.invoicesCreate()).resolves.toBe("created");
  });
