...
// Mini router mirroring expense.ts's "expenses" feature wiring: a view
// (matrixProcedure) and a create (writeProcedure + matrixMiddleware) endpoint.
const testRouter = createTRPCRouter({
  expensesView: matrixProcedure("expenses", "view").query(() => "ok" as const),
  expensesCreate: writeProcedure.use(matrixMiddleware("expenses", "create")).mutation(() => "created" as const),
});
...
describe("expense router — matrix migration", () => {
  it("allows a role WITH the matrix grant for the requested feature+action", async () => {
    ...
    await expect(caller.expensesView()).resolves.toBe("ok");
  });
  ...
  it("denies a role WITHOUT the matrix grant (deny-by-default)", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.expensesView()).rejects.toThrow(TRPCError);
    await expect(caller.expensesView()).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("mutation endpoint composed via writeProcedure.use(matrixMiddleware) allows a granted role", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.expensesCreate()).resolves.toBe("created");
  });
