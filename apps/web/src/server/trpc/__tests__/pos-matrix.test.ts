...
// Mini router mirroring pos.ts's "pos" feature wiring: a view
// (matrixProcedure) and a create (writeProcedure + matrixMiddleware) endpoint.
const testRouter = createTRPCRouter({
  posView: matrixProcedure("pos", "view").query(() => "ok" as const),
  posCreate: writeProcedure.use(matrixMiddleware("pos", "create")).mutation(() => "created" as const),
});
...
describe("pos router — matrix migration", () => {
  it("allows a role WITH the matrix grant for the requested feature+action", async () => {
    ...
    await expect(caller.posView()).resolves.toBe("ok");
  });
  ...
  it("denies a role WITHOUT the matrix grant (deny-by-default)", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.posView()).rejects.toThrow(TRPCError);
    await expect(caller.posView()).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("mutation endpoint composed via writeProcedure.use(matrixMiddleware) allows a granted role", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.posCreate()).resolves.toBe("created");
  });
