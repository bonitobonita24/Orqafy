...
// Mini router mirroring dtr.ts's "dtr" feature wiring: a view (matrixProcedure)
// and a create (writeProcedure + matrixMiddleware) endpoint.
const testRouter = createTRPCRouter({
  dtrView: matrixProcedure("dtr", "view").query(() => "ok" as const),
  dtrCreate: writeProcedure.use(matrixMiddleware("dtr", "create")).mutation(() => "created" as const),
});
...
describe("dtr router — matrix migration", () => {
  it("allows a role WITH the matrix grant for the requested feature+action", async () => {
    ...
    await expect(caller.dtrView()).resolves.toBe("ok");
  });
  ...
  it("denies a role WITHOUT the matrix grant (deny-by-default)", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.dtrView()).rejects.toThrow(TRPCError);
    await expect(caller.dtrView()).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("mutation endpoint composed via writeProcedure.use(matrixMiddleware) allows a granted role", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.dtrCreate()).resolves.toBe("created");
  });
