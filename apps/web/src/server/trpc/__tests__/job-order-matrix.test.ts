...
// Mini router mirroring job-order.ts's "job_orders" feature wiring: a view
// (matrixProcedure) and a create (writeProcedure + matrixMiddleware) endpoint.
const testRouter = createTRPCRouter({
  jobOrdersView: matrixProcedure("job_orders", "view").query(() => "ok" as const),
  jobOrdersCreate: writeProcedure.use(matrixMiddleware("job_orders", "create")).mutation(() => "created" as const),
});
...
describe("job-order router — matrix migration", () => {
  it("allows a role WITH the matrix grant for the requested feature+action", async () => {
    ...
    await expect(caller.jobOrdersView()).resolves.toBe("ok");
  });
  ...
  it("denies a role WITHOUT the matrix grant (deny-by-default)", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.jobOrdersView()).rejects.toThrow(TRPCError);
    await expect(caller.jobOrdersView()).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("mutation endpoint composed via writeProcedure.use(matrixMiddleware) allows a granted role", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.jobOrdersCreate()).resolves.toBe("created");
  });
