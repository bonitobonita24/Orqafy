...
// Mini router mirroring tasks.ts's "tasks" feature wiring: a view
// (matrixProcedure) and a create (writeProcedure + matrixMiddleware) endpoint.
const testRouter = createTRPCRouter({
  tasksView: matrixProcedure("tasks", "view").query(() => "ok" as const),
  tasksCreate: writeProcedure.use(matrixMiddleware("tasks", "create")).mutation(() => "created" as const),
});
...
describe("tasks router — matrix migration", () => {
  it("allows a role WITH the matrix grant for the requested feature+action", async () => {
    ...
    await expect(caller.tasksView()).resolves.toBe("ok");
  });
  ...
  it("denies a role WITHOUT the matrix grant (deny-by-default)", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.tasksView()).rejects.toThrow(TRPCError);
    await expect(caller.tasksView()).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("mutation endpoint composed via writeProcedure.use(matrixMiddleware) allows a granted role", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.tasksCreate()).resolves.toBe("created");
  });
