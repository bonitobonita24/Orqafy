...
// Mini router mirroring project.ts's "projects" feature wiring: a view
// (matrixProcedure) and a create (writeProcedure + matrixMiddleware) endpoint.
const testRouter = createTRPCRouter({
  projectsView: matrixProcedure("projects", "view").query(() => "ok" as const),
  projectsCreate: writeProcedure.use(matrixMiddleware("projects", "create")).mutation(() => "created" as const),
});
...
describe("project router — matrix migration", () => {
  it("allows a role WITH the matrix grant for the requested feature+action", async () => {
    ...
    await expect(caller.projectsView()).resolves.toBe("ok");
  });
  ...
  it("denies a role WITHOUT the matrix grant (deny-by-default)", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.projectsView()).rejects.toThrow(TRPCError);
    await expect(caller.projectsView()).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("mutation endpoint composed via writeProcedure.use(matrixMiddleware) allows a granted role", async () => {
    ...
    const caller = createCaller(ctx());
    await expect(caller.projectsCreate()).resolves.toBe("created");
  });
