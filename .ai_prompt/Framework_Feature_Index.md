...
| Added | V10 |
| Role | Deprecated. Claude Code handles all work Cline used to handle. .clinerules RETIRED (V32.33) — no longer generated (tombstone only), unread by any script/hook. |
| Default model | N/A — deprecated |
| Config | .clinerules RETIRED V32.33 — no longer generated (tombstone only) |
| Task files | docs/tasks/ directory created by Bootstrap but unused |

**Why deprecated:** In practice, Claude Code handles all execution reliably. Cline routing added complexity without benefit. As of V32.33, session-memory files (`lessons.md`, `STATE.md`, `handoffs/`) live under `docs/` (`docs/memory/lessons.md`, `docs/STATE.md`, `docs/handoffs/`) — the canonical home for apps deployed V32.33+; apps deployed before V32.33 keep reading `.cline/` (zero-disruption, never migrated in place).
...
- V31: ⚠ DEPRECATED — do not use. In-place update (no version bump). Claude Code handles everything. `.clinerules` RETIRED (V32.33) — no longer generated (tombstone only), unread by any script/hook.
...
| Attribution | (historical) SpecStory passive capture + Governance Sync reconciliation — Rule 19 RETIRED (V32.34); Governance Sync now reconciles from git (Scenario 12) |
...
- **V32.34: Rule 19 + Scenario 18 RETIRED — attribution now git-sourced (Scenario 12)**
...
### 1.6 SpecStory (Passive Change Capture) — ⚠ RETIRED (V32.34)
| Field | Value |
|---|---|
| Status | ⚠ RETIRED (V32.34) — do not install |
| Added | V11 |
| Role | (historical) Passively captured all agent session diffs to `.specstory/history/` |
| Config | (historical) `.specstory/config.json` (written by Bootstrap) — no longer generated |
| Rule | Rule 19 — RETIRED (V32.34), tombstone only |

**Decision (RETIRED V32.34):** SpecStory's attribution-gap role is obsolete — Claude Code is the sole in-project agent and self-attributes at commit (Rule 15). Governance Sync now sources diffs from git (`git log --since=$LAST` + `git diff HEAD` — Scenario 12) instead of `.specstory/history/`. Kept LISTED here (not removed) so the 6-agent count holds, mirroring the deprecated-Cline entry. Deployed apps keep their existing `.specstory/history/` as a frozen legacy audit trail — never migrated or deleted.

**Change history:**
- V11: Elevated from "with Copilot" to dedicated passive capture layer
- V11: Governance Sync reads `.specstory/history/` for reconciliation (Scenario 17 — RETIRED V32.34)
- V16: Bootstrap writes `v16-master-prompt.md` → bumped each version
- **V32.34: RETIRED — Governance Sync rebased onto git; entry kept listed for the 6-agent count**
...
| Status | ACTIVE |
| Added | V14 |
| Location | `docs/STATE.md` (V32.33+); `.cline/STATE.md` on apps deployed before V32.33 |
| Read order | FIRST — before 9 governance docs |
| Written by | Claude Code after every task |
...
| 8 | `docs/memory/lessons.md` | CLAUDE_CODE | Typed error learnings (Rule 18) — read FIRST (V32.33+; `.cline/memory/lessons.md` on pre-V32.33 apps) |
| 9 | `docs/memory/agent-log.md` | ALL | All agents append attribution entries (V32.33+; `.cline/memory/agent-log.md` on pre-V32.33 apps) |

**Additional:** `docs/STATE.md` — read before the 9 docs (V14+; `.cline/STATE.md` on pre-V32.33 apps)
...
| Phase | Who | When | What |
|---|---|---|---|
| 0 — Bootstrap | Claude Code AUTO | Once per project | Folders, CLAUDE.md, governance templates, docs/STATE.md |
...
### 12.4 CI/CD Pipeline Standard (Rule 36 — V32.32)
| Field | Value |
|---|---|
| Status | ACTIVE — new **Rule 36** |
| Added | V32.32 |
| Authority | `.ai_prompt/cicd.md` (new always-on deliverable **#32**) |
| Generator | `cicd-gen/` (materializes the per-app pipeline scripts/workflow — mirrors the `staging-refresh-setup` generator pattern) |
| Scenario | New **Scenario 45** — Existing-App CI/CD Pipeline Retrofit |
| Core principle | Build the deploy image **ONCE**, promote the exact same bytes forward: dev → staging → prod; demo via **retag-only** |
| CI role | Gates + builds + pushes the image; **NEVER auto-deploys** any environment |
| Staging | Deploys ONLY via the data-first refresh gate (`staging-refresh-gate.md`) — migrations rehearse against a fresh prod-data copy |
| Production | Migration is a **deliberate manual** `prisma migrate deploy` step — never auto-run on container boot; prod data is never wiped or reseeded |
| Demo | Migrate-yes / reseed-never, plus a 6-hour golden-dataset self-heal job |
| Rollback | Couples the image tag and its schema migration together — never roll back one without the other |
| Wiring | `phases.md` Phase 6 (Docker/deploy phase) now emits the CI/CD pipeline scaffold per Rule 36 |
| HARD HOLD | The standard + any app retrofit land as LOCAL commits only — no staging/prod/demo deploy without the owner's explicit word |

**Change history:**
- V32.32: Introduced — Rule 36, `.ai_prompt/cicd.md` deliverable #32, Scenario 45, `cicd-gen/` generator, Phase 6 wiring. Rules 35→36, Scenarios 44→45, Deliverables 31→32.
...
| 17 | SpecStory captured unattributed changes — RETIRED (V32.34), see Scenario 12 | V11 (retired V32.34) | Attribution |
...
| 19 | SpecStory passive capture — RETIRED (V32.34) | V11 (retired V32.34) | Governance Sync now git-sourced (Scenario 12) |
...
|---|---|---|
| V32.54 | Content Human-Voice Pass — on-demand deliverable #44 `content-voice.md` | **MINOR, additive — new on-demand deliverable #44 `.ai_prompt/content-voice.md`; deliverables 43→44, no new Rule (stays 41), no new Scenario. Makes the globally-installed `humanize` + `ai-check` skills an automatic-when-content, never-invoked-by-name pass over human-facing published prose (marketing/landing/blog/caption/in-app/notification/generated-docs copy). ON-DEMAND — pulled ONLY when a content task fires, NOT a constitutional always-on concern (deliberately contrasted with Rule 35 SEO, which every app is scaffolded with). Write-human-first (humanize generate mode) OR humanize-rewrite → `ai-check` gate before content is "done"; rides ON TOP of `copywriting`/`app-showcase`/`doc-coauthoring` (INHERIT-not-REPLACE); SKIP for non-prose (code/config/JSON/data) + rigid/legal register. Strengthens the V32.53 humanize/ai-check phase cue from a loadout hint into a documented on-demand pass; wired at `phases.md` (Phase 2.8/3.3 mockup copy, Phase 7 feature-update content, light `ai-check` report-only at Phase 5 on public copy) + `deploy.sh` (ships #44 to `.ai_prompt/`) + `AI_Tools_Reference.md` §3.9 cross-ref. Counts: 44 deliverable files (was 43). All others UNCHANGED — 41 Rules · 51 Scenarios · 62 Prompts (39 NEW ✨) · 14 UI Rules · 6 MCP servers · 18 Phase Hooks · 6 agents · 20 Bootstrap Steps. HARD HOLD — local only.** |
| V32.53 | Capability-Map Loadout Adoption + Skill-Cue Refresh — per-phase loadout now consults the global Capability Map | **MINOR, additive — no new Rule/Scenario/deliverable, no count change. Adds a new `phases.md` MODEL HOOK (after the V32.24 Spec-Expert-Panel Phase-2.8 hook) plus a `CLAUDE.md` note + `AI_Tools_Reference.md` §3.9, referencing the global always-on loadout chain (`~/.claude/rules/analyze-confirm-gate.md` HAND-1 → `skill-loadout-card.md` STEP 0.5) that rescans the live/new available-skills list and consults `~/.claude/library/capability-map.md` — a curated dev-phase × task × skills × Black Magic map. Our OWN library (installed skills/MCPs/plugins) is always the default; AI Black Magic is GATED — never auto-queried (only BM-1 curating our own skills/the map, BM-2 a real capability gap after already searching our library, or BM-3 the owner explicitly asks). Refreshes prescribed skills per phase: `design-brief` (design translator, FIRST at Phase 2.8/3.3), `humanize` + `ai-check` (docs/showcase/marketing copy — Phase 7 + app-showcase), `doubt-driven-development` (high-stakes decisions, pairs with the Phase 3 Spec Expert Panel), `video-shot-planner`/`app-showcase`/`image`/`dataviz`/`web-motion` (showcase/marketing/charts). These skills/rules live GLOBALLY in `~/.claude/` (every seat already loads them) — this change only makes the framework's own phase hooks reference the discipline; nothing new is deployed via `deploy.sh`. INHERIT-not-REPLACE over the existing per-phase skill tables. Counts: ALL UNCHANGED — 41 Rules · 51 Scenarios · 62 Prompts (39 NEW ✨) · 14 UI Rules · 6 MCP servers · 18 Phase Hooks · 6 agents · 20 Bootstrap Steps · 43 deliverable files. HARD HOLD — local only.** |
| V32.52 | Full-Audit Pre-Scope — deterministic target-type × dimension/gate pre-scope engine feeding the `full audit check` swarm | **MINOR, additive — two new deliverables: #42 `audit-scope.mjs` (→ `scripts/`, an advisory always-exit-0 pre-pass that parses a diff, classifies the TARGET TYPE (framework/app/foreign/session), and resolves per audit-worthy surface which audit DIMENSIONS apply (`full-audit-check-trigger.md` taxonomy + invisible-quality-radar D1-D10, primer-FLAG scoped) + which existing mechanical GATE script is relevant (Option A — list-only), emitting a JSON+Markdown manifest to `test-artifacts/audit/`; zero runtime deps) + #43 `audit-scope.md` (→ `.ai_prompt/`, the on-demand authority documenting the two design laws, the target-type classifier, and the dimension/gate maps). The audit-side SIBLING of V32.51's #40/#41 — same pre-scope pattern, lifted from code review to the `full audit check` swarm. Consumed by the `full audit check` Architect at step 1 (`full-audit-check-trigger.md` §5, wired globally) as a guaranteed FLOOR (never a ceiling) — never blocks an audit/commit/build. INHERIT-not-REPLACE over `full-audit-check-trigger.md` + Rule 40 (invisible-quality-radar). Counts: deliverables 41→43; Rules unchanged at 41, Scenarios unchanged at 51; all other counts UNCHANGED — 62 Prompts (39 NEW ✨) · 14 UI Rules · 18 Phase Hooks · 6 agents · 6 MCP servers · 20 Bootstrap Steps. HARD HOLD — local only.** |
| V32.51 | Review Pre-Scope — deterministic `files × rules` pre-scope engine feeding `/code-review` | **MINOR, additive — two new deliverables: #40 `review-scope.mjs` (→ `scripts/`, an advisory always-exit-0 pre-pass that parses a diff, applies a conservative review-worthiness gate, and resolves per review-worthy file which `security.md` §L1-L6 lenses + `audit-app.sh` scanner tools are relevant, emitting a JSON+Markdown manifest to `test-artifacts/review/`; zero runtime deps) + #41 `review-scope.md` (→ `.ai_prompt/`, the on-demand authority documenting the two design laws, the review-worthiness gate, and the lens/scanner maps). New Scenario 51. Consumed by the `/code-review` skill (§5) as a guaranteed FLOOR (never a ceiling) — never blocks a review/commit/build. INHERIT-not-REPLACE over `security.md` L1-L6. Counts: Scenarios 50→51, deliverables 39→41; Rules unchanged at 41; all other counts UNCHANGED — 62 Prompts (39 NEW ✨) · 14 UI Rules · 18 Phase Hooks · 6 agents · 6 MCP servers · 20 Bootstrap Steps. HARD HOLD — local only.** |
| V32.50 | Site Access & Tenancy Bootstrap Standard — `/tm` platform site + role-routed per-tenant login + data-driven platform roles | **MINOR, additive — new Rule 41 (topology, companion to Rule 34's role mechanics); `rbac.md` extended with Part E (platform-scope roles: `scope` discriminator + DISTINCT `PlatformPermissionKey`/`PlatformRolePermission` vocabulary + CHECK-constraint anti-escalation) + Part F (URL topology, `/platform`→`/tm` redirect-shim rename, reserved slugs); new Scenario 50 retrofit (mirrors Scenario 42's dev-first/HARD-HOLD posture); new `Security_Checklist.md` Section 22 (+12 items); `phases.md` gains MODEL HOOKs (Phase 4 Part 3 platform-role seed + extended RBAC hook, Phase 4 Parts 5-6 GATE-CLOSURE route-topology item, Phase 3.3 URL-topology cue) — Phase Hooks count stays 18; `templates.md` 7G seed template (tenant_billing/tenant_tech + demo now ALSO seeds tenant_admin); new `LESSONS_REGISTRY.md` entry `framework.site-access.tm-platform-roles-and-role-routed-urls`.** Adopts the owner-approved `docs/SITE_ACCESS_STANDARD.md` (FRMS, 2026-08-16) fleet-wide. The existing Rule 34 3-tier tenant backbone (`tenant_manager`/`tenant_superadmin`/`tenant_admin`) is UNCHANGED — this is a NEW layer alongside it. Demo deployments get NO `/tm` route at all (build-time absent, not merely hidden). **HARD HOLD** — local commits only; per-app adoption (FRMS reference first) is a separate, owner-gated Scenario-50 retrofit. **Counts: 41 Rules (was 40), 50 Scenarios (was 49), Security Checklist 147→159 items/21→22 sections; all other counts UNCHANGED — 62 Prompts (39 NEW ✨) · 39 deliverable files · 14 UI Rules · 18 Phase Hooks · 6 agents · 6 MCP servers · 20 Bootstrap Steps.** |
...
| V32.29 | Telegram Storage SCAFFOLD-CODE port — `templates.md`/`phases.md` now scaffold the dual-backend adapter | Lands the V32.27 "⚠ SCAFFOLD-CODE PENDING" deferral: the framework scaffold now EMITS the Telegram dual-backend storage layer (packages/storage `StorageAdapter` S3/MinIO + Telegram, `MediaObject` ledger model + additive migration, `/api/media` proxy + `resolveMediaBytes` dual-read, upload-router Telegram branch, per-env `STORAGE_BACKEND`/`TELEGRAM_*` wiring — dev+staging+prod=Telegram (dev on its own channel), demo=MinIO, MinIO retained fallback) instead of MinIO-only. Prose-scaffold (agent-generated at Phase 4 Parts 3/4/5), NOT a copied-file deliverable. Host primitives named not fabricated. **Zero count change.** |
| V32.28 | Event Delivery & Notifications — conditional on-demand deliverable `.ai_prompt/notifications.md` #30 + Scenario 43 | Right-sized, FOSS multi-channel event-delivery / notification pattern WITHOUT Kafka's tax. Tier 1 DEFAULT = Valkey Streams + BullMQ (locked stack, zero new infra); Tier 2 graduation (opt-in) = NATS JetStream (Apache-2.0). 6 mandatory additions: schema+versioning, tenant isolation, notification preferences, idempotency-at-ingestion, per-provider rate limits, PII routing. Channels: email/push(Expo)/SMS/webhook(HMAC)/in-app/Telegram. Loaded ONLY when PRODUCT.md declares a notification need — conditional, default stack unchanged. Scenario 43 = the dev-first build. **Count: deliverables 29→30, Scenarios 42→43.** |
| V32.46 | §GEO Generative Engine Optimization — on-demand content/authoring extension of Rule 35 (`seo.md` §8) | **MINOR, additive — extends Rule 35 / `.ai_prompt/seo.md` in place with a new §8 subsection, NO new Rule/Scenario/Prompt/deliverable, NO count change.** Captures the ONE evidence-based cherry-pick from the 2026-08 SEO/GEO harvest (AgriciDaniel/claude-seo, MIT). Grounded in Google's own stated position that AEO/GEO are **rebranded SEO** (same ranking systems; indexation is the eligibility floor Rule 35 §§0–7 already delivers) — so §GEO is an **ON-DEMAND extension, not a new always-on Rule.** Read ONLY on a public-facing/marketing/content-heavy app, at the design/content phases (with §1.5) + the Phase-5 SEO step. Net-new levers: passage citability (self-contained ~130–170-word answer blocks, front-loaded), question-based headings, attribution density (sourced claims + stats + recency), entity presence/consistency, and structured-data-for-AI (reuses §4 JSON-LD, no new mechanism). Includes an explicit **myth-list** debunking the influencer hype so no effort is wasted: `llms.txt` is NOT a Google ranking/citation lever (Google Search ignores it), no magic "content-chunking for AI," no separate "AI-keyword" rewriting. Adds NO new Phase-5 gate (Rule 35 §6 unchanged) — advisory content checklist + Scenario 44 retrofit pass. **HARD HOLD** — local commits only. **Count: UNCHANGED — 39 Rules · 49 Scenarios · 62 Prompts · 39 deliverables · 14 UI Rules.** |
| V32.47 | Invisible-Quality Radar — framework mirror of the global radar (new Rule 40) | **MINOR, additive — 1 new constitutional Rule 40, NO new Scenario/Prompt/deliverable (rides existing files).** STAGED Spec-Driven mirror of the global `~/.claude/library/invisible-quality-radar.md`. Keeps the deep-code, non-functional quality dimensions in active consideration as an app is built — **D1 security · D2 SEO · D3 a11y · D4 privacy · D5 audit-ability · D6 perf-budget · D7 observability · D8 ISO-conformance · D9 reliability · D10 maintainability**. **Rule 40** is an ALWAYS-ON, weight-gated **INDEX/POINTER** (trivial tasks skip): at the analyze-confirm task-start loadout of any REAL feature/build task, ask "which IN-SCOPE invisible dims does this task touch, and is each one's owner surface loaded?" — **NOT a new enforcer, NOT a new blocking gate**, INHERIT-not-REPLACE over the existing owner rules (`security.md` L1-L6 · `seo.md` Rule 35 +§GEO · `ui-rules.md` R13 WCAG · `privacy.md` Rule 33 · `audit.md` Rule 38 · L5 AuditLog). Which dims are in-scope for an app is read from a NEW capability-primer FLAGS field **`INVISIBLE_DIMS_IN_SCOPE:`** (`build-primer.sh` #38; documented in `~/.claude/templates/capability-primer.md`). Framework teeth are advisory only: a **Phase-7 done-claim CONSIDERATION line** + a new **`spec-gap-check.sh` CLASS 6** (a declared-in-scope dim with no evidence built — e.g. public app/no sitemap; data-sensitive/no AuditLog). **D6/D7/D8 = NAMED-but-unowned** (radar keeps them visible; no new authority file — avoid over-engineering). **ISO/IEC 25010** adopted as the vocabulary spine — **conformance-AWARE / certification-READY, NO ISMS** (escalate to an ISO 27001 track ONLY per-client on a real contractual trigger, as a `[WHAT]`). Per-app adoption is a per-seat broadcast (deferred, owner-gated). **Zero-disruption** — nothing new auto-blocks a build. **HARD HOLD** — local commits only. **Count: Rules 39→40 (40 Rules); Scenarios 49, Prompts 62, deliverables 39, UI Rules 14 UNCHANGED.** |
| V32.48 | lint-design advisory expansion — 11 new checks P1k–P1u (content/microcopy + static AI-tells) | **MINOR, additive — extends `lint-design.sh` #26 with 11 new advisory checks P1k–P1u; no new Rule/Scenario/Prompt/deliverable, NO count change (advisory checks are not counted). Per the ratified bump-type policy an additive BEHAVIORAL change earns MINOR even with no count change (`versioning-standard.md`); matches the V32.45 precedent (1 lint check → MINOR headline).** Back-fills the framework-history record for the expansion shipped in repo release v32.47.1 (`4037da6`). The checks cover content/microcopy tells + static AI-slop tells, harvested from taste-skill §9.F + the impeccable rule registry; `CLAUDE.md` #26 advisory list P1a-P1j → P1a-P1u; `design-principles.md` Pillar 8 extended. Advisory (`--report-only`, exit 0) — nothing new auto-blocks a build. **HARD HOLD** — local commits only. **Count: UNCHANGED — 40 Rules · 49 Scenarios · 62 Prompts · 39 deliverables · 14 UI Rules.** |
| V32.48.1 | AdminCN starter-slice DEDUP — `deploy.sh` gitignores the vendored `starter/admincn/` slice | **PATCH, behavioral bugfix — deploy-wiring fix to deliverable #39, no new artifact, NO count change (matches the V32.33.1 behavioral-bugfix-as-PATCH precedent).** Root cause (verified live on FerryBook): the vendored slice ships a full 50-component `components/ui/` set and `deploy.sh` did NOT gitignore it, so an already-shadcn app that synced the framework committed a DUPLICATE copy of ~37 stock shadcn components (37/50 collided in FerryBook). Fix: `deploy.sh` `GITIGNORE_ENTRIES` now adds `starter/admincn/` → the slice becomes a local cherry-pick REFERENCE (still re-copied each sync, never committed), matching Scenario 49's "add the AdminCN extras, never duplicate a component the app already has" + the fact that the slice is non-compiling reference material. Apps that already committed the slice run `git rm -r --cached starter/admincn` on their OWN seat (per-seat, never from AIEF). `admincn-starter.md` #39 documents the gitignored-reference model + the cleanup. **HARD HOLD** — local commits only. **Count: UNCHANGED — 40 Rules · 49 Scenarios · 62 Prompts · 39 deliverables · 14 UI Rules.** |
| V32.45 | Button-Affordance Standard — buttons must read as buttons (subtle emboss, colour follows theme) | **MINOR, additive — extends ui-rules Rule 3 in place + new lint-design P1j check, NO new Rule/Scenario/Prompt/deliverable, NO count change.** Fleet-wide UI standard: every primary/secondary/CTA button carries a subtle `shadow-xs/sm` emboss (or outline border); flat/shadowless = ghost/link TERTIARY only. The emboss adds ELEVATION only — a button's colours ALWAYS follow the current theme accent; the shadow never changes/tints/introduces a colour. Extends `ui-rules.md` Rule 3 + `design-principles.md` (interactive-state contract + DO/DO-NOT) + `lint-design.sh` P1j advisory (tested pos+neg, shellcheck-clean); `CLAUDE.md` #26 advisory list P1a-P1i → P1a-P1j. Advisory (exit 0) — nothing new auto-blocks a build. **HARD HOLD** — local commits only. **Count: UNCHANGED — 39 Rules · 49 Scenarios · 62 Prompts · 39 deliverables · 14 UI Rules.** |
| V32.44 | Existing-App AdminCN Design Adoption Scenario — retrofit path for the existing fleet | **MINOR, additive — new Scenario 49, NO new Rule/Prompt/deliverable.** Companion to V32.43: V32.43 made AdminCN the default for NEW apps + documented the `fake-db`→tRPC graft, but had no first-class NAMED flow for adopting it onto EXISTING apps (owner-flagged 2026-08-07). Adds **Scenario 49 — Existing-App AdminCN Design Adoption / Retrofit**, consistent with the other retrofit scenarios (42 RBAC · 44 SEO · 45 CI/CD · 48 Audit): UI/design-layer ONLY under INHERIT-not-REPLACE (keep tRPC/Prisma/Auth.js; re-wire adopted views off `fake-db`/`zustand`/`nuqs` via the #39 graft), incremental strangler-style app-shell adoption, theme preset reconciled INTO `docs/tokens.json` (Rule 12), per-view graft with 5-state + RBAC (Rule 34), Rule-31 baseline update, verify-all-pages regression gate. Runs PER-SEAT on the target app (global-feature-broadcast — never from the AIEF seat); the AIEF seat authors the standard + a rollout tracker (`docs/planning/V32.44_ADMINCN_FLEET_ROLLOUT_TRACKER.md`). **HARD HOLD** — local commits only. **Count: Scenarios 48→49; Rules 39, Prompts 62, deliverables 39 UNCHANGED.** |
| V32.43 | AdminCN Fleet-Default Design Starter — shadcn/studio Pro admin template as the default UI/UX baseline | **MINOR, additive — new deliverable #39 `admincn-starter.md` + vendored slice `specdrivenprompt/starter/admincn/`, NO new Rule/Scenario/Prompt.** Makes **AdminCN — Shadcn Admin Dashboard Template** (shadcn/studio Pro, v1.0.0) the fleet-DEFAULT UI/UX design starter, WITHOUT changing the locked backend stack — the *template* expression of the "shadcn/studio Pro = priority design asset" rule (V32.11 Pro generator = the *generation* expression). Adoption is **UI/design-layer ONLY under INHERIT-not-REPLACE**: adopt the `default-layout` left-sidebar app-shell (Rule 8), the theme-preset system, the 50 shadcn/ui components (incl. Pro extras), and the dashboard/RBAC/settings/auth view scaffolds; KEEP tRPC + Prisma + Auth.js v5 and re-wire every adopted view off AdminCN's `fake-db`/`zustand`/`nuqs` data layer via the documented **`fake-db`→tRPC graft procedure**. New deliverable **#39 `.ai_prompt/admincn-starter.md`** is the design-baseline authority (curated-slice manifest, layout menu — `default-layout`=default + 5 opt-in, theme-preset catalogue, graft procedure, license/provenance, INHERIT-not-REPLACE — `docs/DESIGN.md`/compiled tokens win token VALUES). A **curated slice** (~222 files: theme + shell + 50 ui + data-decoupled view scaffolds + `PROVENANCE.md`) is vendored to `specdrivenprompt/starter/admincn/`, shipped by `deploy.sh`, and seeded at **bootstrap Step 20d** (nested — bootstrap total stays 20). Integrated at the design phases (`phases.md` V32.43 MODEL HOOK; `Planning_Assistant.md` Step 7; `ui-rules.md` Rule 8; `templates.md` globals.css bridge) + the global `skill-loadout-card.md` design pipeline. **License:** paid template, no bundled EULA (underlying shadcn MIT) — use-in-own/client-projects ONLY, NO redistribution; AIEF repo stays PRIVATE. Per-app rollout to existing apps DEFERRED (per-seat broadcast). **HARD HOLD** — local commits only. **Count: deliverables 38→39; Rules 39, Scenarios 48, Prompts 62/39 UNCHANGED.** |
| V32.42 | CI/CD Komodo Stack Registration Audit — every env stack (prod first) must be Komodo-tracked | **MINOR, additive — extends Rule 36, new generator artifact `deploy/komodo-verify.sh`/`cicd-gen --audit`, NO new Rule/Scenario/Prompt/deliverable, NO count change.** Closes a verified gap (2026-08-06, Marine-Guardian prod): the generated pipeline assumed every env stack already existed at `/etc/komodo/stacks/<slug>` and never verified it was a Komodo-**tracked** resource — a prod stack installed directly on-server is invisible to Komodo, un-promotable, silently drifting. `cicd.md` §8 reconciled to match the real SSH+`docker compose -p` deploy mechanism (was incorrectly claiming a "DeployStack API" call) + new §8.1 **Stack Registration Audit**: `km list stacks` / API `read("ListStacks",{})` confirms each env stack (**PRODUCTION first & loudest**) is tracked; an untracked stack is scripted-registered via a **ResourceSync** `[[stack]]` TOML stanza (manual Scenario-32 Part C UI kept as fallback, new Part **C1a**). **Fail-open on tooling, fail-closed/loud only on a confirmed untracked-prod finding.** Production stays **MANUAL-trigger** — the audit never auto-deploys or auto-registers without a human applying the printed stanza. Wired as an advisory `--report-only` Phase-6 backstop (`phases.md`, same posture as `dev-freshness-check.sh` #37 / `lint-deploy.sh`) + a new Scenario 45 step + the Scenario 32 Part C1a addition. `cicd-gen/` gains a `komodo-verify.sh.template` + `--audit` generator mode. **HARD HOLD** — local commits only. **Count: UNCHANGED — 39 Rules · 48 Scenarios · 62 Prompts · 38 deliverables.** |
| V32.41 | Capability-Primer Continuous Self-Refresh — primer stays current as the app grows | **MINOR, additive — extends deliverable #28, NO new Rule/Scenario/Prompt/deliverable, NO count change.** Closes the staleness gap in V32.40: the primer is a derived artifact, so as an app grows (new modules/integrations) it goes stale until regenerated. `spec-gap-check.sh` (#28) gains **DESYNC CLASS 5 — Capability-Primer staleness**: mechanically flags **PRIMER MISSING** (spec'd app, no `docs/primer.yml`), **PRIMER INCOMPLETE** (unfilled `?` fields), and **PRIMER DRIFT** (a known-integration keyword in `docs/PRODUCT.md` absent from `primer.yml` INTEGRATIONS). Already-wired at the Phase 7 Feature-Update pre-flight MODEL HOOK, so growth is caught automatically. `phases.md` Phase 7 hook gains the **self-refresh response**: on a primer finding, (a) refresh `docs/primer.yml` + rerun `build-primer.sh`, and (b) fire exactly ONE targeted `search-skill <integration>` per drift item — **never a full `scan-project`** (token-cost discipline). Advisory/graceful-degrade (no PRODUCT.md → skipped; no primer.yml → one nudge); fires ONLY at a Feature Update — ordinary daily bugfix/tweak tasks never trigger it. shellcheck-clean + functional-tested (5a/5b/5c + graceful-skip). **HARD HOLD** — local commits only. **Count: UNCHANGED — 39 Rules · 48 Scenarios · 62 Prompts · 38 deliverables.** |
| V32.40 | Capability-Primer Regenerator — project-aware skill/plugin loadout | **MINOR, additive — new deliverable #38, NO new Rule/Scenario/Prompt.** New `scripts/build-primer.sh` (deliverable #38), a sibling of `sync-context.sh` (#27): it regenerates the marker-delimited `<!-- AIEF:PRIMER -->` region of `CLAUDE.md` — the always-on, loadout-deciding **FLAGS slice** (stack/realtime · tenancy+RBAC · gov-LGU · public-facing · mobile · data-sensitivity · integrations) — from `docs/primer.yml`, a flat `KEY: value` spec the agent authors once from `docs/PRODUCT.md`. The FLAGS slice makes the skill/plugin loadout **project-aware** instead of task-shape-generic; it is consumed by `~/.claude/rules/skill-loadout-card.md` STEP 0 + `analyze-confirm-gate.md` HAND-2 (both global config). Rendered from a structured source (NOT grepped from prose) so loadout-deciding semantics like `RBAC=custom-roles-not-3-tier` / `mobile=responsive-web-not-Expo` are the agent's judgement, never a fragile guess; unknown fields stay `?`. Scaffolds `docs/primer.yml` + the on-demand DETAIL slice `docs/CAPABILITY_PRIMER.md` on first run; idempotent pure-function render (no timestamps), atomic write, `--check` (exit 3 on drift) + `--file` (path-hardened) — same machinery as `sync-context.sh`. `deploy.sh` GROUP 15 ships it to `scripts/` + protects `docs/primer.yml`/`docs/CAPABILITY_PRIMER.md` (NEVER-TOUCH). Format spec: `~/.claude/templates/capability-primer.md`; proven on CueLane (`docs/planning/CAPABILITY_PRIMER_PROTOTYPE.md`). Per-app rollout is a per-seat broadcast. **HARD HOLD** — local commits only. **Count: deliverables 37→38; Rules 39, Scenarios 48, Prompts 62/39 UNCHANGED.** |
| V32.39.1 | Audit-Gate Wiring — wires the Rule 38 Tier-2 Phase-5 opt-in gate | **PATCH, wiring-only — no new Rule/Scenario/Prompt/deliverable, NO count change.** Activates the V32.38 **Rule 38** Tier-2 Phase-5 blocking gate that was documented but had no `phases.md` conditional. Adds the `audit: { tier2: { enforce: false } }` stanza to the example inputs.yml (`phases.md` Phase 3 §5e + `Master_Prompt.md` Phase 3 block, default OFF) and a conditional **□ AUDIT TOOLKIT T2 GATE** block to `phases.md` Phase 5 pre-flight: when `audit.tier2.enforce: true`, Phase 5 runs `bash scripts/audit-app.sh --tier=2` as a HARD gate (a CRITICAL/HIGH finding → `exit 1` → blocks Phase 6); default OFF → the gate is skipped and Tier-2 stays manual/on-demand. `audit.md` §1 + banner + tier table reconcile the T2 gate to **WIRED** and mark the **Gitleaks pre-commit hook DEFERRED / documented-only (NOT wired)**; `Master_Prompt.md` Rule 38 T2 bullet + Sync Impact Report + changelog updated; root `CLAUDE.md` #35/#36 prose reconciled. **NON-DISRUPTIVE** — the gate is default OFF, so syncing V32.39.1 never lands a blocking gate on any app without an explicit `inputs.yml` opt-in. **HARD HOLD** — local commits only, no push/merge/deploy. **Count: UNCHANGED — Rules 39, Scenarios 48, Prompts 62/39, deliverables 37.** |
| V32.39 | Dev-Freshness Deliverable — Dev Leads Every Environment | New constitutional **Rule 39 — Dev-Freshness (Dev Leads Every Environment)** + new deployed deliverable **`scripts/dev-freshness-check.sh` #37**, promoting the always-on global invariant `~/.claude/rules/deploy-discipline.md` ("Dev leads every env", hardened 2026-07-28 after the Marine-Guardian dev-behind-staging incident where the app rebuilt fresh but the worker stayed stale) into a first-class framework Rule + deliverable. **Local dev must never serve staler code than any environment you ship** — a staging/prod/demo deploy is NOT complete until the local dev container is rebuilt off the same `main`/sha, app **AND** worker (an app-only `--build` leaves the worker stale). Root cause it prevents: a dev stack serves a **prebuilt image with NO source bind-mount**, so code only appears on REBUILD; shipping to another env recreates THAT env but never touches dev ⇒ dev silently serves stale code while `main` is already current. Detection backstop `bash scripts/dev-freshness-check.sh [--report-only] [TARGET_DIR]` checks only CODE services (`app`/`worker`/`web`/`api`; infra skipped), prefers a stamped git-sha image label, falls back to image-build/container-recreate time vs the `main` commit, and **exits 2 if any dev code container is behind `main`** (flags `--branch`/`--project`/`--service`/`--all-services`/`--container`). **Fail-open on tooling, fail-closed on drift** (same graceful-degrade posture as `lint-deploy.sh`). Wired as an **advisory `--report-only` backstop** at `phases.md` Phase 6 (run after any ship + at session/loop start), never a hard Phase-6 blocker. Copied verbatim from the global fleet helper (logic unchanged), ships via `deploy.sh` Group 14 (chmod +x). **Zero-disruption** — nothing new auto-blocks a build. **HARD HOLD** — local commits only, no push/merge/deploy. **Count: Rules 38→39, deliverables 36→37; Scenarios unchanged at 48.** |
| V32.38 | App Audit Toolkit — tiered, Compose-native, MANUAL/ON-DEMAND by default | New constitutional **Rule 38 — App Audit Toolkit**: a tiered audit toolkit run **MANUAL / ON-DEMAND by default (owner-set 2026-07-28 — nothing auto-runs)**, one-shot `audit-app.sh <target>` (trigger "audit this app"). Tiers are COST bands, not an auto-schedule — **T1** cheap (Gitleaks · Semgrep · tsc[monorepo-aware] · ESLint), **T2** moderate (Trivy image+config · OSV-Scanner · Syft SBOM · BackstopJS, alongside the already-wired Playwright/Lighthouse/axe-Pa11y/k6/pnpm-audit), **T3** expensive & never wired (ZAP DAST · Stryker mutation · manual pentest, documented only). Blocking gates (pre-commit for Gitleaks; T2 Phase-5 via `inputs.yml`) are explicit per-app OPT-INS, off by default. New on-demand deliverable **`.ai_prompt/audit.md` #35** (tool matrix, tier table, per-tool invocation, triage/severity policy, T3 runbooks) + new deliverable **`scripts/audit-app.sh` #36** (the T1/T2 gate runner, `--tier=1|2`/`--report-only`/graceful-degrade, cloning the `lint-deploy.sh` pattern) + new **Scenario 48 — Existing-App Audit Toolkit Retrofit**. Fixes the confirmed **F2 phantom-gate defect** — `security.md`'s false "existing Trivy image gates" claim is replaced with a pointer to the now-real Rule 38 gate. INHERIT-not-REPLACE over `security.md` L1-L6 + the 147-item `Security_Checklist.md`. Fail-closed on findings (a CRITICAL finding blocks), fail-open on tooling (a missing/broken scanner warns, same graceful-degrade posture as `lint-deploy.sh` C8). Zero-disruption — T2 defaults to `--report-only` until an app opts in via `inputs.yml`. Kubescape/Polaris (K8s-native) explicitly OUT of baseline scope (Rule 6's K8s scaffold is opt-in only); Tracetest/Pact remain conditional on Rule 37 escalation. **HARD HOLD** — local commits only, no push/merge/deploy. **Count: Rules 37→38, Scenarios 47→48, deliverables 34→36.** |
| V32.37 | Design-Fidelity Completion — R2/R3/R5/R6 | **Extends Rule 31 (Design-as-Contract)** further, closing the four RCA seams deferred at V32.36's keystone-first ship (`docs/planning/DESIGN_DRIFT_RCA_AND_PLAN.md` §5). **R2 — token-derivation seam closed:** the Planning Assistant now actually emits `docs/tokens.json` (DTCG v2025.10) derived from the SAME color/spacing/radius values as `docs/DESIGN.md`/`docs/MOCKUP.jsx`, so Style Dictionary compiles the mockup's OWN palette, not an unowned translation; `templates.md` token-pipeline.spec.ts gains **CONTRACT 5 (token-value-equivalence)** asserting a compiled `--sd-color-*` value equals the corresponding `tokens.json` `$value`. **R3 — mechanical prototype→production inheritance:** `phases.md` Phase 4 Part 5 now starts from the prototype's own component/markup structure (copy-then-wire real data behind the same boundary, carrying `data-fdl` anchors) instead of re-authoring screens from PRODUCT.md/DESIGN.md prose, lowering the drift the R1 gate has to catch at the source. **R5 — non-skippable gate:** `phases.md` qualifies the Phase 3.3 skip — when `docs/MOCKUP.jsx` exists, the baseline-capture (Step 8c) + the Parts-5-6/Phase-5 `design:fidelity` gate are MANDATORY; only a genuinely mockup-less app may still skip 3.3. **R6 — baseline-update discipline (the Marine-Guardian tail):** Rule 31 + `phases.md` Phase 7 now make it explicit — an intentional design change is valid ONLY via re-approve `docs/MOCKUP.jsx` → `design:fidelity --update-baseline` → commit the new baseline; leaving the old baseline in place while design ships is a Rule-31 violation (tightens/cross-refs V32.7.3 Design Baseline Back-Port). No new Rule — Rule 31 extended in place (clause (d)). **NON-DISRUPTIVE** — no existing app is retro-gated; the gate only tightens for apps that already have a `docs/MOCKUP.jsx`. **HARD HOLD** — local only, no push/merge/deploy. **Count: UNCHANGED — Rules 37, Scenarios 47, Prompts 62/39, deliverables 34.** |
| V32.36 | Design-Fidelity Enforcement — Mockup-Anchored Layout-Fidelity Gate | **Extends Rule 31 (Design-as-Contract)** with a two-layer mockup-vs-built fidelity check, closing the design-drift gap identified in `docs/planning/DESIGN_DRIFT_RCA_AND_PLAN.md` (approved `docs/MOCKUP.jsx` vs the actually-scaffolded/shipped UI diverging silently after Phase 3.3 sign-off). New deliverable **`scripts/design-fidelity.mjs` #34** — a mockup-anchored layout-fidelity gate: both `docs/MOCKUP.jsx` and the scaffolded components carry stable `data-fdl` layout-anchor attributes as the matching key; the script diffs structural placement (not token *values*, which the design system already owns) and reports drift. **Owner-decided scope (2026-07-27):** GREENLIT · tolerance = layout-fidelity diff · **keystone-first R1 (the gate itself) + R4 (deployed, non-droppable)** only; R2 (PA→tokens.json seam), R3 (mechanical prototype-inheritance), R5 (non-skippable enforcement), R6 (baseline-update discipline) are **deferred**, tracked in the RCA doc §5 for a later iteration. INHERIT-not-REPLACE — the gate never dictates design-token values, only structural placement vs the approved mockup. NON-DISRUPTIVE — syncing V32.36 into an existing app drops an inert script + (empty) baseline scaffolding only; nothing auto-runs or auto-blocks an existing build. **HARD HOLD** — local only, no push/merge/deploy. **Count: Rules 37 (unchanged), Scenarios 47 (unchanged), Prompts 62/39 (unchanged), deliverables 33→34.** |
| V32.35 | Architecture Posture — Modular-Monolith Default · Microservices Escalation | Architecture Posture — modular-monolith **DEFAULT** + microservices **ESCALATION** standard. New **Rule 37 — Architecture Posture (Modular-Monolith Default · Microservices Escalation)** + new **Scenario 47 — Microservices Escalation Assessment / Decomposition** + new on-demand deliverable **`.ai_prompt/microservices.md` #33** (decompose-the-locked-stack reference architecture: bounded contexts from PRODUCT.md modules + RBAC registry, database-per-service, typed cross-service tRPC/zod contracts, Valkey Streams+BullMQ / NATS JetStream eventing, outbox+saga, Traefik gateway + per-service Komodo stacks, signed-JWT tenant propagation, per-service build-once tie to Rule 36; strangler migration path; anti-over-engineering "what we deliberately DON'T do"). DEFAULT stays modular monolith (monolithic deployment + modular tRPC/feature internals); microservices are an owner-gated escalation only, never the baseline. **NON-DISRUPTIVE:** syncing V32.35 into an existing app only drops the reference doc into `.ai_prompt/` — nothing is run, restructured, or auto-converted; no existing app changes architecture. INHERIT-not-REPLACE — never overrides the locked stack. **Count: Rules 36→37, Scenarios 46→47, deliverables 32→33.** |
| V32.34 | SpecStory Retirement + Governance-Sync git-rebase | **Tombstone-in-place, zero canonical count change.** Completes the multi-agent retirement arc started at V32.7 (Copilot/Cline already gone) — SpecStory + `.specstory/history/` passive capture are RETIRED; Claude Code is the sole in-project agent and self-attributes via Rule 15 + CHANGELOG_AI + Smart Checkpoint. **Governance Sync rebased onto git**: sources diffs from `git log --since=$LAST --name-status` (committed, self-attributed at commit) + `git diff HEAD`/`git status --porcelain` (uncommitted/manual-edit case SpecStory used to catch) instead of `.specstory/history/`; the COPILOT/UNKNOWN attribution rows collapse to two (`CLAUDE_CODE` committed / `HUMAN`-or-in-flight uncommitted). **Rule 19 tombstoned in place** (slot retained, no renumber) — body points to Scenario 12's git-rebase. **Scenarios 17+18 tombstoned in place** (headers kept, one-line RETIRED body — count stays 46, no renumber); Scenario 12 (Governance Sync) rewritten to the git-sourced procedure; Scenario 13 updated ("git tracks the full resolution session" instead of SpecStory). **SpecStory agent entry (§1.6) marked RETIRED but KEPT LISTED** — 6-agent count holds, mirrors the deprecated-Cline tombstone pattern. **ZERO-DISRUPTION:** `deploy.sh` removes nothing — apps already deployed keep `.specstory/history/` as a frozen legacy audit trail (never migrated/deleted); new apps are simply born without it; `.gitignore`'s `.specstory/` entry stays (additive, harmless on an app that has none). **Files changed:** `Master_Prompt.md` (Sync Impact Report + Rule 19 tombstone + Gov-Sync git block + changelog), `scenarios.md` (Scenario 12 git-rebase, 13 updated, 17+18 tombstoned), `templates.md` (Rule 19 mirror tombstone + attribution-table collapse), `bootstrap.md` (Step 11 tombstone, keeps step 20 count), `phases.md` (Gov-Sync block → git core), `CLAUDE_compact.md` (Rule 19 one-liner + agent-stack rows + version marker), `AI_Tools_Reference.md` (agent row + install checklist + tree comment + link list), `Framework_Feature_Index.md` (this row + §1.6 + §1.4 + Rules/Scenarios Index rows + footer), `Prompt_References.md`/`.html` (Gov-Sync mirror + bootstrap-desc), `Planning_Assistant.md` (Rule 19/Scenario mentions), `deploy.sh` (additive comment only — all 5 `.specstory` refs kept, `bash -n` clean). **`ChatGPT_Cross_Audit.md` FROZEN — untouched.** **Counts unchanged: 36 Rules · 46 Scenarios · 6 agents** (SpecStory retired-but-listed) · all other canonical counts unchanged. |
| V32.33 | Cline `.cline/` Split-Brain Retirement — canonical running-state → `docs/STATE.md` + Scenario 46 | Retires the `.cline/` vs `docs/` running-state split-brain left from the Cline era (Cline deprecated V31). **Canonical running-state is now `docs/STATE.md`** + `docs/memory/lessons.md` + `docs/memory/agent-log.md` + `docs/handoffs/` + `docs/tasks/`; every live Bootstrap/Rule/Scenario instruction that wrote or read `.cline/*` now targets `docs/*`. **`.clinerules` retired outright** (Bootstrap Step 3 no longer generates it — its content already lives as numbered Rules). **Correctness fix (additive, closes a real Rule-32 fail-open):** `scripts/design-stop-hook.sh` + `scripts/spec-gap-check.sh` gain an ADDITIVE `.cline/STATE.md` read-fallback (docs-first, `.cline/STATE.md` fallback, "neither present ⇒ exit 0" unchanged) so an app still on the legacy path keeps Rule-32 evidence enforcement (previously exited 0 fail-open). `deploy.sh` purely additive (every `.cline/`+`.specstory/history` protect entry kept; `docs/memory`, `docs/handoffs`, `docs/tasks` added). **ZERO-DISRUPTION** — deployed apps keep their `.cline/` untouched; only NEW apps are born on `docs/`. **New Scenario 46 — per-app `.cline/` → `docs/` migration** walkthrough. **Count change: Scenarios 45→46 ONLY** (Rules stay 36; deliverables stay 32 — touches existing deliverables, adds none). |
| V32.32 | CI/CD Pipeline Standard — new **Rule 36** + on-demand deliverable `.ai_prompt/cicd.md` #32 + Scenario 45 | Codifies the canonical **build-once, promote-forward** pipeline: CI (`ci.yml` governance→quality→security + `docker-publish.yml` build+tag+push) builds and tags an image ONCE, then the SAME bytes promote dev→staging→prod (demo by retag) — never rebuilt per environment. **CI gates and publishes; it never auto-deploys** any environment — deployment stays a separate deliberate step per the 3-tier deploy contract. **Staging rehearses on a fresh production-data copy** (data-first gate, `staging-refresh-gate.md`) before the candidate image deploys; **production migrations are a deliberate manual `prisma migrate deploy`** (rehearsed on staging, never auto-run on container boot, prod data never wiped/reseeded); **demo migrates but never reseeds**, plus a **6-hour golden self-heal** (`demo-reset.sh`). Rollback couples the image with its matching schema. Generated per app by the repo-root **`cicd-gen/`** generator (delegates staging refresh to `staging-gate/`). **HARD HOLD** — wiring/deploy stays owner-gated. **Count change: Rules 35→36, Scenarios 44→45, deliverables 31→32.** |
| V32.30 | SEO Foundation (Adaptive Baseline) — new constitutional **Rule 35** + always-on deliverable `.ai_prompt/seo.md` #31 + Scenario 44 | Every framework-built app is scaffolded with SEO out of the box — a CONSTITUTIONAL, always-on concern (same posture as the L1-L6 security stack), **NOT** a conditional PRODUCT.md-gated capability like V32.28's `notifications.md`: no new PRODUCT.md section, no human planning step. **Adaptive baseline:** public routes get full SEO (indexable, canonical, Open Graph + Twitter, JSON-LD, sitemap inclusion); private/authed/internal routes get a private baseline (`noindex,nofollow` + `robots.ts` disallow + sitemap exclusion), detected from the app's existing route-group/middleware-auth boundary, fail-closed to private on an undetermined route. Built entirely on Next.js App Router native primitives (verified via context7 against `/vercel/next.js`): `metadata`/`generateMetadata`, `app/sitemap.ts`, `app/robots.ts`, `alternates.canonical`/`alternates.languages` (hreflang), `openGraph`/`twitter` fields, opt-in dynamic `opengraph-image`/`next/og` `ImageResponse`. Config sourced from existing PRODUCT.md App Identity + env vars — never fabricated. Enforced at Phase 4 (scaffold emits sitemap/robots/metadata/JSON-LD) and Phase 5 (validation gate: metadata on every route, clean sitemap/robots build, canonical+OG+Twitter on public routes, noindex+disallow on private routes, valid structured data, hreflang when multi-locale, **Lighthouse SEO ≥ 90 HARD-gated for any app with a public-facing surface**). **Count: Rules 34→35, Scenarios 43→44, deliverables 30→31.** |
| V32.31 | SEO-Aware Design & Content — extends **Rule 35** + `.ai_prompt/seo.md` (new **§1.5**) + Scenario 44 into the design/content phases | V32.30 scaffolded SEO at build time; V32.31 pulls it **upstream** so a public/marketing surface is Google-friendly **by design**, not bolted on after the frontend exists. The Planning Assistant's DESIGN.md/MOCKUP.jsx generation step, **Phase 2.8** (mockup), and **Phase 3.3** (prototype) now plan SEO-informed content structure as part of the design pass: a single clear H1 per page, a logical H2/H3 heading hierarchy, keyword-informed headline/body copy, descriptive link text, meaningful `alt` text planned alongside the imagery, and a Core-Web-Vitals-aware layout (LCP-friendly hero, CLS-stable dimensions) — captured directly into `docs/DESIGN.md` / `docs/MOCKUP.jsx` / `docs/PROTOTYPE.md`. Automatic for public marketing/landing surfaces; a lighter touch applies to internal/authed screens (still `noindex,nofollow` per Rule 35 §1). No new PRODUCT.md section, no new interview question — this is an EXTENSION of Rule 35's adaptive baseline and Scenario 44's retrofit procedure, not a new numbered item. **Zero count change: Rules stay 35, Scenarios stay 44, deliverables stay 31.** |
| V32.26 | Sidebar-Footer Version + White-Label Standard (fleet-wide UI convention) | **Wires a fleet-wide UI convention (owner-set 2026-07-10, `~/.claude/rules/design-defaults.md` Entry 3) into the framework scaffold as a required output-contract deliverable — NOT a new numbered rule/scenario/UI-rule.** Every functional app's shadcn/ui app-shell `SidebarFooter`, placed just below the profile/user block, MUST render two small muted marks: (1) a **version tag** `v{X.Y.Z}` sourced from the release version/`package.json` per `~/.claude/rules/versioning-standard.md` (`-rc.N` suffix on staging, clean number on prod); (2) a **white-label credit** — the whole "Developed by Powerbyte IT Solutions" label is ONE clickable link opening a NEW TAB to `https://www.powerbyteitsolutions.com/` (`target="_blank" rel="noopener noreferrer"`, opener-hijack + referrer-leak guard). Non-sidebar archetypes (marketing full-width, immersive/kiosk) place the marks per the same global rule's page-footer/about-panel fallback slots. INHERIT-not-REPLACE — docs/DESIGN.md may relocate/restyle either mark. Wired as: (a) `phases.md` UI_LAYOUT task-type note + a new **Phase 4 Parts 5-6 GATE-CLOSURE checklist item** (Parts 5-6 cannot close without it) + a Notes pointer; (b) `ui-rules.md` reference note under item 8 (Rule 8, no new numbered UI rule — UI rules stay 14); (c) `Master_Prompt.md` Sync Impact Report + changelog narrative + active-version bump V32.25→V32.26; (d) `CLAUDE_compact.md` version marker + one-liner. **Zero Rule/Scenario/Prompt/deliverable count change — UI rules stay 14, all other canonical counts unchanged.** |
| V32.25 | Tenant RBAC Standard (3-Tier Backbone + Custom-Role Matrix) — Rule 34 + Scenario 42 + deliverable `.ai_prompt/rbac.md` #29 + Security_Checklist SECTION 21 | **Promotes the tenant-authorization model to a fleet-wide standard.** New **Rule 34 — Tenant RBAC Standard**: 3 fixed system tiers — `tenant_manager` (platform, `tenant_id=NULL`, break-glass, many allowed) / `tenant_superadmin` (tenant owner, EXACTLY ONE per tenant DB-enforced, sole Billing+User-Management) / `tenant_admin` (top in-tenant admin below the owner, the custom-role ceiling, never Billing/User-Management); app domain roles keep their names below `tenant_admin` and map onto Supervisor/Operator/Contributor/Viewer capability presets. New on-demand deliverable **`.ai_prompt/rbac.md` (#29)** carries the custom-role SYSTEM design: a feature registry × a `role_permissions` STRICT-CRUD matrix `(tenant_id, role_id, feature_key, view, write, update, delete)` + a `hasPermission()` resolver wired identically at tRPC + route middleware + sidebar nav + a `tenant_superadmin`-only role-builder UI. New **Scenario 42 — Existing-App RBAC 3-Tier Retrofit (MG pattern)** documents the dev-first, LOCAL-only retrofit. Prescribed mechanics: rename role enums with `ALTER TYPE … RENAME VALUE` (data-preserving, NEVER DROP/CREATE); enforce one owner per tenant with a PARTIAL unique index `unique(tenant_id) WHERE role='tenant_superadmin' AND tenant_id IS NOT NULL`; two-way succession (platform break-glass reassign + mediated owner-transfer) is mandatory + tested. Guardrails: custom roles tenant-scoped, ≤ `tenant_admin`, NEVER Billing/User-Management (exclusive to `tenant_superadmin`); only `tenant_superadmin` (+ platform `tenant_manager`) create/assign; roles always server-derived. Per-env seeded creds from the Server-Setups vault. `security.md` L3 (RBAC) block + `Security_Checklist.md` new SECTION 21 (12 items); `phases.md`/`bootstrap.md`/`templates.md` seed the backbone + cred env keys by default; `ui-rules.md` pointer (no new UI rule). Reference impl: Marine-Guardian `feat/tenant-rbac-3tier`. **Count change: Rules 33→34, Scenarios 41→42, Deliverables 28→29, Security Checklist 135→147 items / 20→21 sections.** UI rules stay 14, prompts stay 62, Bootstrap steps stay 20, Phase Hooks stay 18. |
| V32.24 | Spec Expert Panel (SC2, SuperClaude harvest) — Prompt 3.24 + Scenario 41 + Phase 2.8/3 panel gate | Harvests SuperClaude's multi-persona review pattern into a structured, gated check on the spec itself before it locks. **New Prompt 3.24 — Spec Expert Panel Review** and **Scenario 41**: at Phase 2.8 (design pre-handoff) and Phase 3 pre-lock, the PM (Opus) dispatches **5 expert-lens Sonnet subagents in parallel**, each an ephemeral reviewer loading one installed skill as its lens — `secure-code-guardian` (security/authz/data-privacy), `architecture-designer` (structure/scalability/coupling), `api-designer` (API surface/contracts/versioning), `test-master` (testability/coverage/acceptance criteria), `database-optimizer` (schema/indexing/tenancy) — each scoped to `docs/PRODUCT.md` (+ `docs/DESIGN.md`/`docs/MOCKUP.jsx` at Phase 2.8), returning prioritized (Critical/High/Medium) findings. The PM synthesizes the 5 lists, dedups overlapping findings, prioritizes, and feeds every finding into the Flow-Back/LIVING-SPEC reconcile (Rule 1 Spec-Persistence Model addendum, Scenario 40's 5-step loop). **New `phases.md` MODEL HOOKs** (NOT new §3 phase hooks — count stays 18) at Phase 2.8 and the Phase 3 Output Contract implement the gate: Phase 2.8 handoff and Phase 3 spec-lock CANNOT close while any CRITICAL finding is unresolved (`✅ spec-expert-panel: clear` / `⛔ spec-expert-panel: N Critical findings unresolved`). The Phase 2.8 gate is additive alongside the existing V32.5.1 `/design-review` gate-closure. The 5-expert roster is embedded directly in Prompt 3.24 / Scenario 41 — no new deliverable file. **Count change: Prompts 61→62 (38→39 NEW ✨), Scenarios 40→41 ONLY** — no rule/deliverable/phase-hook/MCP/security-checklist count change. |
| V32.24.1 | Content-container / responsive-gutter design default (owner-set 2026-07-10) | **PATCH — global basic-design default.** Triggered by CueLane admin pages rendering full-bleed with no horizontal gutter on wide screens (reads as unfinished). Establishes the standing default that **readable / dense page content** (admin panels, dashboards, forms, data tables, settings, reading columns) is **never full-bleed**: it sits in a centered container that does two co-equal things — (1) a **mobile-first SAFE RESPONSIVE GUTTER** that is never 0 on any breakpoint (`px-4`→`sm:px-6`→`lg:px-8`, ~16→24→32px, so content never touches the phone edge) and (2) a **MAX-WIDTH CAP** (`mx-auto max-w-7xl`, ~1280px) so wide monitors also get side gutters. Full default: `mx-auto max-w-7xl px-4 sm:px-6 lg:px-8`. **Exemption:** intentionally immersive / full-bleed surfaces stay edge-to-edge — kiosk, wall/big-display, full-bleed hero-marketing, maps/canvas, or any surface a spec calls immersive. Encoded three ways: (a) NEW global rule surface `~/.claude/rules/design-defaults.md` Entry 1 (applies to ANY UI at ANY seat, not just framework apps — structured so more design defaults append later); (b) prescriptive default in `design-principles.md` #24 Pillar 3 "Content container & responsive gutter" (INHERIT-not-REPLACE — docs/DESIGN.md measure wins); (c) a pointer in `ui-rules.md` Rule 8 layout guidance (no new numbered UI rule — lean placement, so no 14→15 count cascade). **Advisory lint DELIBERATELY NOT added:** a reliable low-false-positive static check for a full-bleed dense page root is not feasible in the grep-based `lint-design.sh` (grep cannot identify which element is the top-level page container; `max-w-*` usually lives on a different line/component than the `w-full`/`w-screen` wrapper; and legitimate full-bleed exemptions exist) — per the file's low-false-positive contract, no noisy check was shipped. **Zero count change** — no new Rule/Scenario/Prompt/deliverable/phase-hook; enriches the existing design-principles.md deliverable + ui-rules.md guidance only. |
| V32.24.2 | Layout-archetype design default (owner-set 2026-07-10) | **PATCH — global basic-design default (Entry 2 of the design-defaults surface).** Establishes the standing default that the whole page shell is chosen by **what the app IS**: (1) a **marketing / brand website or webapp = FULL-WIDTH** — full-bleed immersive layout where hero bands and sections may span the entire viewport edge-to-edge on purpose (still honoring Entry 1's container/gutter for the readable copy *inside* a full-bleed band); (2) a **general-purpose / purpose-driven app (dashboard, admin panel, tool, back-office, console) = PERSISTENT LEFT-SIDEBAR APP-SHELL** — primary navigation lives in an always-available left sidebar (collapsible via hamburger/rail, off-canvas on mobile) with the dynamic content in the large right area; do NOT scatter primary nav across a top header-tab strip for a multi-section functional app. Mobile-friendliness is intrinsic (sidebar collapses to an off-canvas sheet so content gets full width); a single-purpose focused-work console (operator/serve screen) may run the sidebar in a minimal collapsed-by-default **icon-rail** so the worker keeps maximum working area. **Framework build:** the shadcn/ui `sidebar` component (our only UI system) — `SidebarProvider` + collapsible `Sidebar` + `SidebarTrigger` (hamburger) + `SidebarInset`; inset content still uses Entry-1's container/gutter. **Exemption (immersive, no app-shell):** kiosk, wall/big-display, and public marketing/landing/auth pages. Encoded three ways: (a) global rule surface `~/.claude/rules/design-defaults.md` **Entry 2** (any UI at any seat); (b) prescriptive default in `design-principles.md` #24 Pillar 3 "Layout archetype" (INHERIT-not-REPLACE — docs/DESIGN.md / PRODUCT.md override per surface); (c) a pointer in `ui-rules.md` Rule 8 layout guidance (no new numbered UI rule — lean placement, so no 14→15 count cascade). **Zero count change** — no new Rule/Scenario/Prompt/deliverable/phase-hook; enriches the existing design-principles.md deliverable + ui-rules.md guidance only. |
| V32.23 | Constitution as Versioned Contract (H3) — Sync Impact Report + SemVer-for-rules + Constitution-Check gate | Harvests the GitHub Spec-Kit "constitution" pattern (a versioned, machine-checkable governance contract) — the Master Prompt already IS the constitution; this makes its versioning + compliance-checking machine-readable. **(a) Sync Impact Report:** a new HTML-comment header block at the very top of `Master_Prompt.md` (above "WHAT THIS FILE IS"), recording `Version:` delta, `Bump:` type, a one-line summary, and a `[x]`/`[ ]` dependent-file propagation checklist (`CLAUDE_compact.md`, `phases.md`, `Framework_Feature_Index.md`, project `CLAUDE.md`) — updated FIRST on every future version bump; it is the machine-readable propagation tracker that replaces the retired `ChatGPT_Cross_Audit.md`'s "verify everything matches" role. **(b) SemVer-for-rules:** a new subsection next to the VERSION + FILENAME POLICY block defining MAJOR (removes/redefines a Rule/Phase/L1-L6 security level — requires a migration note), MINOR (additively introduces a Rule/Scenario/Prompt/gate/phase-hook/deliverable — backward-compatible), and PATCH (wording/count/typo fix — non-behavioral); the Sync Impact Report's `Bump:` field always cites one of these three. **(c) Constitution-Check gate:** a new step 0b in `phases.md`'s MANDATORY PRE-FLIGHT block (alongside the V32.8 registry work-start consult) — before executing any planned task, confirm it does not violate any of the 33 Rules or the L1-L6 security model (esp. Rule 1, L3 RBAC, L5 AuditLog, L6 Prisma guardrails); a conflict is resolved as **[HOW]** if technical or escalated as **[WHAT]** (recorded in `PENDING_DECISIONS.md`) if it's a product/scope tradeoff — this is the app-BUILD companion to the Master Prompt's own Sync Impact Report: the SIR verifies FRAMEWORK edits propagate, the Constitution-Check verifies APP-BUILD tasks comply. **Count change: NONE** — governance-tooling addition only, no rule/scenario/prompt/deliverable/phase-hook/MCP/security-checklist count change. `ChatGPT_Cross_Audit.md` stays RETIRED — the Sync Impact Report is its machine-readable successor for propagation tracking. |
| V32.22 | Attack-Informed Security Hardening — Security Checklist 114→135 items / 16→20 sections | Harvests offense-side findings from the `Claude-Red` red-team methodology into defensive controls, so every framework-built app gets the hardening without loading any red-team skill. **`security.md` gains 10 new hardening blocks:** JWT algorithm-pinning (reject `alg: none`, pin `HS256`/`RS256`), OAuth/SSO safety (state-parameter CSRF binding, PKCE, redirect_uri allow-list), open-redirect prevention (allow-list/same-origin validation, never raw user-supplied redirect target), RCE/command-injection prohibition (no `eval`/`exec`/`child_process.exec` on untrusted input), race/TOCTOU generalization (the L6 Prisma pattern extended to file ops + cache-then-write + balance/inventory checks), prototype-pollution guard (`__proto__`/`constructor`/`prototype` key validation on user-controlled object merges), cloud-credential safety (never log access keys, short-lived STS over static keys, scoped IAM, per-server secrets stay in Server-Setups), mobile-app safety (conditional on PRODUCT.md native-mobile flag — no hardcoded client-bundle API keys, cert pinning, secure token storage), request-smuggling note (proxy header normalization), and clickjacking (mandatory `X-Frame-Options`/`frame-ancestors` CSP). **`Security_Checklist.md` gains SECTION 17 — Auth-Token/OAuth Security, SECTION 18 — Command/Code-Execution Safety, SECTION 19 — Cloud-Credential Safety (S3/R2/MinIO), SECTION 20 — Mobile App Safety (conditional, native mobile only): 114 → 135 items / 16 → 20 sections.** **Count change: Security Checklist 114→135 items / 16→20 sections ONLY** — no rule/scenario/prompt/deliverable/phase-hook/MCP count change. All other canonical counts unchanged (33 Rules · 40 Scenarios · 20 Bootstrap Steps · 61 Prompts (38 NEW ✨) · 14 UI Rules · 18 Phase Hooks · 28 deliverable files · 5 MCP servers · 6 memory-governance sections). |
| V32.21 | Spec-Persistence Taxonomy + Cross-Artifact Gap-Check — New Deliverable `scripts/spec-gap-check.sh` #28 | **Two linked additions.** (A) Rule 1 (`Master_Prompt.md`) gains a **Spec-Persistence Model addendum** naming three ways a spec can relate to its derived artifacts: **Flow-Forward** (completed artifacts immutable — a change is a new feature/version), **LIVING-SPEC** (**AIEF's default** — `docs/PRODUCT.md` is the Rule 1 contract; `inputs.yml`, the Prisma schema, and `docs/IMPLEMENTATION_MAP.md` are disposable derivations regenerated from it; this is exactly what Rule 1/2/3 already describe, now named), and **Flow-Back** (the artifact closest to the work — including code — is edited first, then the rest of the set is reconciled up; see the new **Scenario 40** for the 5-step reconcile loop: capture → classify → update disagreeing artifacts → run the gap-check → resume). **No new rule number** — this rides Rule 1. (B) **Adds `scripts/spec-gap-check.sh` as deliverable #28** — a pure-bash, read-only, advisory cross-artifact gap-check. Compares `docs/PRODUCT.md` ↔ the Prisma schema (DRIFT = schema model not in spec; UNBUILT = spec entity not in schema) ↔ `docs/IMPLEMENTATION_MAP.md` (UNBUILT = spec feature not tracked as built) ↔ `docs/STATE.md` (PHASE/REALITY MISMATCH = a late STATE.md phase but IMPLEMENTATION_MAP still shows BLOCKED/PARTIAL/TODO areas) ↔ `inputs.yml` (DERIVATION DRIFT = a top-level inputs.yml key with no PRODUCT.md mention). Supports `--report-only` (default, always exit 0) and `--check` (dry-run, exit 3 on any finding); `--file <relpath>` path-hardened against absolute paths / `..` traversal / symlink escape, mirroring `sync-context.sh` (#27). **NEVER writes any file, NEVER gates a phase** — purely advisory. **Wired in:** `deploy.sh` GROUP 11 (overwrite-with-backup + chmod +x); `phases.md` Phase 7 pre-flight MODEL HOOK (surfaces "🔍 Cross-Artifact Gaps" — non-blocking, complements the V32.5.5 + V32.7.3 back-port checks; NOT a new `memory-governance.md` §3 injection point, Phase Hooks stay 18); `memory-governance.md` §5 gains a "Flow-Back reconcile loop" subsection (section count stays 6); `Prompt_References.md`/`.html` Prompt 2.9 extended (not renumbered — Prompts stay 61) to run the script and enumerate the 5-way surface. **Deliverable count 27 → 28. Scenario count 39 → 40.** All other canonical counts unchanged (33 Rules · 61 Prompts · 20 Bootstrap Steps · 14 UI Rules · 18 Phase Hooks · 5 MCP servers · 114 Security Checklist items / 16 sections · 6 memory-governance sections). |
| V32.20 | Managed-Context Region + sync-context.sh — New Deliverable `scripts/sync-context.sh` #27 | **Adds `scripts/sync-context.sh` as deliverable #27** — a pure-bash, idempotent regenerator for a marker-delimited `AIEF:MANAGED` region in `CLAUDE.md` (`<!-- AIEF:MANAGED START -->` … `<!-- AIEF:MANAGED END -->`). Renders a compact Phase / Locked-stack / Last-done / Next / Blockers / Recent-CHANGELOG snapshot from `docs/STATE.md` (fallback `.cline/STATE.md`), the locked-stack line in `docs/DECISIONS_LOG.md` (fallback `docs/PRODUCT.md`), and the last 3 `docs/CHANGELOG_AI.md` headlines. **Idempotency is a hard contract:** the block is a pure function of those sources; the only "rev" identifier is `docs/STATE.md`'s `Updated:` field, falling back to `git rev-parse --short HEAD` — never `date`; an unchanged render performs zero writes. Supports `--check` (dry-run, exit 3 on drift) + `--file <relpath>` (path-hardened against absolute paths / `..` traversal / symlink escape). Corrupt single-marker files abort without writing; missing markers append the section at EOF. **Wired in:** `deploy.sh` GROUP 10 (overwrite-with-backup + chmod +x), `CLAUDE_compact.md` (new `## CURRENT PROJECT STATE (auto-managed)` section + FILE OWNERSHIP exception + CONTEXTUAL FILE LOADING row + version marker), `memory-governance.md` §2 Target 1 + §3 Hook Text POST (run after every STATE.md rewrite — mechanical tail step of Smart Checkpoint, 18 Phase Hooks count unchanged). **Deliverable count 26 → 27.** All other canonical counts unchanged (33 Rules · 39 Scenarios · 20 Bootstrap Steps · 61 Prompts · 14 UI Rules · 18 Phase Hooks · 5 MCP servers · 114 Security Checklist items / 16 sections). |
| V32.19 | Anti-Slop Gate Expansion — lint-design.sh D8 + P1b–P1i + design-principles.md Pillar 8 extended tells (designer-skill MIT harvest) | **Extends the V32.17 design anti-slop layer with tells harvested concept-only (MIT) from Pythoughts-labs/designer-skill's 44-rule detector.** `scripts/lint-design.sh` (#26) gains one new P0 cardinal sin — **D8 gradient/clip-to-text fill** (cardinal P0 sins 7→8, now D1–D8) — plus 8 advisory P1 warnings (P1b italic-serif display headline, P1c marketing buzzwords, P1d repeating-gradient stripes, P1e layout-property transitions [ties motion.md transform+opacity-only rule], P1f bounce/elastic easing, P1g justified body text, P1h image hover-transform, P1i crushed/negative letter-spacing). `design-principles.md` (#24) Pillar 8 broadens from 'the seven cardinal sins' to a full catalogue of extended visual/type/motion/copy/token-drift tells added as agent + `/design-review` guidance (NOT auto-checked — they need a browser/DOM/contrast engine the static bash gate can't run). The cream/beige tell is deliberately kept OUT of the lint gate to avoid false-positives on an intentional DESIGN.md cream theme; a deliberate DESIGN.md choice is never slop. QA checklist gains three items (no justified body text, no nested-cards for depth, no overflow:hidden clipping popover/tooltip ancestors). Posture UNCHANGED: advisory `--report-only` at all design phases, only P0 gates in `--strict` (Phase 5); INHERIT-not-REPLACE preserved. **Deliverable count UNCHANGED at 26** (extends existing #24 + #26, no new file). **Files changed:** `specdrivenprompt/lint-design.sh` (#26 source), `specdrivenprompt/design-principles.md` (#24 source), `Master_Prompt.md` (V32.19 changelog + active-version), `CLAUDE_compact.md` (version marker + lint-design table row), repo `CLAUDE.md` (deliverable #26 description), `Framework_Feature_Index.md` (this row + header + footer), `ChatGPT_Cross_Audit.md` (header + K.79). All other canonical counts unchanged (33 Rules · 39 Scenarios · 61 Prompts · 14 UI Rules · 18 Phase Hooks · 5 MCP servers · 114 Security Checklist items / 16 sections). |
...
*Last updated: V32.54 (41 rules · 51 scenarios · 20 bootstrap steps · 62 prompts (39 NEW ✨) · WSL2 native only · 44-file deliverable set · 159 security checklist items / 22 sections · 14 UI component rules · compact CLAUDE.md (ONLY auto-loaded file, V32.7) + 7 detail files in .ai_prompt/ (all on-demand, V32.7) · interactive HTML prompt references · DESIGN.md integration · Phase 3.3 interactive prototype + Phase 3.5 execution plan · Phase 4 + Phase 8 anti-thrashing rules · Skill Installer integration · Memory Governance Layer (6 sections, 18 phase hooks) · Architect-Execute Model — Zero Opus Execution (V32 — ALL phases + ad-hoc edits) · 500-Line Dispatch Gate (`wc -l` file-size checks) · Nine V32 Dispatch Rules (R1-R5 file-size mechanics + R6-R9 dispatch discipline) · R6 Scout-Before-Plan (non-allow-list reads > 100 lines via Scout-Sonnet) · R7 Default Parallel Fan-Out (≥2 independent dispatches MUST be parallel) · R8 Opus Write Allow-List (5 files: STATE.md, DECISIONS_LOG.md, CHANGELOG_AI.md, IMPLEMENTATION_MAP.md, .cline/STATE.md) · R9 Dispatch Ratio Metric (sonnet_writes / opus_writes ≥ 3.0) · Sonnet Status DONE tightened (full diff review mandatory — review-by-summary FORBIDDEN) · Sonnet Scout (R5 — files > 200 lines; R6 — non-allow-list reads > 100 lines) · **V32.3 Smart Governance Hydration** — R6 architect-read allow-list size qualifier (≤200 lines direct read; >200 lines route through Scout-Sonnet with Governance Extraction Schema in `memory-governance.md §4`); Rule 4 reframed from "read" to "hydrate"; per-doc hydration modes (keyword-filtered, domain-section-filtered, recency+flag-filtered, keyword+unresolved-filtered, area-status-filtered, session/task-scoped); R9 interaction — >200-line allow-list governance doc direct read counts as `opus_writes` for `dispatch_ratio` · UI loading-state dual-path with shadcn `<Skeleton>` + `<phantom-ui>` for custom components · Deploy-script PA-artifact detection routes fresh projects to Bootstrap, others to prompt 1.2 · Brownfield PA reverse-extraction via prompt 4.14 — Situation D injects PRODUCT.md generation from existing app/mockup artifacts) · **V32.4 react-doctor Phase Integration** — react-doctor (deterministic React diagnostics by Aiden Bai/Million.js) added as a per-phase supplementary skill at Phase 4 Parts 5-6 / Phase 5 / Phase 7; surfaced by /scan-project audit-driven recommendation + approval-gated install · **V32.5 Designer-Skills Phase Integration** — `julianoczkowski/designer-skills` bundle (8 children) prescribed at Phase 2.8 / Phase 4 Parts 5-6 / Phase 7 with INHERIT-not-REPLACE contract over PA's `docs/DESIGN.md` + `docs/MOCKUP.jsx`; `/design-tokens` expands, `/design-review` audits, `/design-refine` only on flagged components · **V32.5.1 Designer-Skills Gate-Closure Enforcement + Governance Hook Repair** — all three V32.5 MODEL HOOKs now block phase close until `/design-review` returns green; `memory-governance.md §3` hook template bumped (V32)→(V32.3); §3 enumerates 13 phase hooks · **V32.5.2** HTML parity fix · **V32.5.3** Prompt 3.23 Clean-Slate Rebuild (count 60→61 / 37→38 NEW ✨) · **V32.5.4** cosmetic sweep · **V32.5.5** DECISIONS_LOG↔PRODUCT.md back-port surface check · **V32.6 Interactive Prototype & Simulation Phase** — Phase 3.3; Phase Hooks 13→14 · **V32.6.1** Prompt 3.23.C manual-handoff · **V32.7 Detail-File Relocation** — all 7 detail files relocated from always-on .claude/rules/ to on-demand .ai_prompt/; CLAUDE.md is now the ONLY auto-loaded file; ~24 pre-flight Read-hardening edits; counts unchanged (17 files relocated not added) · **V32.7.1** PA dual-host · **V32.7.2** spec-executor subagent + settings.json caps (deliverable 17→19) · **V32.7.3** Design Baseline Back-Port Surface Check (non-blocking Phase 7/8 pre-flight token-diff against DESIGN.md/MOCKUP.jsx baseline; INHERIT-not-REPLACE back-port; closes TODO-21; design analogue of V32.5.5; zero count change) · **V32.7.4** lint-deploy.sh Phase 5/6 gate wiring (pre-deploy footgun gate named in phases.md; tooling reference, no count change) · **V32.7.5** lint-deploy.sh promoted to deliverable #20 (deploy.sh ships it to target scripts/lint-deploy.sh; deliverable 19→20) · **V32.8 Design-as-Contract + Verifiable-Done + Learning Loop** — Rule 31 Style Dictionary v5 compiled token pipeline (DTCG `docs/design-tokens.json` → CSS variables + Tailwind + Expo) + Playwright `toHaveScreenshot` visual-diff gate; Rule 32 evidence-backed DONE (test + screen + log) + LESSONS_REGISTRY + Stop-hook auto-append; `superpowers:verification-before-completion` driver skill; Rules 30→32 · Scenarios 35→39 · UI-rules 11→12 · Phase-hooks 14→17 · Bootstrap-steps 19→20 · Deliverables 20→22 · Memory-governance sections 5→6 · **V32.8.1** Drop getdesign.md/awesome-design-md — shadcn/ui CSS variables are the only design-token source; PA derives values from named direction, no external catalog; docs-only patch, zero count change · **V32.11 shadcn/studio Pro as Default Design Generator** — phase-routed /cui /iui /rui /ftc (3.3 trio · Parts 5-6 /cui+/rui · Phase 7 /cui+/iui+/rui · /ftc Figma-conditional); INHERIT-not-REPLACE over docs/DESIGN.md (Rule 12); fallback = plain shadcn/ui MCP + Blocks; MCP servers 4→5; all other counts unchanged · **V32.12 Design-Principles On-Demand Reference** — new `.ai_prompt/design-principles.md` deliverable #24 (library-agnostic UI/UX principles: hierarchy, spacing, type, 9-state control contract, UX laws, WCAG by success-criterion + QA checklist); INHERIT-not-REPLACE; deliverable count 23→24; all other counts unchanged · **V32.13 CI → Docker Hub → Komodo-API Auto-Deploy** — fleet Watchtower-free staging deploy: push-to-main → build+push image → Komodo API redeploys staging with the exact SHA (UpdateVariableValue → DeployStack → poll); app-side `deploy/komodo-deploy.sh` + `.github/workflows/docker-publish.yml` (Phase-6 scaffold templates, vendored from Server-Setups, NOT deliverables); prod NEVER auto-deployed; templates.md Rule 5c; zero count change · **V32.14 Motion Layer** — new `.ai_prompt/motion.md` deliverable #25 (library-agnostic UI/UX motion principles: when/when-not, easing-by-intent, duration budgets, transform+opacity-only performance rule, prefers-reduced-motion first-class + WCAG SC 2.3.3, spring-vs-tween, CSS-vs-JS, Motion+Tailwind appendix) + ui-rules.md Rule 14 "Motion & Micro-interactions" (Motion (motion.dev) only prescribed lib · LazyMotion/mini default · mandatory useReducedMotion() ties R13 WCAG gate · transform/opacity only · GSAP opt-in on PRODUCT.md signal · Three.js/R3F parked); INHERIT-not-REPLACE over DESIGN.md motion tokens; UI rules 13→14 · deliverable count 24→25; all other counts unchanged · **V32.16 Storage Default Stays MinIO** (Cloudflare R2 demoted to budget-gated OPT-IN; V32.15 reverted before rollout; zero count change) · **V32.17 Design Anti-Slop Gate + Sharpened Craft Rules** — new `scripts/lint-design.sh` deliverable #26 (D1–D7 seven cardinal sins + P1a advisory; --report-only at design phases; clones lint-deploy.sh #20 pattern); design-principles.md (#24) gains Pillar 8 Anti-AI-Slop + letter-spacing table + five-state render contract; deliverable count 25→26; all other counts unchanged · **V32.19 Anti-Slop Gate Expansion** — lint-design.sh #26 gains D8 gradient/clip-to-text P0 (cardinal sins 7→8) + P1b–P1i advisory; design-principles.md #24 Pillar 8 extended visual/type/motion/copy/token-drift tells; designer-skill MIT harvest; deliverable count unchanged 26 · **V32.20 Managed-Context Region** — new `scripts/sync-context.sh` deliverable #27 (idempotent AIEF:MANAGED CLAUDE.md regenerator sourced from docs/STATE.md + DECISIONS_LOG/PRODUCT.md + CHANGELOG_AI.md; path-hardened; invoked by the Smart Checkpoint POST hook); deliverable count 26→27; all other counts unchanged · **V32.21 Spec-Persistence Taxonomy + Cross-Artifact Gap-Check** — Rule 1 gains a named Spec-Persistence Model addendum (Flow-Forward / LIVING-SPEC AIEF-default / Flow-Back, no new rule number); new `scripts/spec-gap-check.sh` deliverable #28 (advisory, non-blocking PRODUCT.md↔inputs.yml↔Prisma-schema↔IMPLEMENTATION_MAP.md↔STATE.md gap scan, wired at the Phase 7 pre-flight MODEL HOOK + Prompt 2.9); new Scenario 40 (Flow-Back 5-step reconcile loop); deliverable count 27→28, scenario count 39→40; all other counts unchanged · **V32.22 Attack-Informed Security Hardening** — Claude-Red offense→defense harvest; security.md gains 10 new hardening blocks (JWT alg-pinning, OAuth/SSO safety, open-redirect, RCE/command-injection prohibition, race/TOCTOU generalization, prototype-pollution, cloud-credential safety, mobile-app safety conditional, request-smuggling note, clickjacking); Security_Checklist.md gains SECTION 17 Auth-Token/OAuth, SECTION 18 Command/Code-Execution, SECTION 19 Cloud-Credential, SECTION 20 Mobile-conditional; Security Checklist 114→135 items / 16→20 sections; all other counts unchanged · **V32.23 Constitution as Versioned Contract** — Sync Impact Report machine-readable header at the top of Master_Prompt.md (version delta + bump type + dependent-file propagation checklist, replaces retired ChatGPT_Cross_Audit.md propagation-verification role); SemVer-for-rules subsection (MAJOR = remove/redefine Rule/Phase/security-level · MINOR = additively add Rule/Scenario/Prompt/gate/phase-hook/deliverable · PATCH = non-behavioral clarification); Constitution-Check gate at phases.md pre-flight step 0b (confirm no violation of the 33 Rules/L1-L6 before executing; [HOW] resolved directly, [WHAT] escalated to PENDING_DECISIONS.md); zero count change · **V32.24 Spec Expert Panel** — new Prompt 3.24 + Scenario 41: at Phase 2.8 + Phase 3 pre-lock, PM dispatches 5 parallel expert-lens Sonnet subagents (secure-code-guardian / architecture-designer / api-designer / test-master / database-optimizer) against docs/PRODUCT.md, synthesizes + dedups + prioritizes findings, feeds them into the Flow-Back/LIVING-SPEC reconcile; new phases.md MODEL HOOKs (not new phase hooks) gate Phase 2.8 handoff + Phase 3 spec-lock on zero unresolved CRITICAL findings; prompts 61→62 (38→39 NEW ✨), scenarios 40→41; all other counts unchanged · **V32.24.1 Content-Container / Responsive-Gutter Design Default** — global basic-design default (owner-set 2026-07-10): readable/dense content is never full-bleed — centered `mx-auto max-w-7xl` (~1280px cap) + mobile-first safe gutter never 0 on any breakpoint (`px-4 sm:px-6 lg:px-8`); immersive surfaces (kiosk/wall-display/full-bleed hero/maps-canvas) exempt; encoded in `~/.claude/rules/design-defaults.md` Entry 1 + `design-principles.md` Pillar 3 + `ui-rules.md` Rule 8 pointer (no new numbered UI rule, advisory lint deliberately skipped as not low-false-positive); zero count change · **V32.24.2 Layout-Archetype Design Default** — global basic-design default (owner-set 2026-07-10, Entry 2 of the design-defaults surface): pick the page shell by what the app IS — marketing/brand site = FULL-WIDTH (full-bleed immersive), general-purpose/purpose-driven app (dashboard/admin/tool/back-office/console) = PERSISTENT LEFT-SIDEBAR APP-SHELL (always-available left nav, off-canvas on mobile / icon-rail for focused-work consoles, + large right content area; no top header-tab-strip nav for multi-section functional apps); framework build = shadcn/ui `sidebar` (SidebarProvider + Sidebar + SidebarTrigger + SidebarInset), inset content still uses the Entry-1 container/gutter; exempt (immersive, no app-shell) = kiosk/wall-display/public marketing-landing-auth; encoded in `~/.claude/rules/design-defaults.md` Entry 2 + `design-principles.md` Pillar 3 "Layout archetype" + `ui-rules.md` Rule 8 pointer (no new numbered UI rule, INHERIT-not-REPLACE); zero count change · **V32.25 Tenant RBAC Standard** — new Rule 34 (3-tier backbone tenant_manager/tenant_superadmin/tenant_admin + one-owner-per-tenant partial-unique index + two-way succession + sub-role presets + tenant-scoped custom-role permission-matrix system) + new deliverable `.ai_prompt/rbac.md` #29 + new Scenario 42 (Existing-App RBAC 3-Tier Retrofit) + `security.md` L3 (RBAC) block + `Security_Checklist.md` SECTION 21 (+12 items); ALTER TYPE…RENAME VALUE (never DROP/CREATE) + partial-unique index prescribed; guardrails: custom roles tenant-scoped, ≤ tenant_admin, never Billing/User-Management; per-env creds from Server-Setups vault; Rules 33→34 · Scenarios 41→42 · Deliverables 28→29 · Security Checklist 135→147 items / 20→21 sections; UI rules stay 14 · prompts stay 62 · **V32.26 Sidebar-Footer Version + White-Label Standard** — every app-shell SidebarFooter renders a muted version tag v{X.Y.Z} + a "Developed by Powerbyte IT Solutions" new-tab white-label link (target="_blank" rel="noopener noreferrer"); Phase 4 Parts 5-6 GATE-CLOSURE item + ui-rules.md Rule 8 pointer; mirrors ~/.claude/rules/design-defaults.md Entry 3; zero count change, UI rules stay 14 · **V32.27 Storage Default → Telegram** — Telegram is the DEFAULT persistent-media backend on dev + staging + prod, demo = MinIO only; zero count change · **V32.28 Event Delivery & Notifications** — new CONDITIONAL on-demand deliverable `.ai_prompt/notifications.md` #30 + Scenario 43 (right-sized FOSS multi-channel event-delivery pattern; Tier 1 DEFAULT Valkey Streams + BullMQ, Tier 2 opt-in NATS JetStream; 6 mandatory additions: schema+versioning, tenant isolation, notification preferences, idempotency-at-ingestion, per-provider rate limits, PII routing; NO new Rule; Scenarios 42→43, Deliverables 29→30) · **V32.29 Telegram Storage SCAFFOLD-CODE port** — the scaffold now EMITS the Telegram dual-backend storage layer (packages/storage StorageAdapter S3/MinIO + Telegram, MediaObject ledger + additive migration, /api/media proxy + resolveMediaBytes dual-read, upload-router Telegram branch, per-env STORAGE_BACKEND/TELEGRAM_* wiring; dev+staging+prod=Telegram/dev-own-channel, demo=MinIO, MinIO retained fallback) instead of MinIO-only; prose-scaffold, not a copied-file deliverable; lands the V32.27 SCAFFOLD-CODE-PENDING deferral; zero count change · **V32.30 SEO Foundation (Adaptive Baseline)** — new constitutional Rule 35 (every app scaffolded with SEO out of the box, adaptive to route — public=full SEO, private/authed=noindex+disallow, fail-closed to private) + new always-on deliverable `.ai_prompt/seo.md` #31 + new Scenario 44 (Existing-App SEO Foundation Retrofit); built on Next.js native primitives (metadata/generateMetadata, sitemap.ts, robots.ts, alternates.canonical/.languages, openGraph/twitter, opt-in next/og); enforced Phase 4 scaffold + Phase 5 gate (Lighthouse SEO ≥ 90 hard-gated on public-facing apps); Rules 34→35, Scenarios 43→44, deliverables 30→31 · **V32.31 SEO-Aware Design & Content** — extends Rule 35 + `.ai_prompt/seo.md` (new §1.5) + Scenario 44 upstream into the Planning Assistant DESIGN.md/MOCKUP.jsx step + Phase 2.8 mockup + Phase 3.3 prototype, so public/marketing surfaces are Google-friendly by design (single-H1 heading hierarchy, keyword-informed copy, descriptive link text, planned alt text, CWV-aware layout) instead of bolted on after the frontend is built; no new PRODUCT.md section; zero count change · **V32.32 CI/CD Pipeline Standard** — new Rule 36 (build the deploy image ONCE, promote the SAME bytes forward dev→staging→prod, demo retag-only; CI gates/builds/pushes, NEVER auto-deploys; staging = data-first refresh gate; production migration = deliberate manual `prisma migrate deploy`, never auto-on-boot, prod data never wiped/reseeded; demo = migrate-yes/reseed-never + 6h golden-dataset self-heal; rollback couples image+schema) + new always-on deliverable `.ai_prompt/cicd.md` #32 + new Scenario 45 (Existing-App CI/CD Pipeline Retrofit) + new generator `cicd-gen/` + `phases.md` Phase 6 wiring; Rules 35→36, Scenarios 44→45, Deliverables 31→32; all other counts unchanged; HARD HOLD — local commits only · **V32.33 Cline STATE Split-Brain Retirement** — retires `.cline/` as the canonical session-memory home in favor of `docs/` (`docs/STATE.md`, `docs/memory/lessons.md`, `docs/memory/agent-log.md`, `docs/handoffs/`, `docs/tasks/`); `.clinerules` generation stops (one-line tombstone note); MINOR, purely additive — ZERO-DISRUPTION: apps deployed before V32.33 keep `.cline/` and behave identically (never migrated in place), new apps are born on `docs/`; `design-stop-hook.sh` + `spec-gap-check.sh` gain an additive `.cline/STATE.md` read-fallback (never removes a read path, `sync-context.sh` already had it); `deploy.sh` NEVER_TOUCH/dir-protect gains `docs/memory`, `docs/handoffs`, `docs/tasks` alongside the retained `.cline/` + `.specstory/history` entries; new Scenario 46 (per-app `.cline/`→`docs/` migration); Scenarios 45→46, Rules stay 36; all other counts unchanged · **V32.34 SpecStory Retirement + Governance-Sync git-rebase** — SpecStory + `.specstory/history/` passive capture RETIRED (tombstoned in place, slot kept); Claude Code is the sole in-project agent and self-attributes via Rule 15; Governance Sync now sources diffs from `git log --since=$LAST` + `git diff HEAD` instead of `.specstory/history/`; Rule 19 + Scenarios 17+18 tombstoned in place (no renumber); SpecStory agent entry marked RETIRED but KEPT LISTED so agents stay 6 (mirrors deprecated-Cline); ZERO-DISRUPTION — deployed apps keep `.specstory/history/` as a frozen legacy audit trail, never migrated/deleted; `ChatGPT_Cross_Audit.md` FROZEN; Rules stay 36, Scenarios stay 46, agents stay 6, all other counts unchanged · **V32.35 Architecture Posture** — Rules 36→37, Scenarios 46→47, deliverables 32→33 (deliverable #33 `microservices.md`) · **V32.36 Design-Fidelity Enforcement** — deliverables 33→34 (deliverable #34 `design-fidelity.mjs`), Rules/Scenarios unchanged · **V32.37 Design-Fidelity Completion** — Rules/Scenarios/deliverables all UNCHANGED (37/47/34) · **V32.38 App Audit Toolkit** — new Rule 38 (tiered Compose-native audit toolkit, MANUAL/ON-DEMAND by default — nothing auto-runs; tiers are cost bands; blocking gates pre-commit/Phase-5 are per-app opt-ins) + new deliverables `.ai_prompt/audit.md` #35 + `scripts/audit-app.sh` #36 (monorepo-aware type-check) + new Scenario 48; fixes the F2 phantom-gate defect in `security.md`; HARD HOLD; Rules 37→38, Scenarios 47→48, deliverables 34→36 · **V32.39** Dev-Freshness Deliverable — new Rule 39 (Dev Leads Every Environment) + deliverable #37 `scripts/dev-freshness-check.sh`; Rules 38→39, deliverables 36→37 · **V32.40** Capability-Primer Regenerator — new deliverable #38 `scripts/build-primer.sh` (idempotent AIEF:PRIMER FLAGS-slice regenerator sourced from docs/primer.yml; sibling of sync-context.sh #27; makes skill/plugin loadout project-aware); no new Rule/Scenario, deliverables 37→38 · **V32.41** Capability-Primer Continuous Self-Refresh — spec-gap-check.sh #28 CLASS 5 (PRIMER MISSING/INCOMPLETE/DRIFT) + Phase 7 self-refresh wiring (refresh primer + ONE targeted search-skill, never full scan-project); no count change · **V32.42** CI/CD Komodo Stack Registration Audit — extends Rule 36; new generator artifact `deploy/komodo-verify.sh`/`cicd-gen --audit` verifies every non-dev env stack (prod first) is Komodo-tracked (`km list stacks`/`ListStacks`), scripted-registers an untracked stack via ResourceSync TOML (manual Scenario-32 UI fallback), fail-open on tooling/fail-closed on untracked-prod; advisory Phase-6 backstop + Scenario 45 step + Scenario 32 Part C1a; production stays MANUAL-trigger; no count change · **V32.43** AdminCN Fleet-Default Design Starter — new deliverable #39 `.ai_prompt/admincn-starter.md` + vendored curated slice `specdrivenprompt/starter/admincn/` (~222 files, seeded at bootstrap Step 20d); makes AdminCN (shadcn/studio Pro admin template) the fleet-default UI/UX baseline, UI-layer-only INHERIT-not-REPLACE (keep tRPC/Prisma/Auth.js, `fake-db`→tRPC graft); default-layout=default + 5 opt-in; integrated at ui-rules Rule 8 / phases MODEL HOOK / Planning_Assistant Step 7 / templates + global skill-loadout-card; license = use-in-own only, AIEF repo PRIVATE; deliverables 38→39, all other counts unchanged · **V32.44** Existing-App AdminCN Design Adoption Scenario — new Scenario 49 (retrofit path for the existing fleet, consistent with retrofit Scenarios 42/44/45/48): UI-layer-only INHERIT-not-REPLACE, keep tRPC/Prisma/Auth.js, re-wire adopted views off fake-db via the #39 graft, strangler-style shell adoption, preset→tokens.json (Rule 12), Rule-31 baseline update; runs per-seat (never from AIEF seat) + a rollout tracker; Scenarios 48→49, all other counts unchanged · **V32.45** Button-Affordance Standard — every primary/secondary/CTA button carries a subtle emboss (shadow-xs/sm) or outline; flat = ghost/link tertiary only; emboss adds ELEVATION only, colour always follows the theme accent; extends ui-rules Rule 3 + design-principles.md + new lint-design P1j advisory; CLAUDE.md #26 P1a-P1i→P1a-P1j; NO count change · **V32.47** Invisible-Quality Radar (framework mirror of the global radar) — new Rule 40 (always-on, weight-gated INDEX/POINTER over the ten invisible-quality dims; NOT a new enforcer/gate; INHERIT-not-REPLACE) + capability-primer FLAGS field INVISIBLE_DIMS_IN_SCOPE (build-primer.sh #38) + Phase-7 done-claim consideration line + spec-gap-check.sh CLASS 6; D6/D7/D8 named-but-unowned; ISO/IEC 25010 conformance-aware/certification-ready, NO ISMS; mirrors ~/.claude/library/invisible-quality-radar.md; Rules 39→40, all other counts unchanged · **V32.48** lint-design advisory expansion — 11 new checks P1k–P1u (content/microcopy + static AI-tells) added to `lint-design.sh` #26 (`CLAUDE.md` #26 P1a-P1j → P1a-P1u); advisory `--report-only`/exit 0, no count change · **V32.48.1** AdminCN starter-slice dedup — `deploy.sh` gitignores the vendored `starter/admincn/` slice (local cherry-pick reference, re-copied each sync, never committed) so it stops duplicating an already-shadcn app's own `components/ui`; PATCH behavioral bugfix, no count change · **V32.48.2** AdminCN slice dedup COMPLETION — `deploy.sh` `GITIGNORE_ENTRIES` also gitignores the STAGING copy `.ai_prompt/starter/admincn/` that `sync-to-project.sh` seeds (a root-anchored `starter/admincn/` pattern can't cover the `.ai_prompt/` path), closing the incomplete V32.48.1 fix; per-seat cleanup untracks BOTH paths; PATCH, no count change · **V32.49** IconStack default icon-sourcing layer — ui-rules Rule 9 makes the IconStack MCP (51k+ MIT icons, 21 libraries) the default source for any glyph beyond lucide, materialized as a self-hosted SVG (no runtime dependency); `lucide-react` stays the shadcn runtime baseline (INHERIT-not-REPLACE); new `AI_Tools_Reference.md` §2.6, MCP servers 5→6; global `~/.claude/rules/design-defaults.md` icon-source entry; MINOR, no Rule/Scenario/deliverable change · **V32.50** Site Access & Tenancy Bootstrap Standard — new Rule 41 (site/URL topology, companion to Rule 34's role mechanics), `rbac.md` Part E (platform-scope roles, distinct permission vocabulary, CHECK-constraint anti-escalation) + Part F (`/tm` topology, `/platform`→`/tm` rename, reserved slugs), new Scenario 50, new `Security_Checklist.md` §22 (+12 items); Rules 40→41, Scenarios 49→50, Security Checklist 147→159/21→22; HARD HOLD · **V32.51** Review Pre-Scope for `/code-review` — new deliverables #40 `review-scope.mjs` + #41 `review-scope.md`, new Scenario 51, deliverables 39→41; MINOR · **V32.52** Full-Audit Pre-Scope for the `full audit check` swarm — new deliverables #42 `audit-scope.mjs` (deterministic target-type × dimension/gate pre-scope engine) + #43 `audit-scope.md`, the audit-side sibling of #40/#41, deliverables 41→43; MINOR · **V32.53** Capability-Map Loadout Adoption + Skill-Cue Refresh — per-phase loadout now consults the global capability map + STEP-0.5 skill re-check, own-library-first, Black Magic gated BM-1/2/3; new prescribed skills design-brief/humanize/ai-check/doubt-driven-development/video-shot-planner/app-showcase/image/dataviz/web-motion; MINOR, additive, no count change · **V32.54** Content Human-Voice Pass — new on-demand deliverable #44 `content-voice.md` (makes the globally-installed humanize + ai-check skills an automatic-when-content pass over human-facing published prose; strengthens the V32.53 humanize/ai-check phase cue into a documented on-demand pass; NOT constitutional/always-on, contrast Rule 35 SEO); no new Rule (stays 41), deliverables 43→44; MINOR, additive · Maintained by Claude on behalf of Bonito — Powerbyte IT Solutions, Calapan City, Oriental Mindoro PH*

> **Note:** This file is updated by Claude alongside every version bump. You never edit it manually. It is part of the 26-file V32.19 deliverable set — 19 files in `.ai_prompt/`: CLAUDE_compact.md · Master_Prompt.md · bootstrap.md · phases.md · security.md · ui-rules.md · scenarios.md · templates.md · memory-governance.md · Planning_Assistant.md · Framework_Feature_Index.md · AI_Tools_Reference.md · Security_Checklist.md · ChatGPT_Cross_Audit.md · Prompt_References.md · Prompt_References.html · privacy.md · design-principles.md · motion.md — plus `deploy.sh` at project root — plus `spec-executor.md` (→ `.claude/agents/`) and `settings.json` (→ `.claude/settings.json` merged) added at V32.7.2 — plus `lint-deploy.sh` (→ `scripts/lint-deploy.sh`, chmod +x) added at V32.7.5 — plus `design-tokens.json` (→ `docs/design-tokens.json`, DTCG token source-of-truth) and `LESSONS_REGISTRY.md` (→ `docs/LESSONS_REGISTRY.md`, per-project learning log) added at V32.8 — plus `privacy.md` (→ `.ai_prompt/privacy.md`, PH Data Privacy Act + WCAG 2.2 AA compliance rules) added at V32.9 — plus `design-principles.md` (→ `.ai_prompt/design-principles.md`, library-agnostic UI/UX design principles) added at V32.12 — plus `motion.md` (→ `.ai_prompt/motion.md`, library-agnostic UI/UX motion principles + ui-rules.md Rule 14) added at V32.14 — plus `lint-design.sh` (→ `scripts/lint-design.sh`, chmod +x, design anti-slop gate D1–D7 + P1a advisory) added at V32.17. V32.7: all 7 detail files (phases.md through templates.md) are in `.ai_prompt/` only — `.claude/rules/` is intentionally empty.
>
