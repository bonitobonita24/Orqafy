...
## THE 41 RULES (compact — full details in .ai_prompt/phases.md, other .ai_prompt/ files, and AI/Master_Prompt.md)
...
Rule 40  Invisible-Quality Radar — ALWAYS-ON, weight-gated (trivial tasks skip) radar that keeps the deep-code, non-functional quality dimensions in active consideration as you build. At the analyze-confirm task-start loadout of any REAL feature/build task, ask "which IN-SCOPE invisible dims does this task touch, and is each one's owner surface loaded?": D1 security (security.md L1-L6 · audit.md R38) · D2 SEO (seo.md R35 +§GEO) · D3 a11y/WCAG (ui-rules R13 · motion.md) · D4 privacy (privacy.md R33) · D5 audit-ability (audit.md R38 · L5 AuditLog) · D6 perf-budget · D7 observability · D8 ISO-conformance · D9 reliability · D10 maintainability. An INDEX/POINTER, NOT a new enforcer or blocking gate — INHERIT-not-REPLACE over those owner rules. Which dims are in-scope for THIS app = capability-primer FLAGS INVISIBLE_DIMS_IN_SCOPE (build-primer.sh #38). Teeth = advisory only: Phase-7 done-claim consideration line + spec-gap-check.sh CLASS 6 (declared-in-scope dim, no evidence built). D6/D7/D8 = NAMED-but-unowned (radar keeps them visible; no new authority file). ISO/IEC 25010 = vocabulary spine, conformance-AWARE/certification-READY, NO ISMS. Mirrors ~/.claude/library/invisible-quality-radar.md. HARD HOLD.
Rule 41  Site Access & Tenancy Bootstrap — the site/URL topology every tenant-based app exposes, companion to Rule 34's role mechanics. Read .ai_prompt/rbac.md Parts E+F. `/tm` Tenant Management Site for the platform operator (tenant_manager FIXED default ADMIN ceiling unchanged + seeded tenant_billing/tenant_tech platform sub-roles, more creatable in-frontend). Per-tenant ONE role-routed login: /{slug}/login (regular users); admin-tier (tenant_superadmin/tenant_admin) lands /{slug}/admin; global /admin dropped. Platform permissions live in a DISTINCT vocabulary (PlatformPermissionKey/PlatformRolePermission) — never the tenant FeatureKey table — enforced by a CHECK constraint (scope='tenant' AND tenant_id IS NOT NULL) OR (scope='platform' AND tenant_id IS NULL). Demo deployments carry NO /tm at all (separate subdomain, single-tenant). /platform→/tm rename ships a redirect shim; reserved slugs tm/demo/platform/admin/login/api rejected at tenant-slug creation. Retrofit: Scenario 50. Security_Checklist.md §22. HARD HOLD.
```
...
Auth / RBAC / roles / user-mgmt / roles UI / `/tm` platform site / site-access routing Read .ai_prompt/rbac.md (Rule 34 Parts A-D — V32.25 3-tier backbone + one-owner-per-tenant partial-unique index + succession + tenant-scoped custom-role matrix; Rule 41 Parts E+F — V32.50 platform-scope roles (tenant_billing/tenant_tech, distinct permission vocabulary, CHECK-constraint anti-escalation) + `/tm` site-access URL topology; also load when PRODUCT.md Roles & Permissions is populated)
...
> includes step 0b — confirm the planned task does not violate any of the 41 Rules or the L1-L6 security
...
## MCP SERVERS (6 total — 3 project-wired + 2 user-global + 1 plugin)
...
iconstack         Icon SOURCING layer — search_icons/get_icon_svg/list_libraries; 51k+ MIT icons/21 libs, design-time (V32.49).
                  lucide-react STAYS runtime baseline; source anything beyond lucide → self-hosted SVG in components/icons/.
                  NO runtime dep (CSP/offline safe). Detail: .ai_prompt/ui-rules.md Rule 9 + AI_Tools_Reference.md §2.6.
...
<!-- AIEF:MANAGED START -->
<!-- Auto-generated by scripts/sync-context.sh — do not edit between markers. Everything else is human-owned. Source rev: unset -->

**Phase:** (run `bash scripts/sync-context.sh` to populate)
