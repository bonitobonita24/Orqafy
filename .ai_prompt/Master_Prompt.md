...
Version:  V32.53 → V32.54
Bump:     MINOR (adds on-demand deliverable #44 content-voice.md — the Content Human-Voice Pass; strengthens
          the V32.53 humanize/ai-check phase cue into a documented on-demand pass; NO new Rule)
Summary:  New on-demand deliverable #44 `.ai_prompt/content-voice.md` makes the globally-installed `humanize`
          + `ai-check` skills an automatic-when-content, never-invoked-by-name pass over human-facing
          published prose (marketing/landing/blog/caption/in-app/notification/generated-docs copy). It is
          ON-DEMAND — pulled ONLY when a content task fires, NOT a constitutional always-on concern
          (deliberately contrasted with Rule 35 SEO, which every app IS scaffolded with): never loaded in a
          non-content session, never scaffolded into an app. Write-human-first (humanize generate mode) OR
          humanize-rewrite → `ai-check` gate before content is "done"; rides ON TOP of
          `copywriting`/`app-showcase`/`doc-coauthoring` (INHERIT-not-REPLACE); SKIP for non-prose
          (code/config/JSON/data) + rigid/legal register; English-tuned levers, principles transfer to other
          languages (detector-parity does not). This promotes the V32.53 humanize/ai-check loadout hint into
          a documented pass — NO new Rule (rule count stays 41), no new Scenario. HARD HOLD — local only.
Dependent files reviewed (propagation checklist):
  [x] Master_Prompt.md             — this Sync Impact Report block; V32.54 changelog entry
  [x] content-voice.md             — the new deliverable #44 (authored; e0af87a)
  [x] CLAUDE.md (repo)             — Canonical Counts (deliverables 43→44); intro prose + "N-file" blurb;
                                     deliverable enumeration gains #44 `content-voice.md`
  [x] CLAUDE_framework_repo.md     — canonical counts: deliverables 43→44 (3 sites)
  [x] CLAUDE_compact.md            — H1 title version → V32.54
  [x] Framework_Feature_Index.md   — new V32.54 table row; Current-version line + footer → V32.54 (44-file)
  [x] README.md                    — shields.io version badge + "Framework Features" heading → V32.54;
                                     deliverable count 43→44 (2 sites)
  [x] documentation-hub.html · Prompt_References.html (×2) — hero current-version markers → V32.54;
                                     deliverable count 43→44 where shown
  [x] deploy.sh                    — ships #44 `content-voice.md` (→ .ai_prompt/) + header manifest/tree
  [x] phases.md                    — V32.54 MODEL HOOK (Content Human-Voice Pass, on-demand) beside the SEO
                                     content hook; strengthens the V32.53 humanize/ai-check cue
  [x] AI_Tools_Reference.md        — §3.9 cross-reference to the new content-voice.md authority
  [x] scripts/check-framework-alignment.sh — Rules-parity regex widened to enforce at 40+ (linter bug fix)
  Canonical counts:                44 deliverable files (was 43). All others UNCHANGED — 41 Rules ·
                                   51 Scenarios · 62 Prompts (39 NEW ✨) · 14 UI Rules · 6 MCP servers ·
                                   18 Phase Hooks · 6 agents · 20 Bootstrap Steps.
═══════════════════════════════════════════════════════════════════════════════════ -->

<!-- ═══════════ SYNC IMPACT REPORT — auto-maintained on every version bump ═══════════
Version:  V32.52 → V32.53
Bump:     MINOR (adds the "per-phase loadout / capability-map discipline" pointer + refreshes the
          prescribed skills per phase — additive guidance only. No new Rule, no new Scenario, no new
          deliverable, no removals — the existing Rules/Phases/security levels/deliverable count are
          UNCHANGED.)
Summary:  The framework now references the global Capability Map (`~/.claude/library/capability-map.md`)
          + the always-on STEP-0.5 skill re-check (`~/.claude/rules/skill-loadout-card.md` +
          `analyze-confirm-gate.md` HAND-1) for per-phase loadout: rescan live/new skills, arm from a
          curated dev-phase × task × skills × Black Magic map. **Our OWN library is always the default**;
          AI Black Magic is GATED — never auto-queried (only BM-1 curating our own skills/the map, BM-2 a
          real capability gap after already searching our library, or BM-3 the owner explicitly asks).
          New skills now prescribed by phase: `design-brief` (design translator, run FIRST at Phase
          2.8/3.3), `humanize` + `ai-check` (docs/showcase/marketing copy — Phase 7 + app-showcase),
          `doubt-driven-development` (high-stakes/irreversible decisions — pairs with the Phase 3 pre-lock
          Spec Expert Panel), `video-shot-planner` · `app-showcase` · `image` · `dataviz` · `web-motion`
          (showcase/marketing/charts, cross-phase). These skills/rules live GLOBALLY in `~/.claude/` — every
          seat already loads them; this change only makes the framework's phase hooks reference the
          discipline, it deploys nothing new via `deploy.sh`. INHERIT-not-REPLACE over the existing
          per-phase skill tables. HARD HOLD — local only, no push/merge/deploy.
Dependent files reviewed (propagation checklist):
  [x] Master_Prompt.md             — this Sync Impact Report block; active-version marker → V32.53
  [x] phases.md                    — V32.53 MODEL HOOK (per-phase Capability Map loadout), after the
                                     V32.24 Spec-Expert-Panel Phase-2.8 hook
  [x] CLAUDE.md (repo)             — Skills Library Awareness ASCII table (design-brief added to Phase
                                     2.8/3.3 rows) + new V32.53 note paragraph
  [x] AI_Tools_Reference.md        — new §3.9 "Per-phase loadout & capability map (V32.53)"
  [x] CLAUDE_compact.md            — H1 title version → V32.53
  [x] Framework_Feature_Index.md   — new V32.53 table row; Current-version line + footer → V32.53
  [x] README.md                    — shields.io version badge → V32.53
  [x] documentation-hub.html · Prompt_References.html (×2) — hero current-version markers → V32.53
  [x] ~/.claude/library/capability-map.md + rules/skill-loadout-card.md + analyze-confirm-gate.md —
                                     the global discipline authored (non-deliverable, already live)
  Canonical counts:                ALL UNCHANGED — 41 Rules · 51 Scenarios · 62 Prompts (39 NEW ✨) ·
                                   14 UI Rules · 6 MCP servers · 18 Phase Hooks · 6 agents ·
                                   20 Bootstrap Steps · 43 deliverable files.
═══════════════════════════════════════════════════════════════════════════════════ -->

<!-- ═══════════ SYNC IMPACT REPORT — auto-maintained on every version bump ═══════════
Version:  V32.51 → V32.52
Bump:     MINOR (adds the "Full-Audit Pre-Scope" capability — additive throughout. Two new deliverables:
          #42 `audit-scope.mjs` (an advisory, always-exit-0 deterministic pre-scope engine feeding the
          `full audit check` swarm) + #43 `audit-scope.md` (its on-demand authority). No new Rule, no new
          Scenario, no removals — the existing Rules/Phases/security levels are UNCHANGED. Precedent:
          V32.51 added the code-review pre-scope (#40/#41) the same way — new on-demand authority + new
          script deliverable, no Rule churn. This is its audit-side sibling.)
Summary:  Extends the V32.51 review-scope PATTERN (a deterministic 🔧 pre-step that scopes an agent's work)
          from code review to the `full audit check` semantic swarm. `audit-scope.mjs` parses a diff,
          classifies the TARGET TYPE (framework/app/foreign/session), and resolves per audit-worthy surface
          which audit DIMENSIONS apply (`full-audit-check-trigger.md` taxonomy + invisible-quality-radar
          D1-D10, primer-FLAG scoped) + which existing mechanical GATE script is relevant (Option A —
          list-only), emitting a manifest (JSON + Markdown) to `test-artifacts/audit/`. It is a guaranteed
          FLOOR of the audit dimensions in play — never a ceiling, never a blocker (advisory, always exit 0).
          Consumed by the `full audit check` Architect at step 1. `audit-scope.md` documents the two design
          laws, the target-type classifier, and the dimension/gate maps. INHERIT-not-REPLACE over
          `full-audit-check-trigger.md` + Rule 40 (invisible-quality-radar). HARD HOLD — local only.
Dependent files reviewed (propagation checklist):
  [x] Master_Prompt.md             — this Sync Impact Report block; active-version marker → V32.52
  [x] CLAUDE.md (repo)             — Canonical Counts (deliverables 41→43); intro prose; deliverable
                                     enumeration gains #42 `audit-scope.mjs` + #43 `audit-scope.md`
  [x] CLAUDE_framework_repo.md     — canonical counts: deliverables 41→43 (3 sites)
  [x] CLAUDE_compact.md            — H1 title version → V32.52
  [x] Framework_Feature_Index.md   — new V32.52 table row; Current-version line + footer → V32.52 (43-file)
  [x] README.md                    — shields.io version badge + "Framework Features" heading → V32.52; deliverable count 41→43
  [x] documentation-hub.html · Prompt_References.html (×2) — hero current-version markers → V32.52; deliverable count 41→43
  [x] deploy.sh                    — ships #42 `audit-scope.mjs` (→ scripts/) + #43 `audit-scope.md` (→ .ai_prompt/)
  [x] ~/.claude/library/full-audit-check-trigger.md — pre-scope seam wired into the method (global, non-deliverable)
  Canonical counts:                43 deliverable files (was 41). All others UNCHANGED — 41 Rules ·
                                   51 Scenarios · 62 Prompts (39 NEW ✨) · 14 UI Rules · 6 MCP servers ·
                                   18 Phase Hooks · 6 agents · 20 Bootstrap Steps.
═══════════════════════════════════════════════════════════════════════════════════ -->

<!-- ═══════════ SYNC IMPACT REPORT — auto-maintained on every version bump ═══════════
Version:  V32.50 → V32.51
Bump:     MINOR (adds the "Review Pre-Scope" capability — additive throughout. Two new deliverables:
          #40 `review-scope.mjs` (an advisory, always-exit-0 deterministic pre-scope engine feeding
          `/code-review`) + #41 `review-scope.md` (its on-demand authority). New Scenario 51. No new
          Rule, no removals — the existing Rules/Phases/security levels are UNCHANGED. Precedent:
          V32.38 added the Audit Toolkit the same way — new on-demand authority + new script deliverable
          + new Scenario, no Rule churn.)
Summary:  Ships a deterministic `files × rules` pre-scope pass for code review. `review-scope.mjs` parses
          a diff, applies a conservative review-worthiness gate, and resolves per review-worthy file
          which `security.md` §L1-L6 lenses are in play + which `audit-app.sh` T1/T2 scanner tools are
          relevant, emitting a manifest (JSON for tooling, Markdown for humans) to `test-artifacts/
          review/`. It is a guaranteed FLOOR of what a review must look at — never a ceiling, never a
          blocker (advisory, always exit 0). Consumed by the `/code-review` skill (§5). `review-scope.md`
          documents the two design laws, the review-worthiness gate, and the lens/scanner maps.
          INHERIT-not-REPLACE over `security.md` L1-L6. HARD HOLD — local only.
Dependent files reviewed (propagation checklist):
  [x] Master_Prompt.md             — this Sync Impact Report block; active-version marker → V32.51
  [x] CLAUDE.md (repo)             — Canonical Counts (deliverables 39→41, Scenarios 50→51); intro prose;
                                     deliverable enumeration gains #40 `review-scope.mjs` + #41 `review-scope.md`
  [x] CLAUDE_framework_repo.md     — canonical counts: deliverables 39→41, Scenarios 50→51
  [x] CLAUDE_compact.md            — H1 title version → V32.51
  [x] scenarios.md                 — new SCENARIO 51 (Review Pre-Scope); header scenario-count → 51 (Task 8)
  [x] Framework_Feature_Index.md   — new V32.51 table row; Current-version line + footer → V32.51 (51 scenarios / 41-file)
  [x] README.md                    — shields.io version badge + "Framework Features" heading → V32.51; deliverable count 39→41
  [x] documentation-hub.html · Prompt_References.html (×2) — hero current-version markers → V32.51; deliverable/scenario counts
  [x] deploy.sh                    — ships #40 `review-scope.mjs` (→ scripts/) + #41 `review-scope.md` (→ .ai_prompt/) (Task 7)
  Canonical counts:                51 Scenarios (was 50). 41 deliverable files (was 39). All others
                                   UNCHANGED — 41 Rules · 62 Prompts (39 NEW ✨) · 14 UI Rules ·
                                   6 MCP servers · 18 Phase Hooks · 6 agents · 20 Bootstrap Steps.
═══════════════════════════════════════════════════════════════════════════════════ -->

<!-- ═══════════ SYNC IMPACT REPORT — auto-maintained on every version bump ═══════════
Version:  V32.49 → V32.50
Bump:     MINOR (adopts the "Site Access & Tenancy Bootstrap Standard" — additive throughout. New
          constitutional Rule 41 owning the site-access URL topology; Rule 34 / rbac.md EXTENDED in
          place with the platform-scope RBAC mechanics (never redefined — the 3-tier backbone is
          unchanged). Rules 40→41, Scenarios 49→50, Security Checklist 147→159 items / 21→22 sections.
          All other counts UNCHANGED. Precedent: V32.25 added Rule 34 the same way — new Rule +
          extended on-demand authority file + new Scenario + new Checklist section, no removals.)
Summary:  Locks in the owner-approved `docs/SITE_ACCESS_STANDARD.md` (FRMS, 2026-08-16) as a fleet
          standard: (1) a **site/URL topology** — a `/tm` Tenant Management Site for the platform
          operator (Powerbyte), per-tenant ONE role-routed login (admin-tier → `/{slug}/admin`,
          regular users → `/{slug}/login`), and a demo deployment that is a separate subdomain stack
          with NO `/tm` layer; and (2) a **data-driven platform-role system** — `tenant_manager`
          stays the FIXED default ADMIN ceiling, with seeded `tenant_billing`/`tenant_tech` curated
          roles and room for more platform roles to be created in-frontend, via a `scope`
          discriminator (`tenant` | `platform`) added to the existing PD-005/Rule-34 custom-role
          tables plus a **distinct platform permission vocabulary** (`PlatformPermissionKey` /
          `PlatformRolePermission`) that can never resolve against a tenant `FeatureKey` —
          anti-escalation enforced by a DB CHECK constraint `(scope='tenant' AND tenant_id IS NOT
          NULL) OR (scope='platform' AND tenant_id IS NULL)`. The existing 3-tier tenant backbone
          (`tenant_manager`/`tenant_superadmin`/`tenant_admin`) is UNCHANGED — this is a NEW layer
          alongside it, not a redefinition. `/platform` renames to `/tm` (redirect shim, owner-gated
          cutover) with a reserved-slug list (`tm`, `demo`, `platform`, `admin`, `login`, `api`) so no tenant can
          ever claim a routing collision. HARD HOLD throughout — framework authored globally here;
          per-app adoption (FRMS reference first) is a separate, owner-gated retrofit (Scenario 50).
Dependent files reviewed (propagation checklist):
  [x] Master_Prompt.md             — this Sync Impact Report block; new Rule 41; Rule 34 one-liner
                                     cross-ref to Rule 41; active-version marker → V32.50
  [x] CLAUDE_compact.md            — H1 title + STRICTEST discipline marker → V32.50; new Rule 41
                                     one-liner; rules-count 40→41; rbac.md loading-table row extended
  [x] .ai_prompt/rbac.md           — new Part E (platform-scope roles) + Part F (site-access URL
                                     topology, incl. F2a optional public per-tenant landing); Framework
                                     inheritance section cross-refs Rule 41
  [x] scenarios.md                 — new SCENARIO 50 (Existing-App Site-Access & /tm Bootstrap
                                     Retrofit); header prompt-range/scenario-count line → 50
  [x] phases.md                    — Phase 4 Part 3 MODEL HOOK (seed platform roles); Phase 4 Part 3
                                     RBAC MODEL HOOK extended (scope enforcement + platformRole
                                     router); Phase 4 Parts 5-6 GATE-CLOSURE route-topology item;
                                     Phase 3.3 URL-topology cue MODEL HOOK — all MODEL HOOKs, Phase
                                     Hooks count stays 18
  [x] Security_Checklist.md        — new SECTION 22 "Platform Site-Access" (12 items); header total
                                     147→159 items / 21→22 sections; Phase-7 relevant-sections list
                                     gains a Section 22 trigger row
  [x] templates.md                 — 7F seed template extended with 7G platform-role (BILLING/TECH)
                                     seed accounts; demo tenant_admin slot added per the standard
  [x] LESSONS_REGISTRY.md          — new entry `framework.site-access.tm-platform-roles-and-role-
                                     routed-urls` (scope: framework)
  [x] Framework_Feature_Index.md   — new V32.50 table row; Current-version line + footer → V32.50
  [x] README.md                    — shields.io version badge + "Framework Features" heading → V32.50
  [x] documentation-hub.html · Prompt_References.html (×2) — hero current-version markers → V32.50
  [x] CLAUDE.md (repo) · CLAUDE_framework_repo.md — canonical counts: Rules 40→41, Scenarios 49→50,
                                     Security Checklist 147→159/21→22
  Canonical counts:                41 Rules (was 40). 50 Scenarios (was 49). Security Checklist
                                   147 → 159 items / 21 → 22 sections. All others UNCHANGED — 62
                                   Prompts (39 NEW ✨) · 39 deliverable files · 14 UI Rules ·
                                   6 MCP servers · 18 Phase Hooks · 6 agents · 20 Bootstrap Steps.
═══════════════════════════════════════════════════════════════════════════════════ -->

<!-- ═══════════ SYNC IMPACT REPORT — auto-maintained on every version bump ═══════════
Version:  V32.48.2 → V32.49
Bump:     MINOR (adds a fleet UI standard + a new user-global MCP; MCP servers 5→6. Rules 40 · Scenarios
          49 · Prompts 62 · deliverables 39 · UI Rules 14 — all UNCHANGED. Precedent: V32.11 added the
          shadcn/studio Pro MCP as a MINOR, 4→5.)
Summary:  IconStack (iconstack.io) becomes the framework's DEFAULT icon-SOURCING layer. It aggregates
          51,378 MIT-licensed icons across 21 libraries (lucide, tabler, phosphor, material, remix, …)
          behind a free no-auth API + a hosted MCP server (tools: search_icons · get_icon_svg ·
          list_libraries). `lucide-react` STAYS the shadcn RUNTIME baseline (shadcn components import it);
          IconStack is where the agent finds/pulls anything BEYOND lucide, materialized as a self-hosted
          SVG in `components/icons/` — NO runtime IconStack dependency (CSP + offline safe). INHERIT-not-
          REPLACE. License: the API/MCP returns MIT icons only, attribution not required. Rollout to
          already-deployed apps is a per-seat broadcast (never from the AIEF seat).
Dependent files reviewed (propagation checklist):
  [x] ui-rules.md                  — Rule 9 rewritten: lucide runtime baseline + IconStack default sourcing layer
  [x] AI_Tools_Reference.md        — new §2.6 IconStack MCP (config + tools + endpoints); icon table row; MCP 5→6
  [x] Master_Prompt.md             — this Sync Impact Report block → V32.49
  [x] Framework_Feature_Index.md   — Current-version line + footer → V32.49 (+ V32.49 descriptor)
  [x] CLAUDE_compact.md            — H1 title + STRICTEST discipline marker → V32.49
  [x] README.md                    — shields.io version badge → V32.49
  [x] documentation-hub.html · Prompt_References.html (×2) — hero current-version markers → V32.49
  [x] CLAUDE.md (repo) · CLAUDE_framework_repo.md — canonical counts: MCP servers 5→6 + enumeration
  [x] ~/.claude/rules/design-defaults.md (global) — new Icon-source entry (fleet-wide, all projects)
  Canonical counts:                MCP servers 5 → 6. All others UNCHANGED — 40 Rules · 49 Scenarios ·
                                   62 Prompts (39 NEW ✨) · 39 deliverable files · 14 UI Rules · 147 Checklist/21
═══════════════════════════════════════════════════════════════════════════════════ -->

<!-- ═══════════ SYNC IMPACT REPORT — auto-maintained on every version bump ═══════════
Version:  V32.48.1 → V32.48.2
Bump:     PATCH (completes the V32.48.1 behavioral bugfix with a second deploy-wiring gitignore entry; NO
          new artifact, NO count change. Rules 40 · Scenarios 49 · Prompts 62 · deliverables 39 — all UNCHANGED.)
Summary:  V32.48.1 gitignored only the DISTRIBUTED AdminCN slice copy (`starter/admincn/`) and was silent on
          the STAGING copy at `.ai_prompt/starter/admincn/`, which `sync-to-project.sh` seeds
          (`AI_PROMPT_DIRS=(starter/admincn)`) and `deploy.sh` Group 16 reads from. Both copies are
          re-materialized every sync from the 222-file source slice, so committing EITHER duplicates the
          app's own `components/ui` — the `starter/admincn/`-only rule left the `.ai_prompt/` copy tracked
          fleet-wide (surfaced again by the FerryBook seat, still carrying 222 tracked files at the staging
          path). Fix: `deploy.sh` `GITIGNORE_ENTRIES` now adds `.ai_prompt/starter/admincn/` alongside
          `starter/admincn/` (a mid-slash pattern is root-anchored and cannot cover the `.ai_prompt/` path,
          so a second entry is required). Per-seat cleanup now untracks BOTH paths
          (`git rm -r --cached starter/admincn .ai_prompt/starter/admincn`); the untrack broadcast to the 4
          apps named only one path, so the remaining seats + a FRMS re-check use the both-paths command.
          **HARD HOLD** — local commits only.
Dependent files reviewed (propagation checklist):
  [x] deploy.sh                    — GITIGNORE_ENTRIES adds `.ai_prompt/starter/admincn/` (the fix)
  [x] admincn-starter.md (#39)     — cleanup note untracks BOTH paths + two-copy explanation
  [x] Framework_Feature_Index.md   — Current-version line + footer → V32.48.2 (+ V32.48.2 descriptor)
  [x] Master_Prompt.md             — this Sync Impact Report block → V32.48.2
  [x] CLAUDE_compact.md            — H1 title + STRICTEST discipline marker → V32.48.2
  [x] README.md                    — shields.io version badge → V32.48.2
  [x] documentation-hub.html · Prompt_References.html (×2) — hero current-version markers → V32.48.2
  Canonical counts:                UNCHANGED — 40 Rules · 49 Scenarios · 62 Prompts (39 NEW ✨) ·
                                   39 deliverable files · 14 UI Rules · 147 Checklist items/21 sections
═══════════════════════════════════════════════════════════════════════════════════ -->

<!-- ═══════════ SYNC IMPACT REPORT — auto-maintained on every version bump ═══════════
Version:  V32.48 → V32.48.1
Bump:     PATCH (behavioral bugfix — deploy-wiring fix to deliverable #39; NO new artifact, NO count
          change. Per the ratified bump-type policy a non-additive behavioral BUGFIX is a PATCH; matches
          the V32.33.1 precedent (design-stop-hook block-scalar false-positive fix → PATCH). Rules 40 ·
          Scenarios 49 · Prompts 62 · deliverables 39 — all UNCHANGED.)
Summary:  Fixes the AdminCN starter-slice DUPLICATION footgun. The vendored `starter/admincn/` slice
          (deliverable #39) ships a full 50-component `components/ui/` set and `deploy.sh` did NOT
          gitignore it, so an already-shadcn app that synced the framework COMMITTED a duplicate copy of
          ~37 stock shadcn components (verified live on FerryBook: 222 slice files tracked, 37/50 ui
          components collided with its own `src/components/ui/`). Root cause is 100% framework-side — both
          the slice contents and the deploy wiring live in AIEF, which is why the FerryBook seat bounced it
          here. Fix: `deploy.sh` `GITIGNORE_ENTRIES` now adds `starter/admincn/` → the slice becomes a
          local cherry-pick REFERENCE (still re-copied each sync so it is always present to cherry-pick
          from, never committed), matching Scenario 49's "never duplicate a component the app already has"
          + the fact that the slice is non-compiling reference material. Apps that already committed it run
          `git rm -r --cached starter/admincn` on their OWN seat (per-seat, never from AIEF). **HARD HOLD**
          — local commits only.
Dependent files reviewed (propagation checklist):
  [x] deploy.sh                    — GITIGNORE_ENTRIES adds `starter/admincn/` (the fix)
  [x] admincn-starter.md (#39)     — documents the gitignored-reference model + per-seat `git rm --cached` cleanup
  [x] Framework_Feature_Index.md   — NEW V32.48.1 row + footer aggregate note + Current-version line + footer → V32.48.1
  [x] Master_Prompt.md             — this Sync Impact Report block → V32.48.1
  [x] CLAUDE_compact.md            — H1 title + STRICTEST discipline marker → V32.48.1
  [x] README.md                    — shields.io version badge → V32.48.1
  [x] documentation-hub.html · Prompt_References.html (×2) — hero current-version markers → V32.48.1
  Canonical counts:                UNCHANGED — 40 Rules · 49 Scenarios · 62 Prompts (39 NEW ✨) ·
                                   39 deliverable files · 14 UI Rules · 147 Checklist items/21 sections ·
                                   18 Phase Hooks · 6 agents · 5 MCP servers · 20 Bootstrap Steps.
Ratified: 2026-08-10
─────────────────────────────────────────────────────────────────────────────────────
Version:  V32.46 → V32.47
Bump:     MINOR (additive: 1 NEW constitutional Rule 40 — Invisible-Quality Radar; Rules 39→40. NO new
          Scenario/Prompt/deliverable — the mirror rides EXISTING files. Scenarios 49 · Prompts 62 ·
          deliverables 39 — all UNCHANGED.)
Summary:  STAGED framework mirror of the global Invisible-Quality Radar (`~/.claude/library/invisible-quality-radar.md`).
          Keeps the deep-code, non-functional quality dimensions (D1 security · D2 SEO · D3 a11y · D4 privacy ·
          D5 audit-ability · D6 perf-budget · D7 observability · D8 ISO-conformance · D9 reliability ·
          D10 maintainability) in active consideration as an app is built. New constitutional Rule 40 = an
          ALWAYS-ON, weight-gated INDEX/POINTER (trivial tasks skip) — NOT a new enforcer, NOT a new blocking
          gate; INHERIT-not-REPLACE over the existing owner rules (security.md L1-L6 · seo.md Rule 35 +§GEO ·
          ui-rules R13 WCAG · privacy.md Rule 33 · audit.md Rule 38). Which dims are in-scope for an app is read
          from a NEW capability-primer FLAGS field `INVISIBLE_DIMS_IN_SCOPE:` (build-primer.sh #38). Adds a
          Phase-7 done-claim CONSIDERATION line + an advisory spec-gap-check.sh CLASS 6 (declared-in-scope dim
          with no evidence built). D6/D7/D8 = NAMED-but-unowned (no new authority file). ISO/IEC 25010 = the
          vocabulary spine; conformance-AWARE / certification-READY, NO ISMS. **Zero-disruption** — nothing new
          auto-blocks a build. **HARD HOLD** — local commits only.
Dependent files reviewed (propagation checklist):
  [x] Master_Prompt.md             — this Sync Impact Report block; new Rule 40; V32.47 changelog entry; active-version marker → V32.47; Current totals 39→40 Rules
  [x] CLAUDE_compact.md            — H1 title → V32.47; Rule 40 one-liner; rules-count 39→40; STRICTEST marker → V32.47
  [x] Framework_Feature_Index.md   — new V32.47 row; Current framework version + footer → V32.47 · 40 rules
  [x] root CLAUDE.md               — Canonical Counts 39→40 Rules (+ prose mentions)
  [x] scripts/build-primer.sh      — new `INVISIBLE_DIMS_IN_SCOPE` FLAGS field (stub + render); ~/.claude/templates/capability-primer.md documents it
  [x] scripts/spec-gap-check.sh    — new advisory CLASS 6 (in-scope invisible dim, no evidence built)
  [x] phases.md                    — Phase 7 output-contract advisory done-claim consideration line (Rule 40)
  [x] public/documentation-hub.html · README.md · scenarios.md · Prompt_References.html (×2) — 39→40 rules / current-version markers → V32.47
  Canonical counts:                CHANGED — Rules 39→40. UNCHANGED — 49 Scenarios · 62 Prompts (39 NEW ✨) ·
                                   39 deliverable files · 14 UI Rules · 147 Checklist items/21 sections ·
                                   18 Phase Hooks · 6 agents · 5 MCP servers · 20 Bootstrap Steps.
Ratified: 2026-08-08
─────────────────────────────────────────────────────────────────────────────────────
...
Bump:     MINOR (extends the existing constitutional Rule 35 "SEO Foundation (Adaptive Baseline)"
          ...
          content/design SEO pass. No new Rule/Scenario/Prompt/deliverable is added — this is an
          EXTENSION of Rule 35's reach and Scenario 44's procedure, not a new numbered item. No
          Rule/Phase/security-level removed or redefined; counts unchanged)
Summary:  SEO-aware design & content (owner-set 2026-07-19/20) — V32.30 wired SEO only at Phase 4
          (scaffold) + Phase 5 (validation gate), the TECHNICAL foundation (metadata/sitemap/
          robots/JSON-LD/Lighthouse). "Google-friendly" for a marketing/landing surface is a
          design + content-time concern that must start at the mockup/content stage, not be
          bolted on after. V32.31 makes SEO an early design/content concern: the Planning
          Assistant's DESIGN.md/MOCKUP.jsx step, Phase 2.8 (mockup), and Phase 3.3 (prototype) now
          plan SEO-informed content structure (single H1, heading hierarchy, keyword-informed
          copy, alt text, CWV-aware layout) for public-facing surfaces, so the designed frontend
          is Google-friendly BY DESIGN, not retrofitted.
Dependent files reviewed (propagation checklist — the machine-readable Cross-Alignment map):
  [x] Master_Prompt.md             — Sync Impact Report V32.30→V32.31 (this block); Rule 35
                                     extended with a design/content-time paragraph; V32.31
                                     changelog entry; version markers bumped
  [x] seo.md (.ai_prompt)          — NEW "Design & content-time SEO" section (content/copy,
                                     structure/semantics, performance-as-design/CWV checklist,
                                     where-it-lands pointer to DESIGN.md/MOCKUP.jsx); §7 Retrofit
                                     note on the content/design SEO pass
  [x] Planning_Assistant.md        — DESIGN.md/MOCKUP.jsx generation step (Step 7) now drafts
                                     SEO-aware content structure for public-facing surfaces
  [x] scenarios.md                 — Scenario 44 extended with a content/design SEO retrofit pass
  [ ] phases.md                    — Phase 2.8 / Phase 3.3 MODEL HOOK pointer to seo.md's design/
                                     content section (owned by a companion worker — verify before close)
  [ ] CLAUDE_compact.md            — version marker bump only (owned by a companion worker)
  [ ] Framework_Feature_Index.md   — V32.31 row + footer version (owned by a companion worker)
  [ ] CLAUDE.md (project)          — version references, if any (owned by a companion worker)
  Canonical counts:                35 Rules · 44 Scenarios · 62 Prompts (39 NEW ✨) · 14 UI Rules ·
                                   31 deliverables · 147 Checklist/21 sections · 18 hooks — ALL
                                   UNCHANGED from V32.30 (this is a design/content-reach extension
                                   of Rule 35 + Scenario 44, not a new numbered item)
Ratified: 2026-07-20
─────────────────────────────────────────────────────────────────────────────────────
Version:  V32.29 → V32.30
Bump:     MINOR (additive: new constitutional Rule 35 "SEO Foundation (Adaptive Baseline)" + new
          always-on deliverable #31 `.ai_prompt/seo.md` + Scenario 44 existing-app retrofit + a
          Phase 4 scaffold step + a Phase 5 validation gate. No Rule/Phase/security-level removed
          or redefined; SEO becomes constitutional/always-on like security.md — NOT a conditional
          PRODUCT.md-gated deliverable like V32.28's notifications.md — but nothing prior is broken)
Summary:  SEO Foundation (owner-set 2026-07-19) — every framework-built app is scaffolded with an
          adaptive SEO baseline automatically, with NO human planning step and NO new PRODUCT.md
          section. Public routes get full SEO (indexable, canonical, Open Graph/Twitter, JSON-LD,
          sitemap inclusion); private/authed/internal routes get a private baseline (`noindex,
          nofollow` + `robots.ts` disallow + sitemap exclusion). Built entirely on Next.js App
          Router native primitives (`metadata` export / `generateMetadata`, `app/sitemap.ts`,
          `app/robots.ts`, `alternates.canonical` + `alternates.languages` hreflang, Open Graph +
          Twitter card fields, dynamic `opengraph-image`/`ImageResponse` as an opt-in). Config
          values (site name, base URL, default OG image, locales) are sourced from the EXISTING
          PRODUCT.md App Identity section + env vars — never a new interview. Enforced at Phase 4
          (scaffold) + Phase 5 (validation gate, Lighthouse SEO ≥ 90 hard-gated for public apps).
Dependent files reviewed (propagation checklist — the machine-readable Cross-Alignment map):
  [x] Master_Prompt.md             — Sync Impact Report V32.29→V32.30 (this block); new Rule 35
                                     inserted after Rule 34; V32.30 changelog entry; version
                                     markers + canonical counts bumped
  [x] seo.md (.ai_prompt)          — NEW deliverable #31 — the on-demand SEO Foundation authority
                                     (adaptive public/private baseline, Next.js App Router
                                     primitives checklist, config sourcing, JSON-LD types, Phase 4
                                     scaffold checklist, Phase 5 validation gate, retrofit pointer)
  [ ] CLAUDE_compact.md            — version marker + Rule 35 one-liner + deliverable pointer
                                     (owned by a companion worker — verify before close)
  [ ] phases.md                    — Phase 4 scaffold MODEL HOOK (read seo.md, emit metadata/
                                     sitemap.ts/robots.ts/JSON-LD) + Phase 5 SEO validation gate
                                     (owned by a companion worker — verify before close)
  [ ] scenarios.md                 — Scenario 44 (Existing-App SEO Foundation Retrofit)
                                     (owned by a companion worker — verify before close)
  [ ] ui-rules.md                  — pointer to seo.md for OG-image / structured-data UI concerns
                                     (owned by a companion worker — verify before close)
  [ ] deploy.sh                    — `.ai_prompt/seo.md` added to the copy set; count 30→31
                                     (owned by a companion worker — verify before close)
  [ ] Framework_Feature_Index.md   — V32.30 row + footer version + counts
                                     (owned by a companion worker — verify before close)
  [ ] CLAUDE.md (project)          — canonical-counts block + deliverable enumeration (#31 seo.md)
                                     (owned by a companion worker — verify before close)
  [ ] public/documentation-hub.html — sd-counts callout + deliverable list (30→31)
                                     (owned by a companion worker — verify before close)
  Canonical counts:                35 Rules · 44 Scenarios · 62 Prompts (39 NEW ✨) · 14 UI Rules ·
                                   31 deliverables · 147 Checklist/21 sections · 18 hooks (all other
                                   counts unchanged: 20 Bootstrap Steps, 9 V32 Dispatch Rules,
                                   6 memory-governance sections, 12 PRODUCT.md sections,
                                   Planning Assistant 12 rules, 5 MCP servers)
Ratified: 2026-07-19
─────────────────────────────────────────────────────────────────────────────────────
Version:  V32.28 → V32.29
Bump:     MINOR (additive: lands the SCAFFOLD-CODE port deferred by V32.27 — the framework scaffold now EMITS the Telegram dual-backend storage layer instead of shipping MinIO-only. No Rule/Phase/security-level removed or redefined; no new copied-file deliverable — the code is agent-generated from templates.md prose, matching the framework's existing scaffold convention; counts unchanged)
Summary:  Telegram storage SCAFFOLD-CODE port (owner-un-gated 2026-07-17, executed 2026-07-18) — resolves
          the "⚠ SCAFFOLD-CODE PENDING" caveat carried by V32.27. Previously templates.md documented the
          Telegram-default DECISION but a newly-scaffolded app still shipped MinIO-only and had to adopt the
          Telegram adapter from FRMS by hand. This bump ports the FRMS-proven code path into the scaffold
          PROSE so Phase 4 now generates it for every new app: the `packages/${app_slug}/storage`
          dual-backend `StorageAdapter` (S3/MinIO **and** Telegram, selected by `STORAGE_BACKEND`), the
          `MediaObject` ledger Prisma model + `tenants.telegram_channel_id` column + additive migration,
          the `/api/media` proxy route + `resolveMediaBytes` dual-read helper (Telegram-first, MinIO
          fallback), the upload-router Telegram branch that writes the ledger row, and per-env
          `STORAGE_BACKEND`/`TELEGRAM_*` wiring (dev+staging+prod=Telegram — dev on its OWN dedicated
          channel; demo=MinIO; MinIO retained everywhere as the temp/index/scratch fallback). Host
          primitives the app must provide are named, not fabricated (platformPrisma/ctx.db,
          requireRouteAuth, rateLimiters mediaDownload+upload tiers, an AuditLog model). Creds only in
          Server-Setups (SOPS+age); gov/LGU PII = owner-accepted temp measure (privacy.md Rule 33).
          Prose-scaffold shape chosen (not copied template files) because the framework has no template/
          stub dirs and the deploy whitelist ships only .md/.sh/.json — app code is always agent-generated.
Dependent files reviewed (propagation checklist — the machine-readable Cross-Alignment map):
  [x] templates.md                 — Rule 5 storage section: the "⚠ SCAFFOLD-CODE PENDING" caveat REPLACED with the full canonical scaffold code (packages/storage dual-backend adapter + telegram helpers, MediaObject ledger model + migration, /api/media proxy + resolveMediaBytes, upload-router Telegram branch, per-env STORAGE_BACKEND/TELEGRAM_* wiring). Decision-prose + per-env matrix + R2/NATS opt-in blocks + creds/PII caveats preserved
  [x] phases.md                    — Phase 4 Parts 3/4/5 MODEL HOOK pointers: emit MediaObject ledger (canonical model per templates.md), packages/storage dual-backend StorageAdapter, /api/media route + resolveMediaBytes + upload Telegram branch, env wiring (no Part renumber, no new phase hook)
  [x] Master_Prompt.md             — Sync Impact Report V32.28→V32.29 (this block); changelog + version markers bumped
  [x] CLAUDE_compact.md            — version marker bumped V32.28→V32.29 + one-line changelog pointer
  [x] Framework_Feature_Index.md   — V32.29 row + footer version bump; counts unchanged
  [ ] deploy.sh / sync-to-project.sh — n/a (prose scaffold; no new copied-file deliverable, so no whitelist/GROUP/count change)
  [ ] privacy.md / security.md / Security_Checklist.md — n/a (existing Rule 33 gov/LGU + L5 AuditLog coverage already applies)
  [ ] scenarios.md / Prompt_References / bootstrap.md / ui-rules.md — n/a (no scenario/prompt/bootstrap/UI-rule change)
  [ ] CLAUDE.md (project) / public/documentation-hub.html — n/a (no count change)
  Canonical counts:                ALL UNCHANGED — 34 Rules · 43 Scenarios · 62 Prompts (39 NEW ✨) ·
                                   14 UI Rules · 30 deliverables · 147 Checklist/21 sections · 18 hooks
Ratified: 2026-07-18
─────────────────────────────────────────────────────────────────────────────────────
Version:  V32.27 → V32.28
Bump:     MINOR (additive: new CONDITIONAL on-demand deliverable #30 `.ai_prompt/notifications.md` + Scenario 43; NO new Rule; nothing removed/redefined; loaded ONLY when PRODUCT.md declares multi-channel notification/event-delivery needs; default locked stack unchanged)
Summary:  Event Delivery & Notifications (owner-set 2026-07-16) — a right-sized, FOSS multi-channel
          event-delivery / notification pattern for the fleet, WITHOUT Kafka's operational tax. Tier 1
          DEFAULT = Valkey Streams (durable append-only log, consumer groups, XACK/PEL at-least-once) +
          BullMQ (routing/retry/DLQ/rate-limit) — both already in the locked stack, zero new infra.
          Tier 2 GRADUATION (opt-in, documented threshold) = NATS JetStream (Apache-2.0, single binary).
          6 mandatory additions the reference diagram omits: event schema+versioning, tenant isolation,
          notification preferences, idempotency-at-ingestion, per-provider rate limits, PII/compliance
          routing (privacy.md Rule 33). CONDITIONAL: an app with no notification need never loads it.
Dependent files reviewed (propagation checklist):
  [x] notifications.md (.ai_prompt)  — NEW deliverable #30 (event-delivery/notification pattern)
  [x] scenarios.md                   — Scenario 43 (Add multi-channel event delivery / notifications)
  [x] phases.md                      — Phase 4 conditional MODEL HOOK (read notifications.md on the flag)
  [x] templates.md                   — "OPT-IN: NATS JetStream" (Tier-2 graduation) compose + env
  [x] deploy.sh                      — .ai_prompt/notifications.md added to the copy set; count 29→30
  [x] Master_Prompt.md               — Sync Impact Report V32.27→V32.28 (this block) + changelog + markers
  [x] CLAUDE_compact.md              — deliverable pointer + counts (42→43 Scenarios, 29→30 deliverables)
  [x] Framework_Feature_Index.md     — V32.28 row + footer version + counts
  [x] CLAUDE.md (project)            — canonical-counts block + deliverable enumeration (#30 notifications.md)
  [x] public/documentation-hub.html  — sd-counts callout + deliverable list (29→30)
  [ ] security.md / Security_Checklist.md — n/a (L5 AuditLog already covers audit; no new checklist item)
  [ ] Prompt_References / bootstrap.md / ui-rules.md — n/a (no prompt/scaffold/UI-rule change)
  Canonical counts:                  Scenarios 42→43 · deliverables 29→30 · Rules 34 · Prompts 62 ·
                                     UI Rules 14 · Bootstrap 20 · Security Checklist 147/21 · Phase Hooks 18
Ratified: 2026-07-16
─────────────────────────────────────────────────────────────────────────────────────
Version:  V32.26 → V32.27
Bump:     MINOR (redefines the storage DEFAULT non-breakingly — Telegram becomes the default backend for persistent media on dev + staging + prod; demo stays MinIO; MinIO retained everywhere as the temp/index/scratch fallback; R2 demoted from V32.16's sole opt-in to a further S3 opt-in beneath the Telegram default. No Rule/Phase/security-level removed or redefined; templates.md only for the doc; the scaffold-code deliverable is deferred — see caveat)
Summary:  Storage-default reconciliation (owner-set 2026-07-16) — supersedes V32.16 ("MinIO stays the
          dev + staging/prod default") fleet-wide. Telegram is now the DEFAULT for all persistent media
          (images/files/docs) on dev + staging + prod; the file bytes are free on Telegram. Per-env:
          dev = Telegram on its OWN dedicated private channel (same bot, separate channel — dummy dev
          uploads must never pollute the shared staging/prod channel); staging + prod = Telegram on the
          shared channel (so prod→staging data-first refreshes render photos); demo = the ONE env that
          stays on MinIO. Mechanism = `STORAGE_BACKEND` env (`telegram|s3|minio`) selecting the
          `packages/storage/` StorageAdapter + a `media_objects` ledger + `/api/media` proxy; creds only
          in Server-Setups (SOPS+age), never in-repo; gov/LGU PII = owner-accepted temp measure until
          funded storage (privacy.md Rule 33). Global rule of record + first proof (FRMS, all 4 envs):
          ~/.claude/rules/media-storage-default.md.
          ⚠ SCAFFOLD-CODE PENDING: the Telegram code path (StorageAdapter S3+Telegram, /api/media route,
          media_objects migration, env wiring) is proven in FRMS but NOT yet ported into the framework
          scaffold deliverables — a newly-scaffolded app still ships MinIO-only until that owner-gated
          port lands. This bump records the DEFAULT DECISION (doc); the deliverable port is a follow-up.
Dependent files reviewed (propagation checklist — the machine-readable Cross-Alignment map):
  [x] templates.md                 — Rule 5 storage section: new "DEFAULT: Telegram for persistent media" subsection (per-env matrix + STORAGE_BACKEND mechanism + pending-deliverable caveat); R2 section reframed as a further opt-in beneath the Telegram default; compose-file comment updated
  [x] Master_Prompt.md             — Sync Impact Report V32.26→V32.27 (this block); changelog + version markers bumped
  [x] CLAUDE_compact.md            — version marker bumped V32.26→V32.27 + one-line changelog pointer added
  [x] Framework_Feature_Index.md   — V32.27 row + footer version bump; counts unchanged
  [ ] phases.md                    — n/a (no phase-hook/gate change; scaffold-code port is the deferred deliverable, not a phase edit yet)
  [ ] privacy.md                   — n/a (existing Rule 33 gov/LGU coverage already applies to a Telegram media backend)
  [ ] scenarios.md / Prompt_References — n/a (no scenario/prompt change)
  [ ] Security_Checklist.md         — n/a (no checklist item change)
  [ ] public/documentation-hub.html — n/a (no count change)
  Canonical counts:                ALL UNCHANGED — 34 Rules · 42 Scenarios · 62 Prompts (39 NEW ✨) ·
                                    14 UI Rules · 29 deliverables · 147 Checklist/21 sections · 18 hooks
Ratified: 2026-07-16
─────────────────────────────────────────────────────────────────────────────────────
Version:  V32.25 → V32.26
Bump:     MINOR (additive: new scaffold deliverable requirement — every app-shell sidebar footer must render an app version tag + a "Developed by Powerbyte IT Solutions" white-label link, per global design-defaults.md Entry 3; wired as an output-contract item + Phase 4 Parts 5-6 gate-closure item, NOT a new numbered UI rule; UI rules stay 14, no Rule/Scenario/Prompt/deliverable count added)
Summary:  Fleet-wide UI convention (owner-set 2026-07-10) — every functional Powerbyte app's shadcn/ui
          `SidebarFooter`, under the profile/user block, carries two small muted marks: (1) an app
          **version tag** `v{X.Y.Z}` sourced from the release version/package.json per the versioning
          standard (`~/.claude/rules/versioning-standard.md` — staging shows `-rc.N`, prod clean), and
          (2) a **white-label credit** — the whole "Developed by Powerbyte IT Solutions" label is ONE
          clickable link opening a NEW TAB to `https://www.powerbyteitsolutions.com/`
          (`target="_blank" rel="noopener noreferrer"`). Non-sidebar archetypes (marketing full-width,
          immersive/kiosk) place the marks per the same global rule's fallback slots. INHERIT-not-REPLACE
          (docs/DESIGN.md may relocate/restyle). Mirrored to the global surface
          ~/.claude/rules/design-defaults.md Entry 3.
Dependent files reviewed (propagation checklist — the machine-readable Cross-Alignment map):
  [x] Master_Prompt.md             — UI/scaffold output-contract requirement added; Sync Impact Report + changelog + Current totals/version markers bumped V32.25→V32.26
  [x] phases.md                    — UI_LAYOUT app-shell step note + Phase 4 Parts 5-6 GATE-CLOSURE checklist item + Notes pointer to design-defaults.md Entry 3
  [x] ui-rules.md                  — reference NOTE pointing to design-defaults.md Entry 3 (no new numbered rule; UI rules stay 14)
  [x] CLAUDE_compact.md            — version marker bumped; one-line pointer added
  [x] Framework_Feature_Index.md   — V32.26 row + footer version bump; counts unchanged
  [ ] bootstrap.md                 — n/a (no clean scaffold-step anchor found; covered by phases.md gate instead — see V32.26 note)
  [ ] Prompt_References.md/.html    — n/a (no prompt change)
  [ ] scenarios.md                 — n/a (no scenario change)
  [ ] Security_Checklist.md         — n/a (no security-checklist item change)
  [ ] public/documentation-hub.html — n/a (no count change)
  Canonical counts:                ALL UNCHANGED — 34 Rules · 42 Scenarios · 62 Prompts (39 NEW ✨) ·
                                    14 UI Rules · 29 deliverables · 147 Checklist/21 sections · 18 hooks
Ratified: 2026-07-10
─────────────────────────────────────────────────────────────────────────────────────
Version:  V32.24.1 → V32.24.2
Bump:     PATCH (non-behavioral guidance enrichment: adds a prescriptive LAYOUT ARCHETYPE default to the existing design-principles.md deliverable + a pointer in ui-rules.md; NO new Rule/Scenario/Prompt/gate/phase-hook/deliverable; zero count change)
Summary:  Global basic-design default (owner-set 2026-07-10) — pick the page shell by what the
          app IS: a MARKETING / brand website or webapp = FULL-WIDTH (full-bleed immersive hero
          bands); a GENERAL-PURPOSE / purpose-driven app (dashboard, admin, tool, back-office,
          console) = PERSISTENT LEFT-SIDEBAR APP-SHELL (always-available left nav collapsible to
          an off-canvas sheet on mobile / an icon-rail for focused-work consoles + large right
          content area). Do NOT scatter primary nav across a top header-tab strip for a
          multi-section functional app. Framework build = shadcn/ui `sidebar` (`SidebarProvider`
          + `Sidebar` + `SidebarTrigger` + `SidebarInset`); inset content still uses the Entry-1
          container/gutter. Exempt (immersive, no app-shell): kiosk / wall-display / public
          marketing-landing-auth. INHERIT-not-REPLACE (docs/DESIGN.md / PRODUCT.md override per
          surface). Mirrored to the global surface ~/.claude/rules/design-defaults.md Entry 2.
Dependent files reviewed (propagation checklist — the machine-readable Cross-Alignment map):
  [x] design-principles.md         — Pillar 3 "Layout archetype" default added (marketing=full-width · app=left-sidebar shell)
  [x] ui-rules.md                  — Rule 8 LAYOUT ARCHETYPE DEFAULT pointer added (no new numbered rule)
  [x] Framework_Feature_Index.md   — V32.24.2 timeline row + footer version bump; counts unchanged
  [ ] Prompt_References.md/.html    — n/a (no prompt change)
  [ ] scenarios.md                 — n/a (no scenario change)
  [ ] phases.md                    — n/a (no phase-hook/gate change; no advisory lint added)
  [ ] CLAUDE_compact.md            — n/a (PATCH guidance enrichment of an on-demand file; no count/rule marker to bump)
  [ ] public/documentation-hub.html — n/a (no count change)
  Canonical counts:                ALL UNCHANGED — 33 Rules · 41 Scenarios · 62 Prompts (39 NEW ✨) ·
                                    14 UI Rules · 28 deliverables · 135 Checklist/20 · 18 hooks
Ratified: 2026-07-10
─────────────────────────────────────────────────────────────────────────────────────
Version:  V32.24 → V32.24.1
Bump:     PATCH (non-behavioral guidance enrichment: adds a prescriptive content-container / responsive-gutter DEFAULT to the existing design-principles.md deliverable + a pointer in ui-rules.md; NO new Rule/Scenario/Prompt/gate/phase-hook/deliverable; zero count change)
Summary:  Global basic-design default (owner-set 2026-07-10) — readable/dense page content
          (admin, dashboards, forms, tables, settings, reading columns) must NOT be full-bleed:
          wrap it in a centered max-width container (`mx-auto max-w-7xl`, ~1280px cap) WITH a
          mobile-first safe responsive gutter that is never 0 on any breakpoint
          (`px-4 sm:px-6 lg:px-8`). Immersive surfaces (kiosk / wall-display / full-bleed
          hero-marketing / maps-canvas) stay edge-to-edge. INHERIT-not-REPLACE (docs/DESIGN.md
          measure wins). Mirrored to the global surface ~/.claude/rules/design-defaults.md Entry 1.
Dependent files reviewed (propagation checklist — the machine-readable Cross-Alignment map):
  [x] design-principles.md         — Pillar 3 "Content container & responsive gutter" default added
  [x] ui-rules.md                  — Rule 8 CONTENT CONTAINER DEFAULT pointer added (no new numbered rule)
  [x] Framework_Feature_Index.md   — V32.24.1 timeline row + footer version bump; counts unchanged
  [ ] Prompt_References.md/.html    — n/a (no prompt change)
  [ ] scenarios.md                 — n/a (no scenario change)
  [ ] phases.md                    — n/a (no phase-hook/gate change; advisory lint deliberately NOT added — see FFI note)
  [ ] CLAUDE_compact.md            — n/a (PATCH guidance enrichment of an on-demand file; no count/rule marker to bump)
  [ ] public/documentation-hub.html — n/a (no count change)
  Canonical counts:                ALL UNCHANGED — 33 Rules · 41 Scenarios · 62 Prompts (39 NEW ✨) ·
                                    14 UI Rules · 28 deliverables · 135 Checklist/20 · 18 hooks
Ratified: 2026-07-10
─────────────────────────────────────────────────────────────────────────────────────
Version:  V32.23 → V32.24
Bump:     MINOR (additive: new Prompt 3.24 Spec Expert Panel + Scenario 41 + Phase 2.8/3 panel gate; no Rule/Phase/security-level removed or redefined)
Summary:  A2 (SuperClaude harvest) — Spec Expert Panel: 5 parallel expert-lens Sonnet subagents
          (secure-code-guardian / architecture-designer / api-designer / test-master /
          database-optimizer) review docs/PRODUCT.md at Phase 2.8 + Phase 3 pre-lock; PM
          synthesizes + dedups + feeds findings into the Flow-Back/LIVING-SPEC reconcile;
          CRITICAL findings gate Phase 2.8 handoff and Phase 3 spec-lock.
─────────────────────────────────────────────────────────────────────────────────────
Version:  V32.24 → V32.25
Bump:     MINOR (additive: new Rule 34 Tenant-RBAC Standard + Scenario 42 + deliverable #29 .ai_prompt/rbac.md + Security_Checklist SECTION 21; no Rule/Phase/security-level removed or redefined)
Summary:  Tenant-RBAC fleet standard — the 3-tier backbone (tenant_manager / tenant_superadmin /
          tenant_admin) with a one-owner-per-tenant partial-unique index, two-way succession,
          sub-role capability presets, and a tenant-scoped custom-role permission-matrix system
          (feature registry × strict-CRUD matrix, matrix-driven enforcement at tRPC + middleware +
          nav) becomes a framework default for every tenant-based app. Custom roles are tenant-
          scoped, capped at ≤ tenant_admin, and NEVER granted Billing or User-Management. Per-env
          seeded credentials come from the Server-Setups vault. Reference impl: Marine-Guardian
          feat/tenant-rbac-3tier (3-tier backbone proven; custom-role matrix authored as DESIGN).
Dependent files reviewed (propagation checklist — the machine-readable Cross-Alignment map):
  [x] Master_Prompt.md             — Rule 34 added; V32.25 changelog; Current totals + version markers bumped
  [x] rbac.md (.ai_prompt)         — NEW deliverable #29 (custom-role SYSTEM design) — authored by W3
  [x] scenarios.md                 — Scenario 42 (Existing-App RBAC 3-Tier Retrofit) — W3
  [x] ui-rules.md                  — pointer to rbac.md (no new numbered UI rule; UI rules stay 14) — W3
  [x] phases.md                    — Phase 0/4 3-tier seed + succession MODEL HOOK — W3
  [x] bootstrap.md                 — RBAC 3-tier backbone as a default governance assumption — W3
  [x] templates.md                 — seed + .env credential-key templates (vault-sourced) — W3
  [x] LESSONS_REGISTRY.md          — framework.auth.tenant-rbac-3tier-migration entry — W3
  [x] security.md                  — L3 (RBAC) block: 3 fixed tiers + one-owner index + matrix enforcement + guardrails
  [x] Security_Checklist.md        — new SECTION 21 (12 items); 135→147 items / 20→21 sections
  [x] deploy.sh                    — .ai_prompt/rbac.md added to the on-demand copy set
  [x] CLAUDE_compact.md            — Rule 34 one-liner; counts (33→34 Rules, 41→42 Scenarios, 28→29 deliverables); V32.25 segment
  [x] Framework_Feature_Index.md   — V32.25 row + footer version + counts
  [x] CLAUDE.md (project)          — canonical-counts block + deliverable enumeration (#29 rbac.md)
  [x] public/documentation-hub.html — sd-counts callout bumped
  [ ] Prompt_References.md/.html    — n/a (no prompt added; prompt count stays 62)
  Canonical counts:                Rules 33→34 · Scenarios 41→42 · deliverables 28→29 ·
                                    Security Checklist 135→147 / 20→21 sections ·
                                    Prompts 62 · UI Rules 14 · Bootstrap 20 · Phase Hooks 18 (UNCHANGED)
Ratified: 2026-07-10
═══════════════════════════════════════════════════════════════════════════════════ -->

> **MAINTENANCE CONTRACT — read this before ANY version bump:** on every future framework
> version bump, update the Sync Impact Report block above FIRST — new version delta, bump
> type (MAJOR/MINOR/PATCH — see the SemVer-for-rules subsection below), and which dependent
> files were reviewed ([x] done / [ ] not applicable this bump). This block is the
> machine-readable propagation tracker that REPLACES the retired `ChatGPT_Cross_Audit.md`
> role of "verify everything matches" — it lives at the top of the sole-authority file so it
> is never missed.
...
2         CLAUDE.md rules               This file — all 41 rules
...
**Spec-Persistence Model (V32.21 addendum).** Rule 1 names one of three named ways a
spec can relate to the artifacts derived from it. The framework names all three so a
divergence gets classified instead of argued about:

- **Flow-Forward** — completed artifacts are treated as immutable; any change is
  authored as a new feature or version rather than an edit to what already shipped.
- **LIVING-SPEC — AIEF's DEFAULT.** `docs/PRODUCT.md` is the Rule 1 contract; `inputs.yml`,
  the Prisma schema, and `docs/IMPLEMENTATION_MAP.md` are **disposable derivations**
  regenerated from it. The update order is always PRODUCT.md first, then regenerate
  down — this is exactly what Rule 1 + Rule 2 + Rule 3 already describe; naming it
  "Living-Spec" gives the existing behavior a label, it changes nothing about it.
- **Flow-Back** — the artifact closest to the work (including the code itself) is
  edited first, then the rest of the set is reconciled back up to the spec. This is
  the recovery path for brownfield/mid-project drift — see Scenario 40 (`scenarios.md`)
  for the 5-step reconcile loop and `scripts/spec-gap-check.sh` (deliverable #28) for
  the cross-artifact gap-check that tells you when a reconcile is needed.

Living-Spec is the default and requires no behavior change — it is what Rule 1 already
governs. Flow-Forward and Flow-Back are named so a team can consciously choose one for
a given artifact set (e.g. a locked compliance document might stay Flow-Forward) without
it reading as an accidental process violation. No new rule number — this rides Rule 1.
...
| **framework** | "`COPY . .` → phantom CVEs" | a deliverable (`lint-deploy.sh` Cn, a `templates.md` rule, a phase output contract) | **`deploy.sh` (automatic)** |
| **conductor** | "cheap-scout all surfaces before a per-surface wave" | a `/memory` feedback file | **auto-loads each session** |

Inheritance is therefore mostly already solved: framework-scope promotion = "edit the deliverable + add its check", and `deploy.sh` already carries deliverables into every new app; conductor-scope already auto-loads via `/memory`.
...
**Promotion** appends a registry entry and routes the resulting standing check to its scope's home (deliverable / hook / memory). Framework-scope checks inherit into new apps via `deploy.sh`; conductor-scope checks auto-load via `/memory`. This *extends* `lessons.md` and Smart Checkpoint (it does not replace them) and gives `superpowers:verification-before-completion` structural teeth.
...
### Rule 34 — Tenant RBAC Standard (3-Tier Backbone + Custom-Role Matrix) — every tenant-based app builds on the same role backbone; per-app roles never diverge (NEW V32.25)

Every framework app that is multi-tenant (the default multi-tenancy strategy of Rule 7) builds its authorization on **one fleet-wide role backbone**, not an ad-hoc per-app role set. `.ai_prompt/rbac.md` is the **on-demand RBAC authority** (read-on-demand, NOT auto-loaded — same posture as `security.md` / `privacy.md`): read it when writing auth, RBAC, user-management, or role-builder features, or when `PRODUCT.md`'s Roles & Permissions section is populated. This rule EXTENDS the L3 (RBAC) layer of the L1–L6 security stack — it does not replace it; L1 tenant scoping, L5 AuditLog, and L6 Prisma guardrails remain always-active underneath. Reference implementation: Marine-Guardian `feat/tenant-rbac-3tier` (the 3-tier backbone is proven; the custom-role matrix layer is authored as framework DESIGN and implemented per app at scaffold or next-touch).

**Site-access topology lives in Rule 41 (NEW V32.50), not here.** This Rule (34) defines WHO the roles are and HOW they're enforced (the 3-tier backbone + the tenant-scoped custom-role matrix). **Rule 41 — Site Access & Tenancy Bootstrap** defines WHERE each tier lands (`/tm` platform site, per-tenant role-routed `/{slug}/login` + `/{slug}/admin`, demo's no-`/tm` isolation) and layers a **data-driven platform-role system** (seeded `tenant_billing`/`tenant_tech` beside the fixed `tenant_manager` ADMIN default, `.ai_prompt/rbac.md` Part E) on top of the platform tier this Rule already names. Read both together for any auth/RBAC/routing task on a tenant-based app.

**The problem this solves:** each app inventing its own admin role names + its own enforcement wiring means no two apps share an authorization model, cross-tenant ownership is unconstrained (nothing stops two "owners" for one tenant), role renames lose data (DROP/CREATE on an enum wipes existing users' roles), and "custom roles" — when they exist at all — can silently escalate past the app owner or reach Billing/User-Management they were never meant to touch. The standard fixes all of these at the framework level so every app inherits a correct, uniform model.

**The 3-tier fixed system backbone (the same three top roles in every app):**
```
tenant_manager     — PLATFORM operator. tenant_id = NULL (not bound to one tenant). Break-glass:
                     can reassign a tenant's owner across tenants (audited). Multiple allowed.
tenant_superadmin  — the TENANT OWNER. EXACTLY ONE per tenant (DB-enforced). Sole holder of
                     Billing + User-Management for its tenant; the only role (with tenant_manager)
                     that creates/edits/assigns custom roles.
tenant_admin       — top in-tenant administrator BELOW the owner. Broad admin, but NEVER Billing or
                     User-Management. The CEILING every custom role must stay at or below.
```
Below `tenant_admin` sit the app's **domain roles** (e.g. Marine-Guardian keeps `field_coordinator` / `operator` / `viewer`). **The standard NEVER forces an app to rename its domain roles** — they keep their names and map onto the sub-role capability presets (Supervisor / Operator / Contributor / Viewer). Only the three top tiers are fixed, shared names.

**Proven mechanics (framework-prescribed, MG-verified) — these are the exact DDL patterns, do not invent divergent ones:**
- **Rename existing role enums with `ALTER TYPE … RENAME VALUE`, NEVER DROP/CREATE** — this preserves every existing user's (renamed) role with no data migration and no downtime. Retrofitting an existing app maps its current admin enum values → the three tier names this way (see Scenario 42 for the executable retrofit).
- **Enforce one owner per tenant with a PARTIAL unique index**, not a plain unique constraint: `CREATE UNIQUE INDEX one_tenant_superadmin_per_tenant ON users (tenant_id) WHERE role = 'tenant_superadmin' AND tenant_id IS NOT NULL`. The `WHERE role=… AND tenant_id IS NOT NULL` clause is load-bearing — it constrains owners per tenant while leaving platform `tenant_manager` rows (`tenant_id = NULL`) unconstrained so multiple platform managers remain allowed.
- **Succession is MANDATORY, both directions:** (1) break-glass — a platform `tenant_manager` can reassign a tenant's `tenant_superadmin` (reassigns `tenantId`, audited, rejects a non-existent tenant); (2) owner transfer — an owner promotes another user to `tenant_superadmin` and demotes self, mediated (promote-then-demote / transactional swap) so the partial-unique index is never violated mid-transfer. Both ship with tests as part of the auth scaffold.

**Custom-role permission-matrix system (data-driven for tenant_admin-and-below; the 3 top tiers stay fixed system roles).** Beyond the backbone, a tenant owner can build **tenant-scoped custom roles** from a permission matrix — the framework DESIGN lives in `.ai_prompt/rbac.md`:
- **Feature registry** — each app declares an enumerable, typed list of gateable modules/features that both the matrix and the role-builder UI read.
- **`role_permissions` matrix** — `(tenant_id, role_id, feature_key, view, write, update, delete)` with a **STRICT CRUD split** (write = create-only, update = edit-only, as SEPARATE columns). A custom role can be as narrow as View-only on one feature.
- **Matrix-driven enforcement wired identically at three surfaces** — a `hasPermission(role, feature, action)` resolver reads the matrix at tRPC procedures (a `matrixProcedure(feature, action)` factory), route middleware (deny-by-default), and sidebar nav (menu items filtered by `view`). Mirrors MG's proven 3-surface enforcement, generalized to the matrix.
- **Role-builder UI** — a `tenant_superadmin`-only screen (features × the 4 permissions checkbox matrix), shadcn/ui only, WCAG-gated for gov/LGU (paired with `accessibility-agents`). `ui-rules.md` carries a pointer (no new numbered UI rule).

**Guardrails (never violated):** custom roles are **tenant-scoped**, capped at **≤ `tenant_admin`** (a custom role can never exceed the in-tenant admin ceiling), and are **NEVER granted Billing or User-Management** (exclusive to `tenant_superadmin`). Only `tenant_superadmin` (and platform `tenant_manager`) may create, edit, or assign custom roles. Roles are ALWAYS derived from the server session — never trusted from the client (this inherits AGENT PROHIBITION #1).

**Credential model — vault-only, per-env (REFERENCE, never re-specify here).** The three tiers' seeded default accounts come from the Server-Setups vault `Server-Setups/secrets/universal-login-credentials.enc.yaml` (nested, keyed role × env): `tenant_manager` is a single UNIVERSAL platform account across all envs; `tenant_superadmin` and `tenant_admin` are per-environment defaults (local_dev / staging_prod / demo — demo has no `tenant_admin`). Seed passwords ALWAYS come from env (`bcryptjs`, never argv/hardcoded); dev weak creds gate on `SEED_DEV_ACCOUNTS=true` (`.env.dev` only). Operational footnote: the vault schema is 3-level nested and this `sops` version errors on 3-level `--extract` → decrypt-whole + parse. Values live ONLY in the vault — never pasted into a repo.

### Rule 35 — SEO Foundation (Adaptive Baseline) — every framework-built app is scaffolded with SEO out of the box; the baseline adapts to the route, never to a human's plan (NEW V32.30)

SEO is a **CONSTITUTIONAL, always-on concern** for every framework-built app — the same posture as the L1–L6 security stack, not a conditional PRODUCT.md-gated capability like `.ai_prompt/notifications.md` (Rule-less, Scenario 43). There is **no human planning/interview step for SEO and no new PRODUCT.md section**: Claude Code scaffolds the SEO Foundation automatically at Phase 4 and validates it at Phase 5, on every app, without being asked. `.ai_prompt/seo.md` is the **on-demand SEO Foundation authority** (read-on-demand, NOT auto-loaded — same posture as `security.md` / `privacy.md` / `rbac.md`): read it at Phase 4 (scaffold) and Phase 5 (validation), and again on the Scenario 44 retrofit.

**The problem this solves:** left unscaffolded, apps ship with default/duplicate `<title>` tags, no canonical URLs, no Open Graph previews, no sitemap, and — worse — an authenticated back-office route accidentally indexable by search engines. Making SEO opt-in (a PRODUCT.md checkbox) means most apps simply never get it, and a copy-pasted "index everything" default leaks internal/authed surfaces to search engines. The fix is to make it automatic and ADAPTIVE, so it is correct by construction on both sides of the auth boundary.

**Adaptive baseline (the core mechanic) — public vs. private, detected from the route's auth boundary:**
```
PUBLIC route (marketing/landing, docs, public listings, auth-less pages)
  → full SEO: indexable, canonical URL, Open Graph + Twitter card, JSON-LD structured data,
    included in app/sitemap.ts, hreflang alternates when multi-locale.

PRIVATE / AUTHED / INTERNAL route (dashboards, tenant admin, anything behind the auth
middleware / a (protected) route group)
  → private baseline: `robots: { index: false, follow: false }` (renders `noindex,nofollow`),
    excluded from app/sitemap.ts, and covered by app/robots.ts `disallow` rules.
```
The public/private split is detected from the app's existing route-group / middleware-auth boundary (the same boundary that already gates tRPC/session access) — it is never re-declared by hand per page, and it is never left to default-index (fail-closed: an undetermined route defaults to the private baseline, never the public one).

**Built entirely on Next.js App Router native primitives — no third-party SEO library:**
- `metadata` export / `generateMetadata()` (per `layout.tsx`/`page.tsx`) for title, description, and `robots`.
- `app/sitemap.ts` (`MetadataRoute.Sitemap`) — emits ONLY public routes; supports per-entry `alternates.languages` for multi-locale sitemaps.
- `app/robots.ts` (`MetadataRoute.Robots`) — `allow: '/'` + explicit `disallow` rules for every private route-group prefix, plus a `sitemap:` pointer.
- `alternates.canonical` (every public route) and `alternates.languages` (hreflang, only when `PRODUCT.md` App Identity declares more than one locale).
- `openGraph` + `twitter` metadata fields on every public route (Twitter auto-fills from Open Graph when omitted).
- Dynamic `opengraph-image` / `next/og` `ImageResponse` is an **opt-in** (only when `PRODUCT.md` signals per-page/per-post OG images are needed) — the default is a single static `defaultOgImage` from App Identity.

**Config sourcing — never a new interview.** Site name, base URL (`metadataBase` / `NEXT_PUBLIC_SITE_URL`), default OG image, and supported locales come from the **existing PRODUCT.md App Identity section** plus environment variables — Claude Code never fabricates these values and never re-interviews the human for them. If App Identity is silent on a value the scaffold needs (e.g. no base URL set for a given env), the gap is flagged, not guessed.

**JSON-LD structured data (public routes):** `Organization` + `WebSite` always; `BreadcrumbList` on nested public routes; extensible to `Article`/`Product` per app domain, emitted as a `<script type="application/ld+json">`.

**Enforcement (never optional):**
- **Phase 4 (scaffold)** emits `app/sitemap.ts`, `app/robots.ts`, root `metadata`/`generateMetadata`, and per-route-group public/private metadata wiring as part of the standard scaffold — not a follow-up task.
- **Phase 5 (validation)** gates on: every route carries metadata; `sitemap.ts` + `robots.ts` build clean; every public route has canonical + Open Graph + Twitter; every private route carries `noindex,nofollow` + is `robots.ts`-disallowed; structured data validates; hreflang is present when multi-locale. **Lighthouse SEO ≥ 90 is a HARD gate for apps with any public-facing surface** (advisory-only for fully internal/back-office-only apps).
- **Existing-app retrofit** follows **Scenario 44** (dev-first, LOCAL only — the deploy HARD HOLD applies exactly as it does to every other retrofit scenario; staging/prod promotion needs the owner's explicit word).
...
### Rule 40 — Invisible-Quality Radar — keep the deep-code, non-functional quality dimensions in active consideration as you build (NEW V32.47)

**A class of app-quality concerns is invisible in the running app** — security/vuln risk, SEO, accessibility, privacy/compliance, audit-ability, performance budget, observability, ISO/standards conformance, reliability, and maintainability. They live deep in the code and get **silently forgotten** because nothing in the running UI reminds you they exist (a `noindex` leak, an endpoint with no authz depth, a form with no ARIA, a query with no index — none *look* broken; they surface months later as an incident, a failed client audit, or a ranking gap). This Rule keeps the whole set **present**, proportional to task weight. It is an **ALWAYS-ON RADAR — an INDEX/POINTER, NOT a new enforcer and NOT a new blocking gate.** It mirrors the global `~/.claude/library/invisible-quality-radar.md`; **INHERIT-not-REPLACE** over the owner rules named below (the radar re-implements nothing — it just keeps them in view).

**The dimension registry (each dimension → the EXISTING owner surface the radar points at):**
- **D1 Security** → `security.md` L1–L6 (always-on) · `Security_Checklist.md` · `audit.md` Rule 38.
- **D2 SEO** → `seo.md` Rule 35 (+ §GEO).
- **D3 Accessibility (WCAG 2.2 AA)** → `ui-rules.md` Rule 13 · `motion.md` (SC 2.3.3); HARD gate on gov/LGU (`privacy.md`, DICT MC 004).
- **D4 Privacy / compliance (PH DPA / RA 10173)** → `privacy.md` Rule 33.
- **D5 Audit-ability** → `audit.md` Rule 38 · L5 AuditLog · `spec-gap-check.sh`.
- **D6 Performance budget** → `templates.md` R5b profiling · k6 · Lighthouse. ⚠ **IN-SCOPE-BUT-UNOWNED at build time** (owned only at Phase-5 validation) — the radar merely NAMES it; no new authority file.
- **D7 App-level observability** → ⚠ **IN-SCOPE-BUT-UNOWNED** (`microservices.md` names OpenTelemetry only on escalation; infra monitoring = Server-Setups). The radar NAMES it.
- **D8 ISO / standards conformance** → ⚠ **IN-SCOPE-BUT-UNOWNED**; the interim answer is the ISO/IEC 25010 vocabulary spine below.
- **D9 Reliability / resilience** → Phase-5 validation · the Rule-32/blast-radius regression net.
- **D10 Maintainability** → `audit.md` T1 (tsc/ESLint) · Rule 32 lessons.

**When it fires (weight-gated).** At the analyze-confirm task-start loadout of any REAL feature/build task, ask: *"which IN-SCOPE invisible-quality dimensions does THIS task touch, and is each one's owner surface loaded?"* A trivial one-liner / read / status check **skips the radar entirely** — that weight-gate protects both efficiency and signal. **Which dimensions are in-scope for THIS app** is read from the capability-primer FLAGS field `INVISIBLE_DIMS_IN_SCOPE:` (`build-primer.sh` #38), so the radar fires only for relevant dimensions and stays silent on the rest (a web-only internal tool gets no WCAG-hard-gate or Expo-perf nag; a public gov/LGU app gets the full set).

**Worked examples (illustrative — the radar firing, and correctly staying silent).** ① **Full-set — public gov/LGU permit form** (`public-facing · gov/LGU · personal data`): D1 submit-endpoint authz+validation, D2 public-route indexable metadata, D3 **WCAG HARD gate**, D4 RA-10173 lawful-basis/retention, D5 L5 AuditLog — five owners loaded at task-start, so the ARIA-less/`noindex`/no-retention defects are caught by design, not in a failed audit. ② **Scoped subset — internal "archived" orders filter** (`authed · business data`): only D1 (tenant scope + endpoint authz) and D6/D10 (unindexed `WHERE` + typed/lint) fire; D2/D3-hard-gate/D4 stay **correctly silent** (private, non-gov, no new personal data) — that silence is signal-protection, not an oversight. ③ **Skip — "fix the heading typo"**: trivial → the radar does **not** fire at all (the weight-gate protects efficiency). Full worked set + the ISO control-mapping detail live in the canonical global home `~/.claude/library/invisible-quality-radar.md` (this Rule mirrors it INHERIT-not-REPLACE).

**ISO/IEC 25010 — the vocabulary spine (conformance-AWARE / certification-READY, NOT an ISMS).** Adopt the 25010 product-quality characteristics (functional suitability · performance efficiency · compatibility · usability · reliability · security · maintainability · portability) as the registry's canonical vocabulary. Posture = MAP our EXISTING controls (L1–L6, L5 AuditLog, RBAC, `audit.md` toolkit, the WCAG gate, Server-Setups SOPS+age secrets) onto the ISO clauses they already satisfy (25010 + ISO/IEC 27001 Annex A control-mapping table) so a client's "are you ISO-aligned?" is answerable **with evidence**, WITHOUT running a management system. Escalate to a full ISO 27001 ISMS / certification track ONLY per-client on a real contractual trigger — recorded as a `[WHAT]` in that project's `PENDING_DECISIONS.md`, never pre-built.

**Enforcement is a PROMPT, not a wall.** Framework-side the radar adds only (a) a done-claim CONSIDERATION line at the Phase 7 output contract, and (b) an advisory `spec-gap-check.sh` **CLASS 6** (a declared-in-scope dimension with no evidence built — e.g. public app with no sitemap; data-sensitive app with no AuditLog) — both advisory, neither a new blocking gate. **HARD HOLD** — this Rule adds no deploy authority.

### Rule 41 — Site Access & Tenancy Bootstrap — every tenant-based app exposes the SAME site/URL topology: a platform `/tm` site, one role-routed per-tenant login, and a subdomain-isolated demo with no platform layer (NEW V32.50)

**This Rule owns the site-access TOPOLOGY — the URL/routing shape a tenant-based app exposes — as a companion to Rule 34's tenant-role MECHANICS.** Where Rule 34 / `.ai_prompt/rbac.md` define WHO the roles are and HOW they're enforced (the 3-tier backbone + the custom-role matrix), this Rule defines WHERE each tier lands: which URL prefix it reaches, which login form it uses, and how the platform operator's own site is isolated from every tenant and from demo. `.ai_prompt/rbac.md` **Part F** is the on-demand authority for the full topology detail; this Rule is the constitutional anchor.

**The 3-layer model — identical shape in every real environment (Local Dev, Staging, Production):**
```
/tm                        Tenant Management Site — the SaaS platform/server owner (Powerbyte)
   ├─ ADMIN    tenant_manager   ← default platform role (FIXED — Rule 34 unchanged)
   ├─ BILLING  tenant_billing   ← seeded, curated permission set
   └─ TECH     tenant_tech      ← seeded, curated permission set

/{client-slug}              Client Tenant — the subscriber's own space
   ├─ /{slug}/  → OPTIONAL public landing page (per-app flag; public, pre-login — rbac.md Part F2a)
   ├─ tenant_superadmin / tenant_admin → post-login landing /{slug}/admin
   ├─ (app-design RBAC roles below tenant_admin, per Rule 34 Part B)
   └─ regular users → login + landing /{slug}/login

/demo (optional)          Demo Tenant — separate subdomain stack; NO /tm layer (see below)
```

**The platform site (`/tm`) is data-driven, not fixed-only.** `tenant_manager` (ADMIN) stays the FIXED default ceiling — this does not change Rule 34's guarantee that the 3-tier tenant backbone is a stable, framework-constant contract. But the platform tier ALSO gains a **data-driven sub-role layer**: seeded `tenant_billing` (BILLING — subscription/billing management, no destructive tech ops) and `tenant_tech` (TECH SUPPORT — data overrides/technical support, no billing) ship by default, and MORE platform roles are creatable in-frontend by `tenant_manager` — the same "data-driven, matrix-backed" pattern Rule 34 Part B already established for tenant-scoped custom roles, applied to the platform scope. **Full DDL + resolver + guardrails: `.ai_prompt/rbac.md` Part E.**

**Two physically separate permission namespaces — the anti-escalation guarantee.** A platform role (`tenant_manager`/`tenant_billing`/`tenant_tech`/any future platform custom role) resolves permissions from a **distinct vocabulary** (`PlatformPermissionKey` / `PlatformRolePermission`) that is structurally separate from the tenant `FeatureKey` / `RolePermission` tables Rule 34 already defines — never the same table, never a shared enum, never cross-referenced. A **DB CHECK constraint** enforces the split at the schema layer: `(scope='tenant' AND tenant_id IS NOT NULL) OR (scope='platform' AND tenant_id IS NULL)`. This is the strongest anti-escalation guarantee available — a tenant-scoped role can **never** resolve a platform permission, and a platform account can never silently gain tenant-scoped access, because the two vocabularies do not share a resolution path even if application code has a bug.

**URL scheme — ONE role-routed login per tenant, never two forms.** A client tenant exposes exactly ONE login form at `/{slug}/login`. Post-login, the landing is role-routed server-side (never a client-chosen redirect): admin-tier (`tenant_superadmin`, `tenant_admin`) lands at `/{slug}/admin`; every other app role lands back at `/{slug}/login` as its home. The global `/admin` route (if an app still has one from before this Rule) is DROPPED in favor of the per-tenant `/{slug}/admin` — this closes the ambiguity of "which tenant does an un-scoped `/admin` even belong to." A client tenant MAY also expose an OPTIONAL public landing page at its slug root `/{slug}/` (a per-app flag) shown before login; when enabled it is public under the SAME exact-match exception as `/{slug}/login`, and when disabled `/{slug}/` redirects to login (`.ai_prompt/rbac.md` Part F2a).

**Demo is a separate subdomain stack with NO `/tm` layer — always.** Because a demo deployment is a single-tenant, client-facing stack (never a tenant merged into a real multi-tenant prod), it has **no Tenant Management Site at all** — there is no `/tm` route registered or reachable on a demo deployment. Demo accounts follow the standard client-tenant shape (`tenant_superadmin` + `tenant_admin`) with their own vaulted credentials; see `.ai_prompt/rbac.md` Part D/F.

**`/platform` → `/tm` rename — a redirect shim, owner-gated cutover.** Where an app already has a management surface at `/platform/*` (the pre-standard convention), the rename to `/tm/*` ships a redirect shim so existing bookmarks/`callbackUrl`/Traefik path-matching don't silently 404 mid-cutover — the cutover itself (removing the shim, flipping default routing) is owner-gated exactly like any other promotion (`deploy-discipline.md`). **Reserved slugs** — `tm`, `demo`, `platform`, `admin`, `login`, `api` — are rejected at tenant-slug creation time (case-insensitive, server-side) so a new tenant can never collide with a platform route.

**Retrofit path.** An existing tenant-based app adopts this Rule via **Scenario 50** (dev-first, LOCAL-only, HARD HOLD — same posture as Scenario 42). Full topology detail, the reserved-slug list, and the platform-role DDL: **`.ai_prompt/rbac.md` Parts E + F**. Security verification: **`Security_Checklist.md` Section 22**. **HARD HOLD** — adopting or changing an app's site-access topology lands as LOCAL commits only; no staging/prod/demo deploy or live re-seed without the owner's explicit word.
...
AI_Tools_Reference.md §2.5 + ui-rules.md "sanctioned design generator".
...
**Full specification:** see `Planning_Assistant.md` — Phase 2.8 section.
...
- **V32.54 (2026-08-31) — Content Human-Voice Pass (on-demand):** Adds on-demand deliverable **#44 `.ai_prompt/content-voice.md`**, which makes the globally-installed `humanize` + `ai-check` skills an automatic-when-content, never-invoked-by-name pass over human-facing published prose (marketing/landing/blog/caption/in-app/notification/generated-docs copy). ON-DEMAND — pulled ONLY when a content task fires, NOT a constitutional always-on concern (deliberately contrasted with Rule 35 SEO, which every app IS scaffolded with): never loaded in a non-content session, never scaffolded into an app. Write-human-first (humanize generate mode) OR humanize-rewrite → `ai-check` gate before content is "done"; rides ON TOP of `copywriting`/`app-showcase`/`doc-coauthoring` (INHERIT-not-REPLACE); SKIP for non-prose (code/config/JSON/data) + rigid/legal register. Strengthens the V32.53 humanize/ai-check phase cue from a loadout hint into a documented on-demand pass; wired at `phases.md` (Phase 2.8/3.3 mockup copy · Phase 7 feature-update content · light `ai-check` report-only at Phase 5 on public copy), `deploy.sh` (ships #44 to `.ai_prompt/`), and `AI_Tools_Reference.md` §3.9. **Counts: 44 deliverable files (was 43). All others UNCHANGED — 41 Rules · 51 Scenarios · 62 Prompts (39 NEW ✨) · 14 UI Rules · 18 Phase Hooks · 6 agents · 6 MCP servers · 20 Bootstrap Steps.** **HARD HOLD** — local commits only. Historical preservation (Rule 4): all prior changelog entries are unmodified.
- **V32.50 (2026-08-16) — Site Access & Tenancy Bootstrap Standard:** Adopts the owner-approved `docs/SITE_ACCESS_STANDARD.md` (FRMS, 2026-08-16) fleet-wide. New constitutional **Rule 41 — Site Access & Tenancy Bootstrap** owns the site/URL TOPOLOGY (companion to Rule 34's role MECHANICS): a `/tm` Tenant Management Site for the platform operator (Powerbyte) with a data-driven sub-role layer (`tenant_manager` FIXED default ADMIN ceiling unchanged, + seeded `tenant_billing`/`tenant_tech`, more creatable in-frontend); a per-tenant ONE role-routed login (`/{slug}/login`, admin-tier lands `/{slug}/admin`, global `/admin` dropped); and a demo deployment that is a separate subdomain stack with **NO `/tm` layer at all**. `.ai_prompt/rbac.md` gains **Part E** (platform-scope roles: `scope` discriminator on the existing custom-role tables, a **distinct** `PlatformPermissionKey`/`PlatformRolePermission` vocabulary physically separate from tenant `FeatureKey`/`RolePermission`, enforced by a DB **CHECK constraint** `(scope='tenant' AND tenant_id IS NOT NULL) OR (scope='platform' AND tenant_id IS NULL)` — the anti-escalation guarantee) and **Part F** (the URL topology detail, `/platform`→`/tm` redirect-shim rename, reserved-slug list `tm`/`demo`/`platform`/`admin`/`login`/`api`). New **Scenario 50** (Existing-App Site-Access & `/tm` Bootstrap Retrofit, dev-first/HARD-HOLD, mirrors Scenario 42's posture). New **Security_Checklist.md Section 22** "Platform Site-Access" (+12 items — CHECK constraint present, scope enforced server-side deny-by-default, `/tm` gated to platform roles, demo has no `/tm`, distinct vocab never cross-grantable, `/{slug}/login` EXACT-match public exception in BOTH guard layers, role-routed post-login). `phases.md` gains MODEL HOOKs (not new phase hooks — count stays 18): seed platform roles @ Phase 4 Part 3, extend the existing RBAC MODEL HOOK with scope enforcement + a `platformRole` router, a Phase 4 Parts 5-6 GATE-CLOSURE route-topology item, and a Phase 3.3 URL-topology cue. `templates.md` 7F seed template extended with 7G (platform BILLING/TECH accounts + demo `tenant_admin` slot). MINOR — purely additive; the existing 3-tier tenant backbone (Rule 34) is UNCHANGED, this is a NEW layer alongside it. **Counts: 41 Rules (was 40), 50 Scenarios (was 49), Security Checklist 147→159 items/21→22 sections — all others UNCHANGED: 62 Prompts (39 NEW ✨) · 39 deliverable files · 14 UI Rules · 18 Phase Hooks · 6 agents · 6 MCP servers · 20 Bootstrap Steps.** **HARD HOLD** — this standard + any per-app retrofit (FRMS first, per Scenario 50) land as LOCAL commits only; no staging/prod/demo deploy or live re-seed without the owner's explicit word. Historical preservation (Rule 4): all prior changelog entries are unmodified.
- **V32.48 (2026-08-10) — lint-design advisory expansion (P1k–P1u):** Records the framework-history version for the `lint-design.sh` #26 advisory expansion that shipped in repo release v32.47.1 (`4037da6`). Adds **11 new advisory checks P1k–P1u** — content/microcopy tells + static AI-slop tells — harvested from taste-skill §9.F + the impeccable rule registry; `CLAUDE.md` #26 advisory list **P1a-P1j → P1a-P1u**; `design-principles.md` Pillar 8 extended. All advisory (`--report-only`, exit 0) — nothing new auto-blocks a build. MINOR — additive behavioral (new lint checks), earns a MINOR headline per the ratified bump-type policy + the V32.45 precedent; no new Rule/Scenario/Prompt/deliverable, no count change (advisory checks are not counted). **Counts: UNCHANGED — 40 Rules · 49 Scenarios · 62 Prompts (39 NEW ✨) · 39 deliverable files · 14 UI Rules · 147 Checklist items/21 sections · 18 Phase Hooks · 6 agents · 5 MCP servers · 20 Bootstrap Steps.** **HARD HOLD** — local commits only. Historical preservation (Rule 4): all prior changelog entries are unmodified.
- **V32.47 (2026-08-08) — Invisible-Quality Radar (framework mirror):** STAGED Spec-Driven mirror of the global Invisible-Quality Radar (`~/.claude/library/invisible-quality-radar.md`). A class of app-quality concerns is **invisible in the running app** — security/vuln risk, SEO, accessibility, privacy/compliance, audit-ability, performance budget, observability, ISO/standards conformance, reliability, maintainability — they live deep in the code and get silently forgotten because nothing in the running UI reminds you they exist. New constitutional **Rule 40 — Invisible-Quality Radar** keeps the whole set in active consideration as an app is built: an **ALWAYS-ON, weight-gated INDEX/POINTER** (trivial tasks skip) that at the analyze-confirm task-start loadout asks *"which IN-SCOPE invisible dimensions does this task touch, and is each one's owner surface loaded?"* — **NOT a new enforcer, NOT a new blocking gate**, INHERIT-not-REPLACE over the existing owner rules (`security.md` L1-L6 · `seo.md` Rule 35 +§GEO · `ui-rules.md` R13 WCAG · `privacy.md` Rule 33 · `audit.md` Rule 38 · L5 AuditLog). Which dimensions are in-scope for an app is read from a NEW capability-primer FLAGS field **`INVISIBLE_DIMS_IN_SCOPE:`** (`build-primer.sh` #38; documented in `~/.claude/templates/capability-primer.md`). Teeth inside the framework are advisory only: a **Phase 7 done-claim CONSIDERATION line** + a new **`spec-gap-check.sh` CLASS 6** (a declared-in-scope dimension with no evidence built — e.g. public app with no sitemap; data-sensitive app with no AuditLog). **D6 perf-budget / D7 app-level observability / D8 ISO-conformance = NAMED-but-unowned** (the radar keeps them visible; no new authority file built — avoid over-engineering). **ISO/IEC 25010** adopted as the registry's vocabulary spine — posture is **conformance-AWARE / certification-READY** (map existing controls → ISO clauses so a client audit is answerable with evidence), **NO full ISMS** (escalate to an ISO 27001 certification track ONLY per-client on a real contractual trigger, recorded as a `[WHAT]`). MINOR — 1 new Rule, no removals; the mirror rides EXISTING files (no new deliverable). **Counts: 39→40 Rules · all others UNCHANGED — 49 Scenarios · 62 Prompts (39 NEW ✨) · 39 deliverable files · 14 UI Rules · 147 Checklist items/21 sections · 18 Phase Hooks · 6 agents · 5 MCP servers · 20 Bootstrap Steps.** **Zero-disruption** — nothing new auto-blocks a build; per-app adoption is a per-seat broadcast (deferred, owner-gated). **HARD HOLD** — local commits only. Historical preservation (Rule 4): all prior changelog entries are unmodified.
- **V32.46 (2026-08-08) — §GEO Generative Engine Optimization (on-demand extension of Rule 35):** Captures the single evidence-based cherry-pick from the 2026-08 SEO/GEO harvest (`AgriciDaniel/claude-seo`, MIT) as a new **§GEO** subsection appended to the always-on `.ai_prompt/seo.md` — an **ON-DEMAND** content/authoring extension of Rule 35, NOT a new constitutional Rule. GEO/AEO are rebranded labels for SEO (the same ranking systems decide eligibility; indexation — the always-on floor Rule 35 §§0–7 already delivers — is the prerequisite), so §GEO is read ONLY on a public-facing/marketing/content-heavy app at the design & content phases (§1.5) + the Phase-5 SEO step; internal/authed apps render `noindex` and never need it. Net-new levers: passage citability (self-contained ~130–170-word answer blocks, direct-answer-first), question-based headings, attribution density (sourced claims + stats + recency), entity presence/consistency, structured-data-for-AI (reuses the §4 JSON-LD mechanism — no new schema). Includes an explicit MYTH-LIST: `llms.txt` is NOT a Google ranking/citation lever, no magic content-chunking, no separate AI-keyword rewriting. Adds NO new Phase-5 gate (Rule 35 §6 unchanged) + a Scenario-44 retrofit content pass. MINOR — additive, extends Rule 35 / `seo.md`. **Counts at this bump: UNCHANGED — 39 Rules · 49 Scenarios · 62 Prompts (39 NEW ✨) · 39 deliverable files** (the subsequent V32.47 then took Rules 39→40). **HARD HOLD** — local commits only. Historical preservation (Rule 4): all prior changelog entries are unmodified.
...
- **V32.28 (2026-07-16) — Event Delivery & Notifications (conditional on-demand capability):** Adds a right-sized, FOSS multi-channel event-delivery / notification pattern as a NEW on-demand deliverable **`.ai_prompt/notifications.md` (#30)** + **Scenario 43 — Add Multi-Channel Event Delivery / Notifications**, WITHOUT a new Rule. Loaded ONLY when `docs/PRODUCT.md` declares a multi-channel notification / event-delivery need — an app without such a need never loads it, and the default locked stack is unchanged. **Right-sizing (avoids Kafka's tax):** Tier 1 DEFAULT = **Valkey Streams** (durable append-only log, consumer groups, `XACK`/PEL, `XAUTOCLAIM`) + **BullMQ** (routing/retry/backoff/DLQ/per-provider rate-limit) — both already in the locked stack, zero new infra; Tier 2 GRADUATION (opt-in, documented threshold) = **NATS JetStream** (Apache-2.0, single Go binary). Kafka/Redpanda reserved for hypothetical massive-scale (Redpanda is BSL). **6 mandatory additions** the canonical reference diagram omits: event schema+versioning (Zod = schema registry), tenant isolation (HARD — `tenant_id` on every event, never cross-tenant), notification preferences (+ quiet hours + cross-channel dedup), idempotency AT INGESTION, per-provider rate limits, PII/compliance routing (`privacy.md` Rule 33, RA 10173). Channels: email (SES/SendGrid) · push (Expo→FCM/APNs) · SMS (Twilio/SNS or PH-local) · webhook (HMAC-signed) · in-app (WS/SSE+DB) · Telegram. `templates.md` gains "OPT-IN: NATS JetStream"; `phases.md` gains a Phase 4 conditional MODEL HOOK. **Count change: deliverables 29→30, Scenarios 42→43.** Rules stay 34, prompts 62, UI rules 14, Bootstrap 20, Phase Hooks 18, Security Checklist 147/21. **Canonical counts after V32.28: 34 Rules · 43 Scenarios · 20 Bootstrap Steps · 62 Prompts (39 NEW ✨) · 14 UI Rules · 9 V32 Dispatch Rules · 18 Phase Hooks · 30 deliverable files · 6 memory-governance sections · 12 PRODUCT.md sections · Planning Assistant 12 rules · 5 MCP servers · 147 Security Checklist items / 21 sections.** Historical preservation (Rule 4): all prior changelog entries unmodified.
- **V32.17 (2026-06-26) — Design Anti-Slop Gate + Sharpened Craft Rules:** Adds a new deliverable **`scripts/lint-design.sh`** (deliverable count **25 → 26**) — a design anti-slop gate that checks **D1–D7 (seven cardinal sins)**: D1 = Tailwind default indigo/violet accent (`indigo-*` / `violet-*` / `#6366f1` / `#a855f7`); D2 = two-stop purple/blue/cyan "trust gradient" hero; D3 = emoji used as feature icons (✨🚀🎯 in headings / buttons / list items); D4 = hardcoded sans on display headings when a seed type binding exists; D5 = rounded card + coloured left-border ("AI dashboard tile") pattern; D6 = invented metrics ("10× faster", "99.9% uptime") without a real source; D7 = filler copy (lorem ipsum, "feature one/two/three", "placeholder text"). Plus **P1a (warning-only, never fails build):** ALL-CAPS text without `letter-spacing`/`tracking-*` on the same line or class. **All invocations at design phases (3.3 / Phase 4 Parts 5-6 / Phase 5) use `--report-only` (advisory, exit 0)** — surfaces findings for agent self-correction, never hard-blocks any design phase. Clones the `lint-deploy.sh` (#20) cheap-gate pattern (same colour/state/summary/exit-code shape). Also sharpens **`design-principles.md`** (#24, existing deliverable — no count change for this file): adds a **letter-spacing table** under Pillar 4 (Readability), a **"Five required render states" threshold guide** under Pillar 6 (Feedback), and a new **Pillar 8 — Anti-AI-Slop** (anchors the linter conceptually — documents D1–D7 as the seven cardinal sins with remediation guidance). Idea source: nexu-io/open-design `craft/` axis (Apache-2.0, surfaced via referodesign/refero_skill MIT). **Wired across the framework:** `deploy.sh` GROUP 9 copies `scripts/lint-design.sh` (overwrite-with-backup + chmod +x); `phases.md` MODEL HOOKs at Phase 3.3 / Phase 4 Parts 5-6 / Phase 5 reference `bash scripts/lint-design.sh --report-only src/`; `CLAUDE_compact.md` file-loading map + version marker + counts; `ui-rules.md` R13 carries a `+ lint-design.sh D1–D7 advisory` note. **Canonical counts after V32.17:** 33 Rules · 39 Scenarios · 20 Bootstrap Steps · 61 Prompts (38 NEW ✨) · 14 UI Rules · 9 V32 Dispatch Rules · 18 Phase Hooks · **26 deliverable files** · 6 memory-governance sections · 12 PRODUCT.md sections · Planning Assistant 12 rules · 5 MCP servers. **Migration:** run `deploy.sh` / `spec-update` on target app projects; restart Claude Code after deploy. **Historical preservation (Rule 4):** all prior changelog entries are unmodified.

- **V32.25 (2026-07-10) — Tenant RBAC Standard (3-Tier Backbone + Custom-Role Matrix):** Promotes the tenant-authorization model to a fleet-wide framework standard so every multi-tenant app builds on ONE role backbone instead of an ad-hoc per-app role set. Adds **Rule 34 — Tenant RBAC Standard** (the 3 fixed system tiers `tenant_manager` [platform, `tenant_id=NULL`, break-glass, many allowed] / `tenant_superadmin` [tenant owner, EXACTLY ONE per tenant, sole Billing+User-Management] / `tenant_admin` [top in-tenant admin below the owner, the custom-role ceiling, never Billing/User-Management]; app domain roles keep their names below `tenant_admin` and map onto Supervisor/Operator/Contributor/Viewer capability presets), a new on-demand deliverable **`.ai_prompt/rbac.md` (#29)** carrying the custom-role SYSTEM design (feature registry × a `role_permissions` STRICT-CRUD matrix `(tenant_id, role_id, feature_key, view, write, update, delete)` + a `hasPermission()` resolver wired identically at tRPC + route middleware + sidebar nav + a `tenant_superadmin`-only role-builder UI), and new **Scenario 42 — Existing-App RBAC 3-Tier Retrofit (MG pattern)** (the dev-first, LOCAL-only retrofit playbook). Proven mechanics the standard prescribes: rename role enums with **`ALTER TYPE … RENAME VALUE`** (data-preserving, NEVER DROP/CREATE) and enforce one owner per tenant with a **PARTIAL unique index** `unique(tenant_id) WHERE role='tenant_superadmin' AND tenant_id IS NOT NULL`; **two-way succession** (platform break-glass reassign + mediated owner-transfer) is mandatory and ships with tests. **Guardrails:** custom roles are tenant-scoped, capped ≤ `tenant_admin`, and NEVER grant Billing or User-Management (exclusive to `tenant_superadmin`); only `tenant_superadmin` (+ platform `tenant_manager`) create/assign custom roles; roles are always server-derived (inherits AGENT PROHIBITION #1). Per-env seeded credentials come from the Server-Setups vault `universal-login-credentials.enc.yaml` (nested role×env; `tenant_manager` universal, `tenant_superadmin`/`tenant_admin` per-env) — values vault-only, passwords from env via bcryptjs, dev creds gated on `SEED_DEV_ACCOUNTS`. `security.md` gains an L3 (RBAC) block; `Security_Checklist.md` gains **SECTION 21 — Tenant RBAC & Custom Roles (12 items)**; `phases.md`/`bootstrap.md`/`templates.md` seed the 3-tier backbone + credential env keys by default; `ui-rules.md` gains a pointer to `rbac.md` (no new numbered UI rule). Reference impl: Marine-Guardian `feat/tenant-rbac-3tier` (3-tier backbone proven; custom-role matrix authored as framework DESIGN, implemented per app at scaffold/next-touch). **Count change: Rules 33→34, Scenarios 41→42, deliverables 28→29, Security Checklist 135→147 items / 20→21 sections.** UI rules stay 14, prompts stay 62, Bootstrap steps stay 20, Phase Hooks stay 18. **Canonical counts after V32.25:** **34 Rules** · **42 Scenarios** · 20 Bootstrap Steps · 62 Prompts (39 NEW ✨) · 14 UI Rules · 9 V32 Dispatch Rules · 18 Phase Hooks · **29 deliverable files** · 6 memory-governance sections · 12 PRODUCT.md sections · Planning Assistant 12 rules · 5 MCP servers · **147 Security Checklist items / 21 sections**. **Historical preservation (Rule 4):** all prior changelog entries are unmodified.
- **V32.24 (2026-07-05) — Spec Expert Panel (SC2, SuperClaude harvest):** Harvests SuperClaude's multi-persona review pattern into a structured, gated pre-lock check on the spec itself. Adds **Prompt 3.24 — Spec Expert Panel Review** (Scenario Group 3) and **Scenario 41 — Spec Expert Panel review before spec-lock**: at **Phase 2.8** (design pre-handoff) and **Phase 3 pre-lock** (before spec files generate), the PM (Opus) dispatches **5 expert-lens Sonnet subagents IN PARALLEL**, each an ephemeral reviewer loading exactly ONE installed skill as its lens — `secure-code-guardian` (security/authz depth/data-privacy), `architecture-designer` (system structure/scalability/coupling/module boundaries), `api-designer` (API surface/contracts/versioning/integration seams), `test-master` (testability/coverage gaps/edge cases/acceptance criteria), `database-optimizer` (schema/data-model/indexing/query patterns/tenancy) — each scoped to `docs/PRODUCT.md` (+ `docs/DESIGN.md`/`docs/MOCKUP.jsx` at the Phase 2.8 pass) and returning prioritized findings (Critical/High/Medium). The PM **synthesizes** the 5 findings lists into one, **dedups** overlapping findings across lenses, **prioritizes** the merged list, and **feeds every finding into the Flow-Back / LIVING-SPEC reconcile** (Rule 1 Spec-Persistence Model addendum, Scenario 40's 5-step loop — BEHAVIOR findings become a proposed `docs/PRODUCT.md` edit the human applies per Rule 1; STRATEGY findings go to `docs/DECISIONS_LOG.md`; TASK-BREAKDOWN findings go to `docs/IMPLEMENTATION_MAP.md`). **New `phases.md` MODEL HOOKs** (NOT new `memory-governance.md §3` phase hooks — Phase Hooks count stays 18) at Phase 2.8 and at the Phase 3 pre-lock (added to the Phase 3 Output Contract checklist) implement the **GATE:** Phase 2.8 design handoff and Phase 3 spec-lock **CANNOT close while any CRITICAL Spec-Expert-Panel finding is unresolved** — a clean pass emits `✅ spec-expert-panel: clear`, an unresolved-Critical pass emits `⛔ spec-expert-panel: N Critical findings unresolved`. The Phase 2.8 gate is ADDITIVE alongside the existing V32.5.1 `/design-review` gate-closure (both must clear). The 5-expert roster is embedded directly in Prompt 3.24 / Scenario 41 — no new deliverable file. **Count change: Prompts 61→62 (38→39 NEW ✨), Scenarios 40→41 ONLY** — no rule/deliverable/phase-hook/MCP/security-checklist count change. **Canonical counts after V32.24:** 33 Rules · **41 Scenarios** · 20 Bootstrap Steps · **62 Prompts (39 NEW ✨)** · 14 UI Rules · 9 V32 Dispatch Rules · 18 Phase Hooks · 28 deliverable files · 6 memory-governance sections · 12 PRODUCT.md sections · Planning Assistant 12 rules · 5 MCP servers · 135 Security Checklist items / 20 sections. **Historical preservation (Rule 4):** all prior changelog entries are unmodified.

- **V32.23 (2026-07-05) — Constitution as Versioned Contract (H3):** Harvests the GitHub Spec-Kit "constitution" pattern (a versioned, machine-checkable governance contract) into the framework — the Master Prompt already IS the constitution; this makes its versioning + compliance-checking machine-readable instead of implicit. Three parts, all **COUNT-NEUTRAL**: **(a) Sync Impact Report** — a new HTML-comment header block at the very top of this file (above "WHAT THIS FILE IS"), seeded with `Version: V32.22 → V32.23`, `Bump: MINOR`, a one-line summary, and a `[x]`/`[ ]` propagation checklist against the same dependent-file set the Cross-Alignment Discipline in `CLAUDE.md` already names (`CLAUDE_compact.md`, `phases.md`, `Framework_Feature_Index.md`, `CLAUDE.md`) — this block MUST be updated FIRST on every future version bump, and is the machine-readable propagation tracker that replaces the retired `ChatGPT_Cross_Audit.md`'s "verify everything matches" role. **(b) SemVer-for-rules** — a new subsection next to the VERSION + FILENAME POLICY block defining MAJOR (removes/redefines a Rule/Phase/L1-L6 security level — requires a migration note), MINOR (additively introduces a Rule/Scenario/Prompt/gate/phase-hook/deliverable — backward-compatible), and PATCH (wording/count/typo fix — non-behavioral); the Sync Impact Report's `Bump:` field always cites one of these three. **(c) Constitution-Check gate** — a new universal pre-flight step in `phases.md`'s MANDATORY PRE-FLIGHT block (step 0b, alongside the existing V32.8 registry work-start consult) requiring that, before executing any planned task, Claude Code confirms the task does not violate any of the 33 Rules or the L1-L6 security model (esp. Rule 1 PRODUCT.md-as-source-of-truth, L3 RBAC, L5 AuditLog, L6 Prisma guardrails); a would-be violation is resolved as **[HOW]** (technical, decide and proceed) if it is an execution-level conflict, or escalated as **[WHAT]** (product/scope tradeoff, record in PENDING_DECISIONS.md) if it is a scope/business call — this is the app-BUILD companion to the Master Prompt's own Sync Impact Report: the SIR verifies FRAMEWORK edits propagate, the Constitution-Check verifies APP-BUILD tasks comply. **Zero rule/scenario/prompt/deliverable/phase-hook/MCP/security-checklist count change** — this is a governance-tooling addition, not a new Rule/Scenario/Prompt. **Canonical counts after V32.23 (unchanged from V32.22):** 33 Rules · 40 Scenarios · 20 Bootstrap Steps · 61 Prompts (38 NEW ✨) · 14 UI Rules · 9 V32 Dispatch Rules · 18 Phase Hooks · 28 deliverable files · 6 memory-governance sections · 12 PRODUCT.md sections · Planning Assistant 12 rules · 5 MCP servers · 135 Security Checklist items / 20 sections. **`ChatGPT_Cross_Audit.md` stays RETIRED** — the Sync Impact Report is its machine-readable successor for propagation tracking; do not resume maintaining it. **Historical preservation (Rule 4):** all prior changelog entries are unmodified.

- **V32.22 (2026-07-05) — Attack-Informed Security Hardening:** Harvests offense-side findings from the `Claude-Red` red-team methodology into defensive controls, so every framework-built app gets the hardening without loading any red-team skill. **`security.md` gains 10 new hardening blocks:** (1) JWT algorithm-pinning (reject `alg: none`, pin `HS256`/`RS256` explicitly, never trust the client-supplied `alg` header); (2) OAuth/SSO safety (state parameter CSRF binding, PKCE for public clients, redirect_uri allow-list exact-match); (3) open-redirect prevention (never redirect to a raw user-supplied URL; validate against an allow-list or same-origin check); (4) RCE/command-injection prohibition (no `eval`/`exec`/`child_process.exec` with untrusted input; parameterized/allow-listed subprocess invocation only); (5) race-condition/TOCTOU generalization (the L6 Prisma pattern extended beyond DB writes to file operations, cache-then-write, and balance/inventory checks); (6) prototype-pollution guard (freeze/validate `__proto__`/`constructor`/`prototype` keys on any user-controlled object merge); (7) cloud-credential safety (never log/echo access keys, short-lived STS tokens over static keys, scoped IAM policies, per-server secrets stay in Server-Setups not app repos); (8) mobile-app safety, conditional on PRODUCT.md native-mobile flag (no hardcoded API keys in the client bundle, certificate pinning guidance, secure storage for tokens); (9) request-smuggling note (front-end/reverse-proxy header normalization, Content-Length/Transfer-Encoding conflict awareness); (10) clickjacking (mandatory `X-Frame-Options`/`frame-ancestors` CSP directive on all HTML responses). **`Security_Checklist.md` gains SECTION 17 — AUTH TOKEN & OAUTH SECURITY, SECTION 18 — COMMAND / CODE EXECUTION SAFETY, SECTION 19 — CLOUD CREDENTIAL SAFETY (S3/R2/MinIO), and SECTION 20 — MOBILE APP SAFETY (CONDITIONAL — native mobile only): 114 → 135 items / 16 → 20 sections.** **Count change: Security Checklist 114→135 items / 16→20 sections ONLY** — no rule/scenario/prompt/deliverable/phase-hook/MCP count change. **Canonical counts after V32.22:** 33 Rules · 40 Scenarios · 20 Bootstrap Steps · 61 Prompts (38 NEW ✨) · 14 UI Rules · 9 V32 Dispatch Rules · 18 Phase Hooks · 28 deliverable files · 6 memory-governance sections · 12 PRODUCT.md sections · Planning Assistant 12 rules · 5 MCP servers · **135 Security Checklist items / 20 sections**. **Migration:** the harvested controls live in `security.md` + `Security_Checklist.md` (existing deliverables) — run `deploy.sh` / `spec-update` on target app projects to inherit them; restart Claude Code after deploy. **Historical preservation (Rule 4):** all prior changelog entries are unmodified.

- **V32.21 (2026-07-05) — Spec-Persistence Taxonomy + Cross-Artifact Gap-Check:** Two linked additions. **(A)** Rule 1 gains a **"Spec-Persistence Model" addendum** naming three ways a spec can relate to its derived artifacts — **Flow-Forward** (completed artifacts immutable; a change is authored as a new feature/version), **LIVING-SPEC** (**AIEF's DEFAULT** — `docs/PRODUCT.md` is the Rule 1 contract; `inputs.yml`, the Prisma schema, and `docs/IMPLEMENTATION_MAP.md` are disposable derivations regenerated from it; this names, without changing, what Rule 1/2/3 already require), and **Flow-Back** (the artifact closest to the work — including code — is edited first, then the rest of the set is reconciled up; recovery path for brownfield/mid-project drift). **No new rule number** — this rides Rule 1. **(B)** Adds new deliverable **`scripts/spec-gap-check.sh`** (deliverable count **27 → 28**) — a pure-bash, read-only, advisory cross-artifact gap-check. It detects four deterministic desync classes: DRIFT (a Prisma `model` with no matching `docs/PRODUCT.md` entity/section), UNBUILT (a `docs/PRODUCT.md` entity/feature with no matching Prisma model or `docs/IMPLEMENTATION_MAP.md` entry), PHASE/REALITY MISMATCH (`docs/STATE.md`'s `PHASE` claims a late phase while `docs/IMPLEMENTATION_MAP.md` still shows BLOCKED/PARTIAL/TODO areas), and DERIVATION DRIFT (an `inputs.yml` top-level key with no `docs/PRODUCT.md` mention). Supports `--report-only` (default, always exit 0) and `--check` (dry-run, exit 3 on any finding); `--file <relpath>` is path-hardened against absolute paths, `..` traversal, and symlink escape, mirroring `sync-context.sh` (#27). **NEVER writes any file and NEVER gates a phase** — purely advisory, exactly like `lint-design.sh` (#26) and `lint-deploy.sh` (#20). **Wired across the framework:** `deploy.sh` GROUP 11 copies it to `scripts/spec-gap-check.sh` (overwrite-with-backup + chmod +x); `phases.md` Phase 7 pre-flight gains a MODEL HOOK that runs `bash scripts/spec-gap-check.sh --report-only` and surfaces **"🔍 Cross-Artifact Gaps"** at the top of phase output — non-blocking, complementing the V32.5.5 (spec back-port) and V32.7.3 (design back-port) checks; this is **NOT a new `memory-governance.md` §3 injection point** (the 18 Phase Hooks count is unchanged); `memory-governance.md` §5 gains a "Flow-Back reconcile loop" subsection (section count stays 6); new **Scenario 40** — "Brownfield spec evolution: the Flow-Back 5-step reconcile loop" (capture → classify → update disagreeing artifacts → run the gap-check → resume); `Prompt_References.md`/`.html` Prompt 2.9 (Validate Spec Consistency) is EXTENDED, not renumbered, to run the script and enumerate the same 5-way surface (Prompts stay 61). **Canonical counts after V32.21:** 33 Rules · **40 Scenarios** · 20 Bootstrap Steps · 61 Prompts (38 NEW ✨) · 14 UI Rules · 9 V32 Dispatch Rules · 18 Phase Hooks · **28 deliverable files** · 6 memory-governance sections · 12 PRODUCT.md sections · Planning Assistant 12 rules · 5 MCP servers · 114 Security Checklist items / 16 sections. **Migration:** run `deploy.sh` / `spec-update` on target app projects. **Historical preservation (Rule 4):** all prior changelog entries are unmodified.

- **V32.20 (2026-07-05) — Managed-Context Region + sync-context.sh:** Adds new deliverable **`scripts/sync-context.sh`** (deliverable count **26 → 27**) — a pure-bash, idempotent regenerator for a marker-delimited `AIEF:MANAGED` region (`<!-- AIEF:MANAGED START -->` … `<!-- AIEF:MANAGED END -->`) inserted into `CLAUDE.md` at project root. The region renders a compact Phase / Locked-stack / Last-done / Next / Blockers / Recent-CHANGELOG snapshot from `docs/STATE.md` (fallback `.cline/STATE.md`), the locked-stack line in `docs/DECISIONS_LOG.md` (fallback `docs/PRODUCT.md`), and the last 3 `docs/CHANGELOG_AI.md` headlines — so a fresh Claude Code session sees live project state on load without a human re-summarizing it. **Idempotency is a hard contract:** the rendered block is a pure function of those source files; the only permitted "rev" identifier is `docs/STATE.md`'s `Updated:` field, falling back to `git rev-parse --short HEAD` — never `date` — and a second run that finds no change performs zero writes (no mtime bump, no `.bak`). Supports `--check` (dry-run, exit 3 if it would change) and `--file <relpath>` (defaults to `CLAUDE.md`); hardened against absolute paths, `..` traversal, and symlink escape — refuses to write anywhere outside the project root. If exactly one marker is present the file is treated as corrupt and the script aborts without writing; if no markers are present the section is appended at EOF. **Wired across the framework:** `deploy.sh` GROUP 10 copies it to `scripts/sync-context.sh` (overwrite-with-backup + chmod +x); `CLAUDE_compact.md` gains the `## CURRENT PROJECT STATE (auto-managed)` section carrying the marker block + a CONTEXTUAL FILE LOADING row + a FILE OWNERSHIP exception note (CLAUDE.md stays human-owned EXCEPT the AIEF:MANAGED region, owned by the script) + version marker bump; `memory-governance.md` §2 Target 1 + §3 Hook Text POST both instruct running it immediately after every STATE.md rewrite (mechanical tail step of the existing Smart Checkpoint POST action — the 18 Phase Hooks count is unchanged, this is not a new hook). **Canonical counts after V32.20:** 33 Rules · 39 Scenarios · 20 Bootstrap Steps · 61 Prompts (38 NEW ✨) · 14 UI Rules · 9 V32 Dispatch Rules · 18 Phase Hooks · **27 deliverable files** · 6 memory-governance sections · 12 PRODUCT.md sections · Planning Assistant 12 rules · 5 MCP servers · 114 Security Checklist items / 16 sections. **Migration:** run `deploy.sh` / `spec-update` on target app projects; the AIEF:MANAGED section is appended to CLAUDE.md at EOF on first run if the file predates V32.20. **Historical preservation (Rule 4):** all prior changelog entries are unmodified.

- **V32.19 (2026-06-30) — Anti-Slop Gate Expansion (designer-skill harvest):** extends the V32.17 design anti-slop layer with tells harvested concept-only (MIT) from Pythoughts-labs/designer-skill's 44-rule detector. `scripts/lint-design.sh` (#26) gains one new P0 cardinal sin — **D8 gradient/clip-to-text fill** (cardinal P0 sins 7→8, now D1–D8) — plus 8 advisory P1 warnings (P1b italic-serif display headline, P1c marketing buzzwords, P1d repeating-gradient stripes, P1e layout-property transitions [ties motion.md transform+opacity-only rule], P1f bounce/elastic easing, P1g justified body text, P1h image hover-transform, P1i crushed/negative letter-spacing). `design-principles.md` (#24) Pillar 8 broadens from 'the seven cardinal sins' to a catalogue of extended visual/type/motion/copy/token-drift tells added as agent + `/design-review` guidance (NOT auto-checked — they need a browser/DOM/contrast engine the bash gate can't run; the cream/beige tell is deliberately kept OUT of the lint to avoid false-positives on an intentional DESIGN.md cream theme). Posture UNCHANGED: advisory `--report-only` at all design phases, only P0 gates in `--strict` (Phase 5); INHERIT-not-REPLACE preserved (a deliberate docs/DESIGN.md choice is never slop). **Deliverable count UNCHANGED at 26** (extends #24 + #26, no new file).

- **V32.18 (2026-06-27) — App-Hardening Harvest (AI/LLM/MCP + API-Authorization + Injection-Family Security):** Harvested a curated 38-skill subset of the community **Anthropic-Cybersecurity-Skills** repo (Apache-2.0; web · API · auth · MCP/LLM · supply-chain · Docker) into the framework's inheritable security layer, so every framework-built app gets the controls without loading any skill. The 38 skills are vendored to `~/.claude/skills-library/Security & Testing/anthropic-cybersecurity-skills/` as on-demand red-team/defensive playbooks (a reference library — **NOT a 27th deliverable**). **`security.md` gains three new blocks (V32.18):** (1) **AI / LLM / MCP SECURITY** — all LLM input (direct AND indirect/RAG/tool/fetched) is untrusted; instruction–data separation as primary defense; tool/function-calling least-privilege + server-side argument validation + human approval for high-impact tools; MCP tool-poisoning / rug-pull / SSRF auditing; RAG corpus provenance + tenant-scoped retrieval; insecure-output-handling (never render/eval/SQL raw model output); secrets/PII + LLM consumption caps — mapped to **OWASP LLM Top 10:2025** + **MITRE ATLAS**. (2) **API AUTHORIZATION DEPTH** — BOLA (API1) · BFLA (API5) · BOPLA mass-assignment + excessive-data-exposure (API3), **OWASP API Top 10:2023**. (3) **INJECTION FAMILY** — NoSQL · XXE · SSTI · insecure-deserialization · CORS/host-header beyond Prisma-parameterized SQL. Plus an **Adversarial Verification** pointer block routing Phase-5/pre-launch hardening to the bundle's runnable test playbooks. **`Security_Checklist.md` gains SECTION 15 (AI/LLM/MCP, 7 items) + SECTION 16 (API Authorization & Injection Family, 9 items): 98 → 114 items / 14 → 16 sections.** **Count change:** Security Checklist 98→114 items / 14→16 sections ONLY. **Canonical counts after V32.18:** 33 Rules · 39 Scenarios · 20 Bootstrap Steps · 61 Prompts (38 NEW ✨) · 14 UI Rules · 9 V32 Dispatch Rules · 18 Phase Hooks · 26 deliverable files · 6 memory-governance sections · 12 PRODUCT.md sections · Planning Assistant 12 rules · 5 MCP servers · **114 Security Checklist items / 16 sections**. **Migration:** the harvested controls live in `security.md` + `Security_Checklist.md` (existing deliverables) — run `deploy.sh` / `spec-update` on target app projects to inherit them; restart Claude Code after deploy. **Historical preservation (Rule 4):** all prior changelog entries are unmodified.
- **V32.16 (2026-06-26) — Storage default stays MinIO; Cloudflare R2 demoted to a budget-gated OPT-IN (V32.15 reverted):** **Reverts V32.15's "R2 as the staging/prod default"** before any app was migrated, and re-establishes **MinIO as the staging/prod default** (the framework's long-standing behavior). R2 is retained as a **documented, owner-budget-gated OPT-IN** in `templates.md` — a ready, one-edit switch, NOT the default. **Why reverted:** R2's free tier (10 GB-month storage · 1M Class A · 10M Class B / month · $0 egress) is **per-Cloudflare-account, NOT per-bucket** — every app's bucket draws from one shared pool, and R2 has **no native per-bucket spend cap or clean per-bucket attribution**. With a growing fleet that means unbounded, hard-to-measure shared I/O and a real surprise-bill risk — the opposite of the cost *predictability* the move was meant to buy. Owner decision: **defer R2 adoption** until a per-app measurement/limit story exists; do not use it in staging or production yet. **What V32.16 keeps from the R2 evaluation:** (1) `templates.md` carries an **"OPT-IN: Cloudflare R2 for staging/prod"** block — the R2 `.env` values (`STORAGE_PROVIDER=r2`, endpoint `https://<account-id>.r2.cloudflarestorage.com`, `STORAGE_REGION=auto`, one bucket per app per env), the "remove `docker-compose.storage.yml` from staging/prod startup" step, R2 creds sourced from **Server-Setups `cloudflare-r2`** (never in-repo), AND a prominent **BUDGET CAVEAT** documenting the account-pooled free tier + no per-bucket cap; (2) the fleet evaluation (R2 vs Telegram-as-storage tg-s3/Pentaract) lives in conductor memory `reference_telegram_as_s3_storage`. **Default unchanged:** dev = MinIO; staging/prod = MinIO (S3-compatible) → externalize to AWS S3 or **R2 (opt-in)** by repointing the provider-agnostic `STORAGE_*` vars. **Files changed:** `Master_Prompt.md` (this entry + active-version), `templates.md` (R2 opt-in block + budget caveat), `CLAUDE_compact.md` / `CLAUDE_framework_repo.md` / repo `CLAUDE.md` (locked-stack reads `MinIO→S3/R2` — R2 an available target, not the default), `Framework_Feature_Index.md` (V32.16 row + header + footer), `ChatGPT_Cross_Audit.md` (version markers), `deploy.sh` (version markers). **ZERO count change** (all canonical counts identical to V32.14): 33 Rules · 39 Scenarios · 20 Bootstrap Steps · 61 Prompts (38 NEW ✨) · 14 UI Rules · 9 V32 Dispatch Rules · 18 Phase Hooks · 25 deliverable files · 6 memory-governance sections · 12 PRODUCT.md sections · Planning Assistant 12 rules · 5 MCP servers. **Migration:** none — apps remain on MinIO; adopting R2 later is the owner-run opt-in. **Historical preservation (Rule 4):** prior entries unmodified; the V32.15 entry was removed by a clean git revert (the change never reached an app) and is preserved in git history.
- **V32.14 (2026-06-24) — Motion Layer (prescribed web-animation layer):** Adds a new on-demand deliverable **`.ai_prompt/motion.md`** (deliverable count **24 → 25**) — a library-agnostic UI/UX motion-principles reference — and a new **UI Component Rule 14 "Motion & Micro-interactions"** in `ui-rules.md` (UI rules **13 → 14**). **`motion.md`** is read-on-demand like `security.md` / `privacy.md` / `design-principles.md` — NOT auto-loaded. Read at the design phases (**3.3 / Phase 4 Parts 5-6 / Phase 7**) when motion/interaction is in scope and `docs/DESIGN.md` / `ui-rules.md` are silent on a motion, timing, easing, or reduced-motion pattern. It covers: when-to / when-not-to animate (skip motion on high-frequency repeated actions); easing by intent (ease-out entrances/exits, ease-in-out on-screen movement, no `linear` for UI); duration budgets by element type; the **performance hard rule — animate `transform` + `opacity` ONLY, never layout props**; accessibility as first-class (**`prefers-reduced-motion` mandatory, reduced-motion is a design decision not a global kill-switch, ties to SC 2.3.3 and the WCAG 2.2 AA hard gate**); spring-vs-tween + gesture interruptibility + CSS-vs-JS as engineering decisions; and a **Motion+Tailwind appendix**. **Prescribed stack:** **Motion (motion.dev)** is the framework's only prescribed React animation library (it is the same primitive shadcn/ui already builds on — so shadcn-only is preserved), via the **LazyMotion/mini import path (~4.6KB)** with a **mandatory `useReducedMotion()` guard** on every animation. **GSAP is opt-in** — only on a PRODUCT.md marketing/scroll-storytelling signal (now 100% free incl. all plugins since the Webflow change; requires `@gsap/react` + a hand-written `gsap.matchMedia()` reduced-motion guard). **Three.js / React Three Fiber (R3F) are PARKED-AVAILABLE** for a future 3D PRODUCT.md requirement only (R3F is the correct entry point, not raw Three.js) — out of scope for standard apps. **INHERIT-not-REPLACE:** principles win structural/timing decisions, the design system (`docs/DESIGN.md` motion tokens) wins values; never regenerate motion ad hoc; conflicts logged to `DECISIONS_LOG.md`. **Wired across the framework:** `deploy.sh` copies it to `.ai_prompt/motion.md` (GROUP 8, overwrite-with-backup); `sync-to-project.sh` whitelist (24 → 25 files); `ui-rules.md` Rule 14 + cross-reference note; `phases.md` MODEL HOOK Read cues at the design phases; `CLAUDE_compact.md` (file-loading map row + version marker + counts); `Framework_Feature_Index.md` (V32.14 row + header + footer); `ChatGPT_Cross_Audit.md` (verification item + version markers). Motion principles informed by Emil Kowalski's "Animations on the Web" (animations.dev) and the MIT-licensed emilkowalski/skills + vercel-labs/open-agents skill files (restated in the framework's own words; a header credit line discharges all obligations). **Canonical counts after V32.14:** 33 Rules · 39 Scenarios · 20 Bootstrap Steps · 61 Prompts (38 NEW ✨) · **14 UI Rules** · 9 V32 Dispatch Rules · 18 Phase Hooks · **25 deliverable files** · 6 memory-governance sections · 12 PRODUCT.md sections · Planning Assistant 12 rules · 5 MCP servers. **Migration:** run `deploy.sh` / `spec-update` on target app projects; restart Claude Code after deploy. **Historical preservation (Rule 4):** all prior changelog entries are unmodified. **Deliverable filenames de-versioned (stripped `_v31` suffix)** — Git is now the version authority; in-file V32.x changelog markers unchanged. `deploy.sh` auto-cleans orphan `AI/Master_Prompt_v*.md` → `.bak` on sync.
- **V32.13 (2026-06-24) — CI → Docker Hub → Komodo-API Auto-Deploy (fleet Watchtower-free standard):** Every framework-scaffolded app now inherits, at its Docker/deploy phase, a **push-to-`main` → build+push image to Docker Hub → call the Komodo API to redeploy staging with the EXACT image SHA** pipeline — the fleet's standard, replacing the V27 Komodo registry-poll (hourly, too slow) and Watchtower for app deploys. **Why the API, not the webhook:** Komodo's git-listener webhook does NOT fire for `files_on_host` stacks and the registry poll is hourly, so CI drives the deploy via the API — `UpdateVariableValue` pins `<APP>_STAGING_TAG` to the SHA → `DeployStack` → poll `GetUpdate` (CI fails on a real deploy failure). Proven in production by the `fmo-fisherfolk` stack. **App-side artifacts scaffolded at the deploy phase** (vendored from the canonical Server-Setups sources — NOT new framework deliverables, exactly like the existing `start.sh` / `push.sh`): `deploy/komodo-deploy.sh` (portable CI helper; sends the Cloudflare-required browser `User-Agent` to `kmd.powerbyte.app`) + the V32.13 `.github/workflows/docker-publish.yml` (multi-arch build → push `sha-XXXXXXX` + `staging-latest` → `Deploy to Komodo (staging)` step). **Tag-variable contract:** staging compose service image = `${DOCKERHUB_USERNAME}/${IMAGE_NAME}:${STAGING_IMAGE_TAG:-staging-latest}`; Komodo staging stack env `STAGING_IMAGE_TAG = [[<APP>_STAGING_TAG]]` — deterministic SHA-pinned deploys + trivial rollback (set the variable to a prior SHA, redeploy). **Repo Actions secrets:** `DOCKERHUB_USERNAME` / `DOCKERHUB_TOKEN` + `KOMODO_API_KEY` / `KOMODO_API_SECRET` (the dedicated `github-actions-ci` Komodo key — independently revocable, never the master key). **Production is NEVER auto-deployed — manual promotion only** (fleet prod-gating policy; no prod step in the push-to-main workflow). **Canonical source of truth (vendor + point, do not reinvent):** `Server-Setups/Powerbyte-Hostinger/runbooks/komodo-ci-deploy.md` + `komodo/ci-deploy/{komodo-deploy.sh,docker-publish.template.yml}`. **Framework files changed:** `templates.md` (new **Rule 5c** — full templates + compose tag-variable contract + per-app enable checklist + manual-prod gate), `phases.md` (Phase 4 Part 8 `docker-publish.yml` V32.13 variant + `.env.staging` tag-variable note + Phase 6 staging-auto-deploy enable checklist), `Master_Prompt.md` (this entry + active-version + VERSION+FILENAME POLICY layer note), `Framework_Feature_Index.md` (V32.13 row + header + footer), `CLAUDE_compact.md` (version marker + deploy pointer), `ChatGPT_Cross_Audit.md` (verification item + version markers). **ZERO count change** — the workflow + helper are Phase-6 scaffold templates, not deploy.sh deliverables. **Canonical counts after V32.13 (all unchanged):** 33 Rules · 39 Scenarios · 20 Bootstrap Steps · 61 Prompts (38 NEW ✨) · 13 UI Rules · 9 V32 Dispatch Rules · 18 Phase Hooks · **24 deliverable files** · 6 memory-governance sections · 12 PRODUCT.md sections · Planning Assistant 12 rules · 5 MCP servers. **Migration:** no `deploy.sh` change required for the count; re-run `deploy.sh` / `spec-update` to pick up the updated `templates.md` + `phases.md`, then enable per app via the Rule 5c checklist. **Historical preservation (Rule 4):** all prior changelog entries are unmodified.
- **V32.12 (2026-06-24) — Design-Principles On-Demand Reference:** Adds a new on-demand deliverable **`.ai_prompt/design-principles.md`** (deliverable count **23 → 24**) — a library-agnostic UI/UX principles reference (hierarchy & layout, spacing, typography, the 9-state control contract, UX laws, WCAG by success-criterion, + a QA checklist), condensed from typeui.sh fundamentals (MIT). Read-on-demand like `security.md` / `privacy.md` — NOT auto-loaded. Read at the design phases (**2.8 / 3.3 / Parts 5-6 / 7**) when `docs/DESIGN.md` / `ui-rules.md` are silent on a pattern, component state, or accessibility approach. **INHERIT-not-REPLACE:** principles win for structural decisions, the design system (compiled tokens / `docs/DESIGN.md`, Rule 12) wins for token values; conflicts are logged to `DECISIONS_LOG.md`. Also hardens the Planning Assistant's `docs/DESIGN.md` QA gate (required component states + interactive patterns + anti-patterns + a QA checklist, Step 7b.2). Wired across the framework: `deploy.sh` copies it to `.ai_prompt/`; `phases.md` MODEL HOOKs at Phase 2.8 / 3.3 / Parts 5-6 / 7 carry the Read cue; `ui-rules.md` cross-references it as the fallback authority; the PA Step 7b.2 QA gate references it. **Canonical counts after V32.12:** 33 Rules · 39 Scenarios · 20 Bootstrap Steps · 61 Prompts (38 NEW ✨) · 13 UI Rules · 9 V32 Dispatch Rules · 18 Phase Hooks · **24 deliverable files** · 6 memory-governance sections · 12 PRODUCT.md sections · Planning Assistant 12 rules · 5 MCP servers. **Migration:** run `deploy.sh` / `spec-update` on target app projects; restart Claude Code after deploy. **Historical preservation (Rule 4):** all prior changelog entries are unmodified.
- **V32.11 (2026-06-23) — shadcn/studio Pro as the default design generator:** The owner's licensed shadcn/studio Pro MCP — a user-global, build-time generator built on shadcn/ui (output = plain shadcn/ui + Tailwind, MIT-compatible, so target apps carry NO runtime dependency on the Pro account) — becomes the framework's DEFAULT design-generation path, phase-routed: **Phase 3.3** (design finalization) = the `/cui → /iui → /rui` trio (`/cui` Create-UI builds page/section structure from Pro blocks · `/iui` Inspire-UI adds per-section distinctiveness, ONE section at a time, never whole pages · `/rui` Refine-UI polishes), THEN compile tokens → `/design-refine` → sign off → capture DESIGN baseline; **Phase 4 Parts 5-6** = `/cui` + `/rui` only (design is FROZEN at 3.3 — no `/iui`); **Phase 7** = `/cui` for new pages, `/iui` ONLY for a genuinely new distinctive section, `/rui` to polish. `/ftc` (Figma→Code) is CONDITIONAL — only when the design source is Figma AND the Figma MCP is present. **INHERIT-not-REPLACE (HARD):** every generated block carries its own tokens and MUST be reconciled to the app's compiled tokens (Rule 12) / `docs/DESIGN.md` — the block NEVER overrides the established design system. **Fallback** when the Pro MCP is unreachable: the plain shadcn/ui MCP + Blocks gallery (same shadcn/ui output target, lower automation). New MCP documentation: `AI_Tools_Reference.md §2.5`; the two stale "shadcn-studio = not recommended (paid)" notes (this file + `ui-rules.md`) reconciled to reflect adoption; `ui-rules.md` gains a "sanctioned design generator" subsection (no new numbered UI rule — count stable); `phases.md` MODEL HOOKs at Phase 2.8→3.3 / Phase 4 Parts 5-6 / Phase 7 carry the command routing. **Counts: MCP servers 4 → 5** (3 project-wired + 1 user-global + 1 plugin). **Canonical counts after V32.11:** 33 Rules · 39 Scenarios · 20 Bootstrap Steps · 61 Prompts (38 NEW ✨) · 13 UI Rules · 9 V32 Dispatch Rules · 18 Phase Hooks · 23 deliverable files · 6 memory-governance sections · 12 PRODUCT.md sections · Planning Assistant 12 rules · **5 MCP servers**. **Migration:** run `deploy.sh` / `spec-update` on target app projects; restart Claude Code after deploy. **Historical preservation (Rule 4):** all prior changelog entries are unmodified.
- **V32.9 (2026-06-19) — Compliance & Data Privacy Layer:** Adds **Rule 33 — Compliance & Data Privacy** (rule count **32 → 33**) and a new on-demand deliverable **`.ai_prompt/privacy.md`** (deliverable count **22 → 23**), read-on-demand like `security.md` and NOT auto-loaded. **Primary regime = PH Data Privacy Act of 2012 (Republic Act No. 10173) + NPC Implementing Rules, regulated by the National Privacy Commission (NPC); GDPR (EU) + CCPA/CPRA (US-California) are secondary deltas; the whole model is organized on ISO/IEC 27701 (PIMS — controller vs processor, control set).** The layer's 9 sub-items: (1) `privacy.md` is the on-demand compliance authority; (2) PH DPA primary / GDPR-CCPA secondary / ISO 27701 organizing; (3) the Planning Assistant captures a **gov/LGU client flag + DPO + data jurisdictions** in `docs/PRODUCT.md` (new Compliance section → PRODUCT.md sections **11 → 12**; PA gains a privacy-capture rule → PA rules **11 → 12**); (4) when the gov/LGU flag is set, **WCAG 2.2 AA is a HARD GATE at Phase 5** (a non-blocking WARN otherwise) — surfaced as a new UI accessibility rule (UI rules **12 → 13**); (5) the **6 data-subject rights** (be informed · object/withdraw · access · rectify · erase/block · portability) are implemented as app features via a tRPC DSR endpoint contract (access/rectify/erase/port/object) behind Auth.js + L5 AuditLog; (6) Prisma `ConsentLog` / `DataSubjectRequest` / retention models + BullMQ retention/erasure jobs; (7) **breach notification to NPC AND affected data subjects within 72 hours** (NPC Circular 16-03); (8) an **honest, configurable compliance-footer** — design-claim badges (e.g. "Designed for RA 10173 compliance", "WCAG 2.2 AA") ON by default, third-party certification badges (ISO 27001/27701, SOC 2, NPC registration number) OFF unless the client actually holds the certification; (9) a **STANDING gap-reminder** — the PA and the build phases actively flag missing privacy/protection items (no lawful basis, unrecorded consent, an unimplemented data-subject right, sensitive PII without heightened protection, no breach path). Delivered across the framework (Master Prompt Rule 33 + canonical counts here in S1; `privacy.md` deliverable in S1; counts propagation, Planning Assistant capture, `deploy.sh`, and the ChatGPT cross-audit follow in S2–S6). Adds the L1–L6 ↔ ISO 27701 / OWASP ASVS mapping pointer in `security.md` and a new A03/A10 ASVS-5.0 secure-generation pattern (S1). **Canonical counts after V32.9:** 33 Rules · 39 Scenarios · 20 Bootstrap Steps · 61 Prompts (38 NEW ✨) · 13 UI Rules · 9 V32 Dispatch Rules · 18 Phase Hooks · 23 deliverable files · 6 memory-governance sections · 12 PRODUCT.md sections · Planning Assistant 12 rules. *(The Post-Generation security-checklist item count is finalized in V32.9 S3.)* **Migration:** run `deploy.sh` / `spec-update` on target app projects; restart Claude Code after deploy. **Historical preservation (Rule 4):** all prior changelog entries are unmodified.
...
**PROPAGATION (derived in subsequent V32.8 tasks — Master Prompt is authored FIRST):** `Planning_Assistant.md` (Step 7 also emits `docs/tokens.json`); `phases.md` (Phase 3.3 compile + baseline, Phase 4 tokens-only, Phase 5/8 visual gate, Phase 7 mockup-first, registry consult hooks); `ui-rules.md` (compiled tokens only + palette disable — UI rule count bump); `templates.md` (tokens.json/SD-config/globals.css/visual-spec skeletons + acceptance-contract + evidence-field + registry-entry + Stop-hook config); `memory-governance.md` (§2 evidence field + learning-registry section + phase-hook bump); `bootstrap.md` (install style-dictionary, scaffold config + Stop hook + registry pointer); `CLAUDE_compact.md` (Rule 31/32 one-liners); `scenarios.md` (re-baseline, Phase-7-needs-mockup, recurrence-strengthen, promote-a-lesson); `AI_Tools_Reference.md` (SD + Playwright gate + registry/Stop-hook); `Framework_Feature_Index.md` (V32.8 row); `deploy.sh` (ship design toolkit + Stop hook + registry pointer + evidence templates); the 6 count-reference files (rule-count 30→32 + UI-rule + phase-hook bumps); `ChatGPT_Cross_Audit.md` (verification items, updated LAST). Canary: dogfood on Yelli-Basic (retro-apply the pipeline + retro-write acceptance contracts; promote the `COPY . .` lesson as the first framework-scope registry entry). Manual `sync-to-project`/`spec-update` only — never auto-rolled to existing apps.
...
**Files changed (full V32.1 propagation):** `memory-governance.md` (§1 new subsection), `Prompt_References.md` (Prompt 3.21 callout), `Prompt_References.html` (Prompt 3.21 callout), `Master_Prompt.md` (this changelog), `Framework_Feature_Index.md` (V32.1 row + footer), `CLAUDE_compact.md` (title + one-line reference), `ChatGPT_Cross_Audit.md` (K.11–K.15 verification items + verified counts header + footer).

**Commits:** `ab0578a` (Prompts 3.21/3.22 V32 alignment), `69e9190` (operational note shipped), plus this propagation commit.

**Migration:** No action needed in target projects — V32.1 is documentation/operational only. Restart Claude Code after `deploy.sh` to pick up the updated `memory-governance.md`.
...
- **V32.1.2 (2026-05-28):** Cross-file consistency fix flagged by ChatGPT cross-audit. AI_Tools_Reference.md Claude Code agent row rewritten to describe V32 Zero Opus Execution (was still describing V31.2-era Step 2.5/2.5b 30K-budget gate + Opus executor escalation as live behavior); Framework_Feature_Index.md header bumped to V32.1 with naming policy note; CLAUDE_compact.md role block bumped to V32.1 STRICTEST; **VERSION + FILENAME POLICY** block added to Master_Prompt.md header explaining that body labels saying "V31 primary" / "V31 STRICTEST" should be interpreted as base-V31 with V32/V32.1 patches layered on. Audit K.21–K.25 verification items added. Counts unchanged.

- **V32.1.3 (2026-05-28):** Deploy script count drift fix. deploy.sh missing-folder error path said "15 V32 reference files" — corrected to "16 V32 reference files" (canonical: 16 in .ai_prompt/ + deploy.sh at project root = 17 total). Framework_Feature_Index.md §1.2 subsection title "current in V31" updated to "current V32.1 behavior, V31 base". Audit K.26 verification item added for deploy script count consistency. Counts unchanged.

- **V32.1.4 (2026-05-29):** Deploy script post-install routing fix. deploy.sh "Next steps" message always recommended pasting prompt 1.2 (Universal Analyzer), which contradicted V32.1.1's Planning Assistant Step 7d guidance ("type Bootstrap" for fresh-from-PA projects). Users coming from Planning Assistant received two contradictory entry-point instructions. New behavior: deploy script detects Planning Assistant artifacts (`docs/PRODUCT.md` AND (`docs/DESIGN.md` OR `docs/MOCKUP.jsx`)) and conditionally routes — if PA signal present, displays V32.1.1 Bootstrap → Phase 2 → Phase 3 sequence with operational interview context; otherwise falls back to existing prompt 1.2 Universal Analyzer guidance. Both paths remain valid; deploy script now picks the right one based on observable state. Audit K.27 verification item added for conditional Next-steps routing. Counts unchanged.
...
- **Migration:** No deploy script change. Restart Claude Code after `deploy.sh` to pick up updated `memory-governance.md` + compact `CLAUDE.md`. R6–R9 activate automatically on next session.
...
- **Files changed (Phase 1, this commit):** `Master_Prompt.md` (Rule 4 amendment + R6 size qualifier + this changelog), `memory-governance.md` (R6 block updated + new Governance Extraction Schema sub-section in §4), `CLAUDE_compact.md` (header bump V32.2→V32.3, STRICTEST line extended, R6 line size-qualified, 9 GOVERNANCE DOCS section hydration note, Rule 4 one-liner hydrate-wording, non-negotiable behavior hydrate-wording).
- **Files changed (Phase 2 propagation):** `Framework_Feature_Index.md` (V32.3 row + §1.2 title + footer + deliverable set count line), `ChatGPT_Cross_Audit.md` (K.34 verification item + verified counts header + count lines), `phases.md` (Phase 7 MODEL hook addendum), `deploy.sh` (header bump cosmetic).
...
**Files changed:** `memory-governance.md` (§1 + §3 + §4 rewritten), `CLAUDE_compact.md` (budget gate + agent stack), `phases.md` (5 MODEL hooks), `Master_Prompt.md` (this changelog + scope updates), `Framework_Feature_Index.md` (V32 row + footer), `ChatGPT_Cross_Audit.md` (K.1-K.6 verification block + verified counts bumped), `deploy.sh` (version refs).
...
**Migration:** Same `deploy.sh` (file set unchanged). Restart Claude Code in each project. No reconciliation needed — V31.4 → V32 is a governance upgrade.
...
- `CLAUDE_compact.md`: Architect-Execute scope updated from "Phase 4/7/8" to "ALL phase work and ad-hoc edits."
- `phases.md`: All 5 phase pre-flight MODEL lines updated to imperative form matching the §3 hook rewrite.
- `Framework_Feature_Index.md`: V31.4 row added, footer bumped to V31.4.
