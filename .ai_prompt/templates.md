...
> `.clinerules` generation RETIRED V32.33 — Cline is deprecated; historical mentions below are preserved changelog only.
...
2. (V32.34: SpecStory retired — no `.specstory/config.json` step. Deployed apps built before V32.34
   keep any existing `.specstory/` as a frozen legacy audit trail; nothing to update on it.)
...
24. V11 (RETIRED V32.34): SpecStory config step no longer applies — see item 2 above
```
...
**Agent attribution values (V32.34 — git-sourced, collapsed to 2 rows; no more COPILOT/UNKNOWN):**
```
CLAUDE_CODE  → committed since $LAST (self-attributed, Rule 15). (Cline deprecated V31; .clinerules generation retired V32.33; SpecStory retired V32.34.)
HUMAN        → uncommitted (git diff HEAD) — manual edit, or in-flight Claude not yet committed
```
...
1. `docs/memory/lessons.md` — READ FIRST. Read ALL 🔴 gotcha entries in full. Then ALL 🟤 decision entries in full. For remaining entries: read ONLY entries whose title contains a keyword matching the current task domain. Skip all others.
...
Attribution detection priority (V32.34 — git-sourced, collapsed to 2 rows): CLAUDE_CODE (committed
since `$LAST`, self-attributed) → HUMAN (uncommitted, `git diff HEAD`, manual edit or in-flight Claude).
(Cline deprecated V31, SpecStory retired V32.34 — neither is part of the active attribution chain.)
See Rule 3 for full detection logic.
...
Smart Checkpoint). Governance Sync now sources diffs from git (Scenario 12). Rule number retained;
no behavior. Deployed apps keep `.specstory/history/` as a frozen legacy audit trail — never
migrated/deleted.
...
**STATE.md — the quick-read session file (NEW V14; retargeted to `docs/` V32.33):**
Bootstrap Step 16 creates `docs/STATE.md`. Every agent reads this FIRST (before the 9 governance docs) to orient instantly.
(Historical: pre-V32.33 apps created `.cline/STATE.md` — `design-stop-hook.sh`/`spec-gap-check.sh` carry a
permanent per-app read-fallback for those until they opt into Scenario 46's migration.)
...
evidence:
  contract: "[the machine-executable done-condition for LAST_DONE]"
  check_command: "[the exact command that proves it]"
  captured_output: |
    [real terminal output from check_command — never left empty; the Stop hook
    (scripts/design-stop-hook.sh) blocks a done-claim whose evidence is empty/absent]
```
See "`evidence:` block for STATE.md / Smart-Checkpoint" below for the full schema + a worked example.
...
**Rule 30 (formerly mirrored in .clinerules, retired V32.33):** See `CONTEXT7 LIVE DOCS — MANDATORY` section.
...
### `docs/tokens.json` — the DTCG v2025.10 token SOURCE (PA-derived, single source of truth)

`docs/tokens.json` is emitted by the Planning Assistant at Phase 2.8 Step 7b.5, DERIVED from the same
color/spacing/radius values in `docs/DESIGN.md` + `docs/MOCKUP.jsx`. **Style Dictionary compiles THIS file
directly** (`sd.config.mjs` → `source: ['docs/tokens.json']`) — there is deliberately **NO second
`tokens/tokens.json` copy** to drift out of sync (V32.37 R2 — closes RCA Seam A: the compiled `--sd-*` palette
IS the approved mockup's palette, provably, via CONTRACT 5). The block below is the FALLBACK seed used ONLY by a
mockup-less app that never ran the PA; a real app's `docs/tokens.json` already holds the derived values. Every
leaf must have `$value`. Never re-derive token values from `docs/DESIGN.md` prose into a separate file — that
re-introduces the transcription drift this closes.
...
> **AdminCN theme presets reconcile INTO this contract (V32.43), never replace it.** The AdminCN
> design starter ships `utils/theme-presets.ts` + `app/globals.css` (10+ presets). On seed (bootstrap
> Step 20d) pick ONE preset and fold its accent/radius/surface VALUES into `tokens.json` → the
> SD-compiled `--sd-*` layer above — do NOT bypass the three-layer bridge with AdminCN's raw preset
> CSS. This keeps `tokens.json` the single source (ui-rules Rule 12) while adopting the AdminCN visual
> baseline. See `.ai_prompt/admincn-starter.md`.
...
test.describe('Design token pipeline', () => {
  ...
  // ── CONTRACT 5: compiled palette equals docs/tokens.json (mockup fidelity) ──
  test('CONTRACT 5: --sd-color-primary equals docs/tokens.json $value (token-value equivalence)', async ({ page }) => {
    // Proves the build's compiled palette IS the approved mockup's declared palette (V32.37 R2),
    // not a coincidental match — reads the SAME derived docs/tokens.json the PA emitted at
    // Phase 2.8 Step 7b.5, not the generic seed.
    const fs = await import('fs');
    const tokens = JSON.parse(fs.readFileSync('docs/tokens.json', 'utf8'));
    const declared = tokens.color.primary.$value; // e.g. "#0066ff"
    ...
    // Browsers may normalise #0066ff → #06f — accept both forms of the SAME declared value.
    const shorthand = declared.length === 7
      ? `#${declared[1]}${declared[3]}${declared[5]}`.toLowerCase()
      : declared;
    expect([declared.toLowerCase(), shorthand]).toContain(compiled.toLowerCase());
  });
...
```json
{
  "scripts": {
    ...
    "design:fidelity": "node scripts/design-fidelity.mjs"
  }
}
```
...
### `scripts/design-fidelity.mjs` — mockup-anchored layout-fidelity gate

**Deployed script (deliverable #34), NOT a template the model instantiates** — `deploy.sh` copies it into
target `scripts/` (chmod +x), the same way `lint-design.sh` and `design-stop-hook.sh` are copied rather than
authored per-app. This section documents its contract so a build session understands what the gate checks and
why a violation blocks; it does not hand-write the script from this template.

**The anchor convention — `data-fdl="<name>"`.** Structural landmarks in both `docs/MOCKUP.jsx` and the
scaffolded/prototype components carry a stable `data-fdl` attribute. Anchors name **regions**, never data —
content-agnostic, so real app data vs the mockup's placeholder data never trips the gate. Matching between
mockup and build is by anchor name only. A screen is tagged with roughly 6-15 anchors covering its structural
skeleton; un-anchored elements are ignored by the gate. Standard anchor vocabulary (extend per-app as needed):

```
app-shell         sidebar           topbar            page-header
primary-content   kpi-row           data-table        detail-panel
footer
```

Author tags these in `docs/MOCKUP.jsx` at Phase 2.8 and carries them through the Phase 3.3 prototype (which
inherits MOCKUP.jsx markup) so the same anchors exist in the build. Dynamic-count regions (lists/tables) are
tagged once at the **container**, never per-row.

**`design-fidelity.config.json`** — per-app tolerance config, seeded at Bootstrap Step 20:
...
`posTol` = max allowed |Δx|/|Δy| (normalized 0..1 of viewport) before an anchor is flagged MOVED. `sizeTol` =
max allowed |Δw|/|Δh| before an anchor is flagged RESIZED. `baseUrl` = origin that **relative** manifest routes
(`/dashboard`) are joined onto before rendering (an absolute `http(s)://…` route is used verbatim). `viewport` =
the deterministic render size for BOTH baseline capture and the gate run — they must match for normalized
signatures to compare fairly.

**`docs/design-baseline/manifest.json`** — maps each screen to its mockup/app routes and the fixture state to
render for a deterministic comparison:

```json
{
  "dashboard": {
    "mockupRoute": "/mockup/dashboard",
    "appRoute": "/dashboard",
    "fixtureState": "seeded-demo-tenant"
  }
}
```

Per-screen captured layout signatures live at `docs/design-baseline/<screen>.layout.json` (committed), written
by `design:fidelity --update-baseline` from the **mockup** render at Phase 3.3 sign-off — the baseline is the
approved mockup, never a re-captured build render.

**Modes:** `--update-baseline` (capture signatures from the mockup routes in manifest.json) · default (render
the built app's routes per manifest + fixture state, diff vs baseline, **exit non-zero on any MOVED / RESIZED /
MISSING / EXTRA / REORDERED violation** — this is a HARD gate, unlike `lint-design.sh`'s `--report-only`/exit-0
posture) · `--report-only` (same diff, always exit 0, for advisory phases) · `--file <relpath>` (path-hardened
single-screen run, cloning the `lint-deploy.sh` guard pattern).
...
Validates that every token group in `docs/tokens.json` follows DTCG v2025.10 structure:
leaf tokens have `$value`; groups have at least one non-`$` child key. `docs/tokens.json` is the
Planning-Assistant-derived token source (Phase 2.8 Step 7b.5, V32.37 R2) that `sd.config.mjs` compiles
directly — so `design:build` emits the approved mockup's actual palette, and CONTRACT 5 asserts the
compiled `--sd-color-*` equals this file. There is no separate `tokens/tokens.json` to keep in sync.

```js
#!/usr/bin/env node
/**
 * design:validate — DTCG schema check for docs/tokens.json
...
const tokensPath = resolve(__dirname, '../docs/tokens.json');

let tokens;
try {
  tokens = JSON.parse(readFileSync(tokensPath, 'utf8'));
} catch (e) {
  console.error('❌ Could not read docs/tokens.json:', e.message);
  process.exit(1);
}
...
if (!allValid) {
  console.error('❌ docs/tokens.json DTCG validation FAILED');
  process.exit(1);
}

console.log(`✓ docs/tokens.json DTCG valid (${topKeys.length} groups: ${topKeys.join(', ')})`);
```
...
evidence:
  ...
  check_command: "npm run design:build && npx playwright test tests/visual/token-pipeline.spec.ts"
  captured_output: |
    ✓ docs/tokens.json DTCG valid (2 groups: color, dimension)
