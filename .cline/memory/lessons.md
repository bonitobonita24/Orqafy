...
## 2026-05-29 — 🟢 change Workspace TS package build pipeline pattern (Direction J)
- Type:      🟢 change
- Phase:     Phase 8 Direction J
- Files:     packages/jobs/package.json, packages/jobs/tsconfig.build.json, apps/worker/Dockerfile, apps/web/Dockerfile, apps/web/next.config.ts
- Concepts:  workspace-packages, typescript-build, exports-map, dockerfile, transpilePackages, extensionAlias, node-type-stripper
- Narrative: Workspace packages that ship at runtime to Node must compile to .js — exporting raw .ts via the package.json `exports` map works for vitest + Next.js webpack (which transpile on the fly) but FAILS at runtime under Node 22's type-stripper, which rejects .ts files under node_modules/. CANONICAL pattern (mirrors apps/worker): (1) tsconfig.build.json extending tsconfig.json with module/moduleResolution=NodeNext, outDir=./dist, declaration+declarationMap+sourceMap, noEmit:false, composite:false, exclude **/*.test.ts; (2) package.json adds main+types+files=["dist"] and conditional exports of form `{"./entry": {"types": "./dist/.../index.d.ts", "default": "./dist/.../index.js"}}`; (3) downstream Dockerfile inserts `RUN pnpm --filter @orqafy/<pkg> build` before the consuming app's build step in the builder stage; (4) Next.js next.config.ts can DROP the package from transpilePackages once it ships JS (db kept since it still ships source). webpack extensionAlias workaround (.js → .ts/.tsx) was added solely for jobs source resolution — removable once jobs ships dist, IF other workspace packages don't use .js-style ESM imports in their .ts source (verify with grep). Source files using `from "../foo.js"` ESM-style imports emit cleanly under NodeNext — do not change the source. pnpm-lock.yaml does NOT change when only adding scripts to package.json. Apply this pattern to ANY future workspace package that needs Node-runtime consumption (workers, scripts, CLI tools) — vitest/webpack-only packages can stay source-export.

## 2026-05-30 — 🔴 Q2 surface is 70+ models — purchasing was a 9-model slice
- Type:      🔴 gotcha
- Phase:     Phase 8 Batch 32 pre-flight
- Files:     packages/db/prisma/schema.prisma
- Concepts:  tenant-isolation, q2, schema-audit, defense-in-depth, scope-discovery
- Narrative: While scoping Vendor tenant parity (the 9th of 9 purchasing models flagged in Batch 26 pre-flight), full-schema grep revealed 70+ Q2 models (public schema + no tenantId). The "purchasing surface is fully Q2" claim in prior .whatsnext was a slice, not the universe. Direction I closed 9 of 70+. Remaining Q2 surface spans catalog (Product et al), HR (Employee, Payroll), accounting (Account, JournalEntry, FiscalYear), projects (Project, Milestone, Task), CRM (Customer, Quotation, Proposal), POS, support, job orders, finance. System tables (Tenant, Plan, User, Role, AuditLog) ARE intentionally Q2 (multi-tenant infra). All others appear to be tenant-scoped business data lacking isolation. Tests pass because fixtures don't exercise cross-tenant access. Future work: bank as Direction L; tackle module-by-module with the now-canonical pattern (top-level entities use tenants-table backfill, child entities use JOIN-from-parent backfill, dual-helper pattern from Batch 30). Reference: Batches 26-32 for canonical pattern.

## 2026-05-30 — 🟤 decision React 19 ReactPortal children-required workaround locked
- Type:      🟤 decision
- Phase:     Phase 8 Task #15 (typescript.ignoreBuildErrors removal, commit 428acf4)
