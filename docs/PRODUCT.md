...
### HR & Payroll
> **Build status (2026-06-16, see DECISIONS_LOG §C):** **BUILT** — PH statutory pay computation: `payroll.process` (DRAFT→PROCESSING) computes per-payslip gross → employee statutory deductions (SSS-EE, PhilHealth-EE, Pag-IBIG-EE, BIR withholding) → net, plus employer shares (SSS-ER/PhilHealth-ER/Pag-IBIG-ER) tracked as employer cost (new `Payslip.*EmployerShare` columns) and a JSON `deductionDetails` audit breakdown. Rates live in a configurable, tenant-scoped, effective-dated, **cited** `StatutoryRate` table seeded with 2025 PH values (owner-editable via `payroll.statutoryRate` procedures; `resolveRatesFromRows` falls back to cited 2025 defaults). `payroll.markPaid` (APPROVED→PAID) deducts the chosen FundSource for net pay (FundTransaction recorded) and posts the payroll Journal Entry (DR salaries + employer-statutory expense, CR Accounts Payable) per Core Flow 8, resolving the default expense/AP/fiscal-year mapping from `AccountingSettings` (clear error if unset). Payroll detail UI surfaces per-payslip SSS/PhilHealth/Pag-IBIG/withholding columns + lifecycle actions (Process / Approve / Mark Paid with fund-source picker). **Still data-entry / out of scope:** auto-generation of payslips from DTR/attendance, cash-advance recovery installments (manual amount per run remains as scaffolded). Statutory rate tables change annually — owner MUST verify the seeded values.
