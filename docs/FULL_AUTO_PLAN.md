...
### M7 — P2 security + a11y hardening  ✅ DONE (2026-07-11, 12 commits 75bc162..418a3c8)
- [x] **M7.1** (75bc162) Zod bypass tighten: `payroll.ts` StatutoryRate.upsert `config: z.record(z.string(),
      z.unknown())` → typed `z.discriminatedUnion("type",[...])` (sss/philhealth/pagibig/withholding, each
      `.strict()`). Grep confirms 0 remaining z.unknown()/z.any() on server mutation inputs.
- [x] **M7.2 — ESCALATED, not "low risk."** PM ground-truth verification + 3 read-only scouts (read-IDOR /
      list-leak / raw-FK-write) found REAL cross-tenant IDOR across **9 routers** (root cause: M5 removed
      the L6 auto-tenant-guard extension, so every query/mutation now needs EXPLICIT tenant validation).
      Fixed w/ `findFirst({tenantId})` / `loadXForTenant` / batch `count({in,tenantId})===n` / membership
      checks + regression tests, across: inventory (SEVERE record-IDOR on productUpdate + a full
      productList LIST-LEAK), project, job-order, purchasing (goodsReceipt.list LIST-LEAK + PO/GR nested
      FKs), pos, crm, tasks+support (bonus: unguarded taskAssign), employee, invoice/expense/department +
      defensive storefront. Scouts cleared 11 routers already-protected (banking/accounting/dtr/payroll/
      compliance/dsr/report/user/admin-xendit/smtp/notification). 10 commits, ~35 new regression tests.
- [x] **M7.3** (audit only, no code change) storage upload posture = STRONG (6/7 controls present: MIME
      whitelist, SVG/HTML blocked, forced-download disposition, tenant-slug key prefix, randomized UUID
      filenames, tenant-verified download). SSRF = none (only fixed-host outbound fetch is Turnstile
      siteverify). Gap: magic-byte sniff missing but architectural (presigned direct-to-S3 — server never
      sees bytes) + XSS vector already closed by SVG/HTML block. Decision: document-and-accept.
- [x] **M7.4** (7d5ac4a) installed shadcn `Skeleton`; replaced ad-hoc `animate-spin` in 10 app-shell
      loading.tsx w/ layout-matched placeholders (dashboard stat-grid+chart; 9 table pages uniform
      title+toolbar+rows). ui-rules Rule 11 PATH A, no skeleton twins.
- Verify: web typecheck 0 · web suite 1101/1101 (+~35 regression tests) · lint-design PASS · worker
  typecheck 0 · live smoke PASS.
- 2 minor residuals logged (not fixed): document-numbering info-leak (PO/GR/quotation sequences span
  tenants — `D-NUM-1`, product decision) + storage magic-byte accept (M7.3, documented above).
...
- 2026-07-11 — M7 COMPLETE (12 commits 75bc162..418a3c8, all LOCAL, verified green each step). M7.1: payroll.ts StatutoryRate config Zod bypass → typed discriminated union (0 remaining z.unknown()/z.any() on mutation inputs). M7.2 ESCALATED (the audit's "nested-include, low risk" call was WRONG): PM ground-truth review + 3 read-only scouts (read-IDOR/list-leak/raw-FK-write angles) found REAL cross-tenant IDOR across 9 routers — root-caused to M5's removal of the L6 auto-tenant-guard extension (every query/mutation now needs explicit tenant validation, and many didn't have it). Fixed inventory (SEVERE record-IDOR + a full list-leak), project, job-order, purchasing (list-leak + nested FKs), pos, crm, tasks+support, employee, invoice/expense/department + storefront — 10 commits, ~35 new regression tests; 11 routers scout-cleared as already-protected. M7.3: storage upload audit — posture strong (6/7 controls), magic-byte sniff gap is architectural + already mitigated, document-accepted (no code change). M7.4: shadcn Skeleton installed, replaced ad-hoc spinners in 10 loading.tsx. Final: web typecheck 0 · 1101/1101 · lint-design PASS · worker typecheck 0 · live smoke PASS. 2 minor residuals logged (D-NUM-1 document-numbering info-leak; magic-byte accept). Only owner-gated [WHAT] items + these 2 minor product-decision residuals remain → milestone-barrier reboot (no --stop) to advance, paced if genuinely nothing un-gated is left.

## M8 — IDOR coverage closure (verification only, no code change)  ✅ DONE (2026-07-11)
- Fresh session ground-truth check: M7.2 accounted for 24 routers (13 fixed + 11 scout-cleared) + 3 elsewhere (demo=M3, storage=M7.3, platform=intentionally cross-tenant). The other **6 routers had NEVER been examined** (auth, client, expense-category, plan, registration, tenant) — precisely the gap the prior session's own logged lesson "write-focused-IDOR-audit-undercoverage" warned about.
- Dispatched a read-only Explore scout with the exact post-L6 threat model (no implicit guard; every read/write must explicitly scope on trusted ctx.tenantId; validate all client-supplied FKs) + the established guard idiom. Verdict: **all 6 CLEAN**.
- PM ground-truth spot-verified the two HIGH-priority write routers at the cited lines: client.ts (list findMany+count tenant-scoped; byId findFirst({id,tenantId}); update/delete prior findFirst({id,tenantId}) guard before bare mutation) and expense-category.ts (update/delete findUnique then existing.tenantId===ctx.tenantId membership check). tenant.ts keys only off trusted ctx.tenantId; auth/plan/registration are public/global (no tenant context).
- **Result: full 33/33 real-router IDOR surface now accounted for.** No vulnerability found; no code change; working tree stayed CLEAN (HEAD dbe7bf0). Un-gated [HOW] queue is now genuinely EXHAUSTED. → TERMINAL-PENDING-OWNER: reboot-and-PACE (no --stop; owner gates open + owner likely asleep), do NOT re-audit next session.
