...
## ⭐ SESSION 2026-09-05 (pm-2, latest) — Full Auto: v0.20.0 release + CI security-gate root-cause & fix

```
[FOCUS: Orqafy]  ·  cold-start authority: docs/memory/MEMORY.md → latest session file

## ✅ DONE THIS SESSION
- **RELEASED v0.20.0** (`3dcc639`, tag pushed). Batch = `v0.19.0..HEAD` (19 commits; the 14 unpushed PLUS 5
  pushed-but-untagged). Bump **minor** — 3 feat commits: ORQ-23 fleet CI/CD standard, ORQ-24 coupled rollback,
  ORQ-25 EC2 retarget. Zero application source in the batch (deploy/ + docs/ + CLAUDE.md only).
  `gen-release-notes --apply` wrote CHANGELOG.md, synced the version into 10 package.json files AND
  `apps/web/src/components/layout/app-sidebar.tsx` (`APP_VERSION` fallback 0.19.0→0.20.0 — the footer-drift guard).
  Pushed `git push --follow-tags origin main`. Tree IN SYNC with origin.
- **Pre-push gate PASS (real exit codes, not self-report):** typecheck 0 (11/11) · lint 0 (11/11) ·
...
3. **ORQ-27(b)** demo-cron live-enable — still HELD pending explicit go to touch the EC2 box.
...
---
## SESSION 2026-09-05 (pm-1) — EC2 deploy retarget (ORQ-25) + coupled-rollback pairing (ORQ-24) + gov-sync plan
