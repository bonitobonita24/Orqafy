...
# cron job (see deploy/demo-reset-cron-install.sh — ORQ-25) or a Komodo scheduled action, do NOT
# wire it as an in-app scheduler. Do not install the cron trigger without the owner's explicit word.
#
...
# Prereq: SSH key $HOME/.ssh/powerbyte_ec2_komodo; run from the app repo root (so the DB package/migrate script match
#         the golden's expected schema baseline as closely as possible).
# Host:   demo lives on EC2-Komodo (ubuntu@18.138.220.90). SSH user `ubuntu` (docker group + passwordless sudo);
#         the stack .env is root-owned mode 600 → sudo for every .env read AND every `docker compose` run from
#         the stack dir (Compose auto-loads ./.env). /root is not accessible to ubuntu → golden + rolling
#         backups live under /home/ubuntu (ORQ-25). docker exec/run stay bare (ubuntu is in the docker group).
set -euo pipefail
...
ssh_vps "U=\$(docker exec ${PROJ}_postgres printenv POSTGRES_USER); D=\$(docker exec ${PROJ}_postgres printenv POSTGRES_DB); \
  docker exec ${PROJ}_postgres pg_dump -U \$U -d \$D | gzip > /home/ubuntu/orqafy-demo-rolling-backup-\$(date -u +%Y%m%d-%H%M%S).sql.gz && echo '  ok'"
...
ssh_vps "cd ${STACK}; sudo docker compose -p ${PROJ} ${CF} stop ${SERVICES} >/dev/null 2>&1 || true"
...
ssh_vps "cd ${STACK}; sudo docker compose -p ${PROJ} --env-file .env ${CF} up -d postgres"
sleep 5
...
DBURL_LOCAL=$(ssh_vps "sudo grep -oP '(?<=^INTERNAL_DATABASE_URL=).*' ${STACK}/.env" | sed -E "s#@[^/]+/#@localhost:${LPORT}/#")
...
ssh_vps "cd ${STACK}; sudo docker compose -p ${PROJ} --env-file .env ${CF} up -d ${SERVICES}"
