...
async function main() {
  ...
  for (const name of deptData) {
    ...
    await prisma.department.upsert({
      where: { tenantId_code: { tenantId: demoTenant.id, code } },
      update: {},
      create: { tenantId: demoTenant.id, name, code, isActive: true },
    });
  }
  ...
  for (let i = 0; i < expenseCats.length; i++) {
    ...
    await prisma.expenseCategory.upsert({
      where: { tenantId_code: { tenantId: demoTenant.id, code } },
      update: {},
      create: { tenantId: demoTenant.id, name, code, isActive: true, sortOrder: i },
    });
  }
  ...
  await prisma.warehouse.upsert({
    where: { tenantId_code: { tenantId: demoTenant.id, code: 'main-warehouse' } },
    update: {},
    create: { tenantId: demoTenant.id, name: 'Main Warehouse', code: 'main-warehouse', isDefault: true, isActive: true },
  });
